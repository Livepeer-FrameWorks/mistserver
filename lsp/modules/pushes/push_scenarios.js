...
function loadStreamList(callback) {
  function loadFromResponse(d) {
    ...
    let finished = false;
    const configuredStreams = mist.data.streams || {};
    const folderInput = findInput('Folder');
    ...
    function finishBrowse() {
      browseComplete++;
      if (browseRequests === browseComplete) finish();
    }

    for (const s in configuredStreams) {
      allthestreams.push(s);
      if (folderInput && configuredStreams[s].source && mistHelpers.inputMatch(folderInput.source_match, configuredStreams[s].source)) {
        allthestreams.push(s + '+');
        apiClient.send(function(bd, opts) {
          const sn = opts.stream;
          const files = (bd.browse && bd.browse.files) || [];
          for (const fi in files) {
            for (const inp in mist.data.capabilities.inputs) {
              ...
              if (mistHelpers.inputMatch(mist.data.capabilities.inputs[inp].source_match, '/' + files[fi])) {
                allthestreams.push(sn + '+' + files[fi]);
              }
            }
          }
          finishBrowse();
        }, { browse: configuredStreams[s].source }, {
          stream: s,
          hide: true,
          noLoginOnError: true,
          onError: finishBrowse
        });
        browseRequests++;
      }
    }
  ...
  apiClient.send(loadFromResponse, { active_streams: 1 }, {
    hide: true,
    noLoginOnError: true,
    onError: function() {
      loadFromResponse({ active_streams: [] });
    }
  });
}
