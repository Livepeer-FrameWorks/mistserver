...
function pushGuided(tab, other, prev, $main, $pageHeader) {
  ...
  const newPushBtn = el('button', {class: 'save', 'data-icon': 'plus'}, tr('New Push'));
  ...
  apiClient.send(function(d) {
    ...
    if (!hasAuto && !hasActive) {
      ...
      emptyState.appendChild(el('h3', null, tr('No pushes configured')));
      emptyState.appendChild(el('p', null, tr('Push a stream to another server, record to a file, or stream to platforms like YouTube and Twitch.')));
      const createBtn = el('button', {class: 'save', 'data-icon': 'plus'}, tr('Create your first push'));
    ...
    const $activeHeader = el('h3', {class: 'push-section-header push-section-header--collapsible', 'data-icon': 'activity'}, tr('Active Pushes'));
    ...
    const $autoHeader = el('h3', {class: 'push-section-header push-section-header--collapsible', 'data-icon': 'repeat'}, tr('Automatic Pushes'));
    ...
    const $varHeader = el('h3', {class: 'push-section-header', 'data-icon': 'variable'}, tr('Variables'));
    const $varIntro = el('p', {class: 'wizard-intro'}, tr('Variables are substituted in push targets, recording paths, and stream sources. Use them for reusable paths or conditional push scheduling.'));
    const newVarBtn = el('button', {'data-icon': 'plus'}, tr('New Variable'));
...
function buildActivePushCard(push, context_menu) {
  ...
  const stopBtn = el('button', {class: 'push-card-stop', 'data-icon': 'square'}, tr('Stop'));
  stopBtn.addEventListener('click', function(e) {
    e.stopPropagation();
    if (confirm(tr('Stop this push?\n') + push.stream + ' \u2192 ' + push.target)) {
...
function buildAutoPushCard(push, context_menu) {
  ...
  toggleInput.addEventListener('change', function() {
    ...
    if (deactivated) {
      ...
      newPush.stream = pr.deactivateStream(streamName);
    }
    let o = {};
    o[push.id] = newPush;
    apiClient.send(function() {
  ...
  const openContextMenu = function(pos) {
    ...
    if (deactivated) {
      ...
      actions.push(['Disable', function() {
        const o = {};
        o[push.id] = Object.assign({}, push, {stream: pr.deactivateStream(streamName)});
        apiClient.send(function() {
          tabView.showTab('Push');
          context_menu.hide();
        }, { push_auto_add: o });
    ...
    actions.push(['Remove', function() {
      if (confirm(tr('Remove this automatic push?\n') + streamName + ' to ' + push.target)) {
        apiClient.send(function() {
          tabView.showTab('Push');
          context_menu.hide();
...
function buildActiveTable($container, d, menuHost) {
  ...
  const $table = dynamicUI.dynamic({
    create: function() {
      ...
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Stream'}, tr('Stream')));
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Target'}, tr('Target')));
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Statistics'}, tr('Statistics')));
    ...
    add: {
      ...
      update: function(push) {
        ...
        if (lastTd) {
          ...
          const actionsBtn = el('button', {class: 'push-row-actions', type: 'button', title: 'Actions'}, tr('Actions'));
...
function buildAutoTable($container, d, menuHost) {
  ...
  const $table = dynamicUI.dynamic({
    create: function() {
      ...
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Stream'}, tr('Stream')));
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Target'}, tr('Target')));
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Conditions'}, tr('Conditions')));
      $header.appendChild(el('th', {class: 'header', 'data-index': 'Notes'}, tr('Notes')));
      ...
      create: function(id) {
        ...
        const openContextMenu = function(pos) {
          ...
          if (push.deactivated) {
            ...
            actions.push(['Disable', function() {
              push.stream = pr.deactivateStream(push.stream);
              let o = {};
              o[push.id] = push;
              apiClient.send(function(rd) {
                $table.update(rd.auto_push);
      ...
      update: function(push) {
        ...
        toggleInput.addEventListener('change', function() {
          ...
          if (push.deactivated) {
            ...
            let o = {};
            o[push.id] = Object.assign({}, push, {stream: pr.deactivateStream(push.stream)});
            apiClient.send(function(rd) { $table.update(rd.auto_push); }, { push_auto_add: o });
          }
        });
...
function pushAdvanced(tab, other, prev, $main, $pageHeader) {
  ...
  apiClient.send(function(d) {
    ...
    const settingsHeader = el('h3', {class: 'push-section-header', 'data-icon': 'settings'}, tr('Automatic Push Settings'));
    ...
    const $varSection = el('section', {class: 'push-section push-variables-section'});
    const $varSectionHeader = el('h3', {class: 'push-section-header push-section-header--collapsible', 'data-icon': 'variable'}, tr('Variables'));
