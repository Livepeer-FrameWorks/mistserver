...
function buildAssemblyPreview(callbacks, ctx) {
  ...
  for (let i = 0; i < completed.length; i++) {
    ...
    if (step.id === 'scenario') {
      ...
      chipValue.textContent = count ? trn('%s parameter', '%s parameters', count, count) : tr('Defaults');
    } else {
...
function buildVarReference(vars, opts) {
  ...
  const label = el('span', {class: 'var-reference-label'}, opts.label || tr('Variables'));
  ...
  for (let i = 0; i < vars.length; i++) {
    (function(v) {
      ...
      item.addEventListener('mouseenter', function(e) {
        ...
        const tipLine = el('div', {class: 'var-reference-tip'}, tr('Click to insert'));
...
function buildStreamReference(suggestions, inputEl) {
  ...
  const labelEl = el('span', {class: 'var-reference-label'}, tr('Stream hints'));
  let emptyEl = el('span', {class: 'stream-reference-empty'}, tr('No matching streams'));
  const items = [];
...
function buildTimeReference(inputEl) {
  ...
  row.appendChild(el('span', {class: 'var-reference-label'}, tr('Quick time')));
  const actions = [
    { label: 'Now', offset: 0 },
  ...
  for (let i = 0; i < actions.length; i++) {
    (function(action) {
      const btn = el('button', {type: 'button', class: 'var-reference-item time-reference-item'}, tr(action.label));
  ...
  const clearBtn = el('button', {type: 'button', class: 'var-reference-item time-reference-item clear'}, tr('Clear'));
...
function buildTargetStep($container, ctx, callbacks, allthestreams) {
  ...
  if (!scenario) {
    $container.appendChild(el('p', null, tr('Please select a scenario first.')));
    callbacks.markValid(false);
  ...
  const introText = scenario.isPlatform
    ? tr('Configure the stream source and %s destination.', tr(scenario.title))
    : tr('Configure the stream source and push target.');
  ...
  if (ctx._scenario === 'record_dvr') {
    ...
    prefixDiv.appendChild(el('span', {class: 'push-platform-label'}, tr(scenario.title)));
    ...
    lprefixDiv.appendChild(el('span', {class: 'push-platform-label'}, tr(scenario.title)));
...
function buildParamsStep($container, ctx, callbacks) {
  ...
  universalElements.push(el('h4', null, tr('Track Selection')));
  const trackHint = el('p', {class: 'wizard-intro'}, tr('Selectors are grouped by track type. Metadata-based hints load automatically for active streams.'));
  ...
  universalElements.push(el('h4', null, tr('Timing')));
  const timingHint = el('p', {class: 'wizard-intro'}, tr('Media timing bounds will be filled from stream metadata when available.'));
  ...
  if (isFile) {
    universalElements.push(el('h4', null, tr('Recording')));
  ...
  function applyMetaHints(hints) {
    ...
    if (hints) {
      trackHint.textContent = tr(
        'Detected tracks: %s video, %s audio, %s subtitle.',
        hints.trackCountByType.video,
        hints.trackCountByType.audio,
        hints.trackCountByType.subtitle
      );
    } else {
      trackHint.textContent = tr('No live track metadata found for this stream yet. Generic selector hints are shown.');
    }

    if (hints && hints.timing) {
      timingHint.textContent = tr(
        'Detected media range: %sms to %sms (max duration %ss).',
        hints.timing.minMs,
        hints.timing.maxMs,
        hints.timing.maxDurationS
      );
      ...
      timingHint.textContent = tr('Media timing bounds will be filled from stream metadata when available.');
      if (timingFields.recstart) {
  ...
  if (connector && mist.data.capabilities.connectors[connector]) {
    ...
    const connectorParameters = conn.push_parameters || {};
    const push_parameters = Object.assign({}, connectorParameters);
    ...
    for (const k in connectorParameters) {
      processParam(connectorParameters[k], k);
    }
    ...
    if (listHasInteractiveFields(classified.basic) || listHasInteractiveFields(classified.advanced)) {
      $container.appendChild(el('h4', null, friendly + tr(' Parameters')));
...
function buildScheduleStep($container, ctx, callbacks) {
  ...
  for (let i = 0; i < modes.length; i++) {
    (function(mode) {
      ...
      head.appendChild(el('div', {class: 'push-schedule-card-title'}, tr(mode.title)));
  ...
  function buildScheduleFields() {
    ...
    if (ctx._scheduleMode === 'now') {
      fieldsDiv.appendChild(el('p', {class: 'wizard-intro'}, tr('The push will start immediately when you click Save.')));
      return;
    }
    ...
    if (ctx._scheduleMode === 'always') {
      fieldsDiv.appendChild(el('p', {class: 'wizard-intro'}, tr('The push will run whenever the stream is active and will retry on failure.')));
      ...
      const newVarBtn = el('button', null, tr('New Variable'));
      ...
      elements.push(el('h4', null, tr('Start condition')));
      elements.push({
        label: 'Variable',
        type: 'str',
      ...
      elements.push(el('h4', null, tr('Stop condition (optional)')));
      elements.push({
        label: 'Variable',
        type: 'str',
        classes: ['endVariable'],
...
function buildReviewStep($container, ctx, callbacks) {
  ...
  function row(label, value) {
    ...
    tr.appendChild(el('td', null, value || tr('(not set)')));
    return tr;
  }
  ...
  scenarioTable.appendChild(row('Type', scenario ? tr(scenario.title) : tr('Unknown')));
  ...
  if (ctx.stream && ctx.stream[0] !== '#' && ctx.stream.indexOf('+') < 0) {
    ...
    if (mist.data.streams && !(base in mist.data.streams)) {
      ...
      warn1.textContent = tr('Warning: stream "%s" is not currently configured. The push may fail if this stream does not exist.', base);
      review.appendChild(warn1);
    }
  }
  if (ctx._scheduleMode === 'now') {
    let warn2 = el('div', {class: 'review-warning'});
    warn2.textContent = tr('This is a one-time push that starts immediately. It will not automatically restart if the stream goes offline.');
...
registerTab('Push Config', function(tab, other, prev, $main, $pageHeader) {
  ...
  apiClient.send(function() {
    ...
    function buildWizard() {
      ...
      const uriLabel = el('label', null, tr('Target:'));
