...
export function pushes(options){
  ...
  apiClient.send(function(d){
    ...
    if (options.push_settings) {
      ...
      settingsSection.appendChild(el("h3",null,tr("Automatic push settings")));
    ...
    function buildPushCont(type,values) {
      let layout = {
        "": function(push){
          ...
          let sortSpan = el("span",null,push.deactivated ? "Disabled" : tr("Enabled"));
        ...
        Stream: function(push){
          ...
          a.addEventListener("contextmenu",function(e){
            ...
            if (streamname.indexOf("+") < 0) {
              editLabel.textContent = tr("Edit stream");
            } else {
              editLabel.innerHTML = tr("Edit <b>")+streamname.split("+")[0]+"</b>";
            }
            const gototabs = [
        ...
        Actions: function(push,type) {
          ...
          stopBtn.addEventListener("click",function(e){
            ...
            if (confirm(tr("Are you sure you want to ")+this.textContent.toLowerCase()+" this push?\n"+push.stream+' to '+push.target)) {
          ...
          if (type == "Automatic") {
            if (options.stop_pushes) {
              const spBtn = el("button",null,tr("Stop pushes"));
              spBtn.addEventListener("click",function(e){
                ...
                if (confirm(msg)) {
                  let button = this;
                  button.textContent = tr('Stopping pushes..');
                  const pushIds = [];
                  ...
                  for (const i in push_list) {
                    if ((push.target == push_list[i][2]) && ((push.stream[0] == "#")  || (push.stream == push_list[i][1]))) {
                      ...
                      if (row) {
                        ...
                        td.appendChild(el("span",{class:"red"},tr("Stopping..")));
                        row.innerHTML = "";
                        row.appendChild(td);
                      }
                    }
                  ...
                  apiClient.send(function(){
                    button.textContent = tr('Stop pushes');
                  },{
                    push_stop: pushIds
                  });
                }
                ...
                latency: function(v){
                  ...
                  if (unitEl) {
                    ...
                    infoSpan.addEventListener("mouseenter",function(e){
                      const tip = el("div");
                      let h = el("h3");
                      h.innerHTML = tr("Latency: ")+out.innerHTML;
                      tip.appendChild(h);
                      tip.appendChild(el("p",null,tr("This the difference between the last sent timestamp and the theoretically highest possible playback position. This is usually mostly jitter buffers.")));
          ...
          add: {
			...
			return false;
                ...
                actions.push(["Remove",function(){
                  if (confirm(tr("Are you sure you want to ")+this.textContent.toLowerCase()+" this push?\n"+push.stream+" to "+push.target)) {
                    this.innerHTML = "";
                    this.appendChild(el("span",{class:"red"},tr("Removing..")));
                    let me = this;
                    apiClient.send(function(d){
                      me.textContent = tr("Done.");
                      $table.update(d.auto_push);
            ...
            create: function(id){
              ...
              $tr.addEventListener("contextmenu",function(e){
                ...
                actions.push(["Stop",function(){
                  if (confirm(tr("Are you sure you want to ")+this.textContent.toLowerCase()+" this push?\n"+push.stream+" to "+push.target)) {
                    this.innerHTML = "";
                    this.appendChild(el("span",{class:"red"},tr("Stopping..")));
                    apiClient.send(function(d){
                      //done
                    },{'push_stop':[push.id]});
                  }
    ...
    if (options.show_auto) {
      ...
      autoSection.appendChild(el("h3",null,tr("Automatic pushes")));
      const autoBtns = el("div",{class:"buttons"});
      const addAutoBtn = el("button",{"data-icon":"plus"},tr("Add an automatic push"));
    ...
    activeSection.appendChild(el("h3",null,tr("Active pushes")));
    const activeBtns = el("div",{class:"buttons"});
    const startBtn = el("button",{"data-icon":"plus"},tr("Start a push"));
    startBtn.addEventListener("click",function(){
      navto('Push Config', '');
    });
    activeBtns.appendChild(startBtn);
    if (options.stop_pushes) {
      const stopAllBtn = el("button",{"data-icon":"stop"},tr("Stop all pushes listed below"));
      stopAllBtn.addEventListener("click",function(){
        ...
        if (pushIds.length) {
          if (confirm(tr("Are you sure you want to stop ")+pushIds.length+" push"+(pushIds.length > 1 ? "es?" : "?"))) {
            rows.forEach(function(row){
              const td = el("td");
              td.setAttribute("colspan","99");
              td.appendChild(el("span",{class:"red"},tr("Stopping..")));
  ...
  outerSection.appendChild(el("h3",null,tr("Pushes and recordings")));
