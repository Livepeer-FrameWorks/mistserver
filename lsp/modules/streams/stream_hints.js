...
updateLiveStreamHint: function(streamname,source,$cont,input,streamkeys) {
    ...
    switch (kind) {
      case "RTMP": {
        ...
        for (const port of ports["RTMP"]) {
          ...
          subDiv.children[0].children[0].textContent = tr("Full url:");
          ...
          for (const key of streamkeys) {
            ...
            if (theurl != newurl) {
              ...
              labelSpan1.textContent = n ? tr("Or URL:") : tr("URL:");
              ...
              labelSpan2.textContent = tr("with key:");
  ...
  if (rawmode) {
    ...
    const title = el('h4', {class: 'livestreamhint-title'}, tr('Configure your source to push to'));
    head.appendChild(title);

    const body = el('div', {class: 'livestreamhint-body'});
    if (streamkeys.length == 0) {
      const noEndpointDiv = el('div', {class: 'livestreamhint-empty'});
      noEndpointDiv.textContent = tr("There is no valid push endpoint with your current settings.");
