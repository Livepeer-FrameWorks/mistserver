...
function streamsPage(tab, other, prev, $main, $pageHeader) {
  ...
  function toggleWildGroup(groupKey) {
    ...
    if (parentEl) {
      parentEl.classList.toggle('wild-expanded', nowExpanded);
      var toggle = parentEl.querySelector('.wildcard-count-badge');
      if (toggle) toggle.setAttribute('aria-expanded', nowExpanded ? 'true' : 'false');
    }
    if ($streams.show_page) $streams.show_page();
  }

  function buildWildcardToggle(groupKey) {
    const toggle = el('button', {
      class: 'wildcard-count-badge',
      type: 'button',
      'aria-expanded': isWildExpanded(groupKey) ? 'true' : 'false',
      'aria-label': tr('Show active wildcard instances')
    });
    toggle.hidden = true;
    toggle.addEventListener('click', function(e) {
      e.stopPropagation();
      e.preventDefault();
      toggleWildGroup(groupKey);
    });
    return toggle;
  }
  ...
  const manageKeysBtn = el('button', {'data-icon': 'key'}, tr('Stream keys'));
  manageKeysBtn.addEventListener('click', function(){ navto("Stream keys"); });

  const createStreamBtn = el('button', {'data-icon': 'plus', class: 'stream-primary-action'}, tr('New stream'));
  ...
  emptyState.appendChild(el('h3', null, tr('No streams configured')));
  emptyState.appendChild(el('p', null, tr('Create your first stream to start delivering media.')));
  const emptyCreateBtn = el('button', {'data-icon': 'plus'}, tr('Create a stream'));
  ...
  emptyHint.appendChild(el('span', null, tr('You can also use New stream or Stream keys at the top.')));
  ...
  if (other == "thumbnails") {
      ...
      add: {
        create: function(id){
          ...
          if (id.indexOf("+") >= 0) {
            ...
            streamEl.elements._wcBadge = buildWildcardToggle(wildGroupKey);
            ...
            if (current_streams[id].isfolderstream) {
              if (!current_streams[id].filesfound) {
                ...
                streamHints.findFolderSubstreams(current_streams[id],function(result){
                  ...
                  streamEl.setAttribute("title",tr("This is a folder stream: it points to a folder with media files inside."));
                  var count = Object.keys(result).length;
                  if (count >= 50) {
                    streamEl.setAttribute("title",tr("Found ") + count + " files in this folder.");
                  }
                });
              }
              else {
          ...
          const actionsBtn = el('button', null, tr('Actions'));
        ...
        update: function(data){
          ...
          if (data.source !== this.raw_source) {
            if (data.source === null) {
              ...
              this.setAttribute("title",tr("This stream has no configuration and will disappear once it goes offline."));
            }
            else {
              ...
              if (inputs_f) {
                if (mistHelpers.inputMatch(inputs_f.source_match,data.source)) {
                  ...
                  this.setAttribute("title",tr("This is a folder stream: it points to a folder with media files inside. Click to request its sub streams."));
                }
                else {
          ...
          if (this.elements._wcBadge && data.name.indexOf('+') < 0) {
            ...
            if (wcCount > 0) {
              ...
              this.elements._wcBadge.title = trn('%s active wildcard instance inheriting this configuration', '%s active wildcard instances inheriting this configuration', wcCount, wcCount);
              this.elements._wcBadge.setAttribute('aria-label', this.elements._wcBadge.title);
    ...
    reverseLabel.appendChild(el('span', null, tr('Reverse')));
    ...
    table.layout = {
      name: function(d,id){
        if (id != this.raw) {
          ...
          const actionsBtn = el("button", {class: "stream-row-actions", type: "button", title: "Actions"}, tr("Actions"));
          actionsBtn.addEventListener("click", function(e){
            const rect = this.getBoundingClientRect();
            context_menu.fill(id,{pageX:rect.left + window.scrollX, pageY:rect.top + window.scrollY});
            e.stopPropagation();
          ...
          if (id.indexOf("+") >= 0) {
            ...
            a.title = tr("This is a wildcard stream: its config is inherited from its parent: '")+parentstream+"'.";
            a.innerHTML = '';
            ...
            this._wcBadge = buildWildcardToggle(wildGroupKey);
          }
          rowTitle.appendChild(a);
          if (this._wcBadge) rowTitle.appendChild(this._wcBadge);
        ...
        if (d.source !== this.raw_src) {
          if (d.source === null) {
            ...
            if (linkEl) linkEl.title = tr("This stream has no configuration and will disappear once it goes offline.");
          }
            ...
            if (linkEl2) {
              ...
              linkEl2.title = parentName ? tr("This is a wildcard stream: its config is inherited from its parent: '%s'.", parentName) : "";
            }
          }
          ...
          if (mist.data.capabilities) {
            ...
            if (inputs_f) {
              if (mistHelpers.inputMatch(inputs_f.source_match,d.source)) {
                this.setAttribute("data-isfolderstream","yes");
                this.title = tr("This is a folder stream: it points to a folder with media files inside. Click the '\u2795' to request its sub streams.");
        ...
        if (this._wcBadge && d.name.indexOf('+') < 0) {
          ...
          if (wcCount > 0) {
            ...
            this._wcBadge.title = trn('%s active wildcard instance inheriting this configuration', '%s active wildcard instances inheriting this configuration', wcCount, wcCount);
            this._wcBadge.setAttribute('aria-label', this._wcBadge.title);
      ...
      state: function(d){
        let state = el("div", {'data-streamstatus': 0}, tr("Inactive"));
        if ("stats" in d) {
    ...
    const headerRow = el("tr", {'data-sortby': 'name'});
    ...
    $streams = dynamicUI.dynamic({
      create: function(){
        ...
        },{controls:headerRow,sortsave:"sortstreams"});
  ...
  context_menu.fill = function(streamname,e){
    ...
    editLabel.innerHTML = streamname.indexOf("+") < 0 ? tr("Edit stream") : tr("Edit <b>%s</b>", streamname.split("+")[0]);
    const gototabs = [
    ...
    let actions = [
      ["Delete stream",function(){
        if (confirm(tr('Are you sure you want to delete the stream "')+streamname+'"?')) {
      ...
      ["Stop sessions",function(){
        if (confirm(tr("Are you sure you want to disconnect all sessions (viewers, pushes and possibly the input) for the stream '")+streamname+"'?")) {
          apiClient.send(function(){
            //done
    ...
    if (settings.source === null) {
      ...
      createLabel.innerHTML = tr("Create <b>")+streamname.split("+")[0]+"</b>";
...
registerTab('Preview', function(tab, other, prev, $main, $pageHeader) {
  ...
  let status = el("section", {class: "activestream stream-slab stream-slab--status preview-status-shell"}, tr("Loading.."));
