 ...
 */
export function streamSave(saveas, other, targetTab, afterSave, relatedStreams) {
  ...
  if (relatedStreams) {
    for (const relatedName in relatedStreams) {
      if (relatedName === saveas.name) continue;
      send.addstream[relatedName] = relatedStreams[relatedName];
      mist.data.streams[relatedName] = relatedStreams[relatedName];
    }
  }
