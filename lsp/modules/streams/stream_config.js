...
import { applyComposerConfig, extractComposerConfig, findComposerOutputCodec, prepareComposerPipeline, removeComposerProcesses } from './composer_config.js';
...
function buildAssemblyPreview(callbacks, ctx) {
  ...
  for (let i = 0; i < completed.length; i++) {
    ...
    const chipLabel = el('span', {class: 'assembly-chip-label'}, tr(step.label));
    let chipValue = el('span', {class: 'assembly-chip-value'});

    if (step.id === 'scenario') {
      const scenario = ctx._scenarioChosen ? ss.SCENARIOS[ctx._scenarioChosen] : null;
      chipValue.textContent = scenario ? tr(scenario.title) : '';
    } else if (step.id === 'source') {
      chipValue.textContent = getSourceSummary(ctx) || tr('(not set)');
    } else if (step.id === 'settings') {
      chipValue.textContent = step.summary ? tr(step.summary) : tr('Defaults');
    } else if (step.id === 'processes') {
      const n = (ctx.processes || []).length;
      chipValue.textContent = n ? trn('%s process', '%s processes', n, n) : tr('None');
    } else if (step.id === 'compose') {
      chipValue.textContent = step.summary ? tr(step.summary) : '';
...
function buildSourceStep($container, ctx, callbacks) {
  ...
  if (!scenario) {
    $container.appendChild(el('p', {class: 'wizard-intro'}, tr('Please select a stream type first.')));
    callbacks.markValid(false);
    return;
  }
  ...
  function updateWildcardCallout() {
    ...
    _wcCallout.innerHTML = tr(
      '<strong>Wildcard template</strong> - Push to <code>%1$s+identifier</code> (e.g.\u00a0<code>%1$s+alice</code>, <code>%1$s+cam2</code>) to create independent child streams that inherit this configuration. Each child has its own buffer and viewer sessions.',
      n
    );
  }
  ...
  if (scenario && scenario.isCompose) {
    ...
    if (inputCap && inputCap.enumerate) {
      const enumNote = el('em', null, tr('Discovering sources...'));
    ...
    srcPrefix.appendChild(el('span', {class: 'push-platform-label'}, tr(scenario.title)));
...
function buildSettingsForm($container, input, ctx, callbacks) {
  ...
  const intro = el('p', {class: 'wizard-intro'}, tr("Configure input-specific options. Common settings are shown first; expand 'Advanced' for tuning options."));
  $container.appendChild(intro);

  const build = formEngine.convertBuildOptions(input, ctx);
  var filtered = build.filter(function(item) {
    if (item && item.type === 'help' && item.help && item.help === input.desc) return false;
    if (ctx._scenarioChosen === 'compose' && item && item.pointer &&
        (item.pointer.index === 'sources' || item.pointer.index === 'resolution')) return false;
    return true;
  });
  const classified = classifyFields(filtered);

  if (!classified.basic.length && !classified.advanced.length) {
    const noSettings = el('p', {class: 'wizard-intro'}, tr('No additional settings for this input type.'));
...
function buildSettingsStep($container, ctx, callbacks) {
  ...
  if (!type) {
    const noInput = el('p', {class: 'wizard-intro'}, tr('No input-specific settings available. Set a source first.'));
  ...
  if (input.dynamic_capa && ctx.source) {
    ...
    let loading = el('p', {class: 'wizard-intro'}, tr('Loading device options\u2026'));
    ...
    _dynamicCapaTimer = setTimeout(function() {
      ...
      apiClient.send(function(d) {
        if (d && d.capabilities) {
          ...
          loading.textContent = tr('Could not load device options. Using defaults.');
...
function buildProcessesStep($container, ctx, callbacks) {
  ...
  for (const k in caps) {
    if (caps[k] && caps[k].type !== 'standalone') { hasProcesses = true; break; }
  }

  if (!hasProcesses) {
    const noProc = el('p', {class: 'wizard-intro'}, tr('No stream processes available on this server.'));
  ...
  function showProcessPicker(addCallback) {
    ...
    for (const procId in caps) {
      ...
      if (!cap || cap.type === 'standalone') continue;
      allCards.push({
        id: procId,
  ...
  const editor = inlineEditor({
    ...
    editItem: function(item, index, $area, editCallbacks) {
      ...
      const cancelBtn = el('button', {class: 'cancel'}, tr('Cancel'));
      cancelBtn.addEventListener('click', function() {
        editCallbacks.cancel();
      });
      const saveBtn = el('button', {class: 'save'}, tr('Save process'));
...
function buildAccessStep($container, ctx, callbacks) {
  ...
  if (!isPushSource(ctx.source)) {
    const naMsg = el('p', {class: 'wizard-intro'}, tr('This step configures stream-name/key rules for push:// ingest sources only. This stream uses a direct source, so those key rules do not apply. You can still enforce access with Automations (for example STREAM_PUSH/CONN_OPEN/USER_NEW), and for SRT-style ingest use protocol-level settings such as passphrase and network ACLs.'));
    $container.appendChild(naMsg);
    callbacks.setStepSummary('Use automations/protocol auth');
    return;
  }

  const introEl = el('p', {class: 'wizard-intro'}, tr('Control who can push to this stream using push:// name/key matching. You can also manage stream keys from the Stream Keys page, or use automations (triggers) for dynamic access control.'));
  $container.appendChild(introEl);
  ...
  const generateBtn = el('button', null, tr('Generate'));
...
function buildComposeStep($container, ctx, callbacks) {
  ...
  if (!ctx._composeData) {
    var hasMistProcAV = !!(mist.data.capabilities.processes && mist.data.capabilities.processes.AV);
    ctx._composeData = extractComposerConfig(ctx);
    ctx.processes = removeComposerProcesses(ctx.processes);
    ctx._composeData._prepareInputs = hasMistProcAV && !ctx._editing;
    ctx._composeData._outputCodec = hasMistProcAV ? (findComposerOutputCodec(ctx.processes) || (ctx._editing ? 'raw' : 'H264')) : 'raw';
    ctx._composeData._outputCodecChanged = false;
    ctx._composeData._rawSources = [];
  }
  ...
  var pipelineIntro = el('div', {class: 'composer-pipeline-callout'});
  pipelineIntro.appendChild(el('strong', null, tr('Composer works with raw media only')));
  pipelineIntro.appendChild(el('p', null, tr('Most streams are encoded. MistProcAV must decode each video source to UYVY (and the selected audio source to PCM) before MistProcComposer can use it. The composed UYVY output normally needs a second MistProcAV step for delivery. SDI/raw inputs and PNG images do not need a video decoder.')));
  $container.appendChild(pipelineIntro);

  var introEl = el('p', {class: 'wizard-intro'}, tr('Design your multiview layout. Add input streams, choose a grid mode, and arrange sources in the preview below.'));
  $container.appendChild(introEl);

  var pipeline = el('section', {class: 'composer-pipeline'});
  var pipelineList = el('div', {class: 'composer-pipeline-list'});

  function flattenCodecs(value, output) {
    if (Array.isArray(value)) {
      for (var ci = 0; ci < value.length; ci++) flattenCodecs(value[ci], output);
    } else if (typeof value === 'string') {
      output.push(value.toUpperCase());
    }
  }

  function sourceHasRawInput(sourceName) {
    var config = mist.data.streams && mist.data.streams[sourceName];
    if (!config && sourceName.indexOf('+') >= 0 && mist.data.streams) config = mist.data.streams[sourceName.split('+')[0]];
    if (!config || !config.source) return false;
    var inputs = mist.data.capabilities.inputs || {};
    for (var inputId in inputs) {
      var inputCap = inputs[inputId];
      if (!inputCap || inputCap.source_match === undefined || !mistHelpers.inputMatch(inputCap.source_match, config.source)) continue;
      var codecs = [];
      flattenCodecs(inputCap.codecs || [], codecs);
      return codecs.indexOf('UYVY') >= 0;
    }
    return false;
  }

  function sourceHasDecoder(sourceName) {
    var config = mist.data.streams && (mist.data.streams[sourceName] || mist.data.streams[sourceName.split('+')[0]]);
    var processes = config && config.processes;
    if (!processes) return false;
    for (var processId in processes) {
      var process = processes[processId];
      if (process && process.process === 'AV' && process['x-LSP-kind'] === 'video' && String(process.codec).toUpperCase() === 'UYVY') return true;
    }
    return false;
  }
  ...
  function renderPipeline() {
    ...
    ctx._composeData._rawSources = [];
    var avAvailable = !!(mist.data.capabilities.processes && mist.data.capabilities.processes.AV);
    if (!avAvailable) {
      pipelineList.appendChild(el('div', {class: 'composer-pipeline-item error'}, tr('MistProcAV is unavailable. Only already-raw sources can be composed, and the result cannot be encoded here.')));
    }
    for (var si = 0; si < ctx._composeData.sources.length; si++) {
      var sourceName = String(ctx._composeData.sources[si].stream || '');
      if (!sourceName) continue;
      var status = '';
      var state = 'warning';
      if (/\.png(?:$|\?)/i.test(sourceName)) {
        status = 'PNG image - no decoder needed';
        state = 'ready';
        ctx._composeData._rawSources.push(sourceName);
      } else if (sourceHasRawInput(sourceName)) {
        status = 'Raw UYVY input - no decoder needed';
        state = 'ready';
        ctx._composeData._rawSources.push(sourceName);
      } else if (sourceHasDecoder(sourceName)) {
        status = 'UYVY decoder already configured';
        state = 'ready';
      } else if (mist.data.streams && (mist.data.streams[sourceName] || mist.data.streams[sourceName.split('+')[0]])) {
        status = ctx._composeData._prepareInputs && avAvailable ? 'MistProcAV UYVY decoder will be added on save' : 'UYVY decoder required on source stream';
      } else {
        status = 'Source is not configured here; verify it provides UYVY';
      }
      var row = el('div', {class: 'composer-pipeline-item ' + state});
      row.appendChild(el('code', null, sourceName));
      row.appendChild(el('span', null, tr(status)));
      pipelineList.appendChild(row);
    }
  }

  pipeline.appendChild(el('h4', null, tr('Media pipeline')));
  var prepareLabel = el('label', {class: 'composer-pipeline-option'});
  var prepareInputs = el('input', {type: 'checkbox'});
  prepareInputs.checked = !!ctx._composeData._prepareInputs;
  prepareInputs.disabled = !(mist.data.capabilities.processes && mist.data.capabilities.processes.AV);
  prepareInputs.addEventListener('change', function() {
    ctx._composeData._prepareInputs = this.checked;
    renderPipeline();
  });
  prepareLabel.appendChild(prepareInputs);
  prepareLabel.appendChild(document.createTextNode(tr(' Add missing MistProcAV UYVY decoders to configured source streams when saving (recommended)')));
  pipeline.appendChild(prepareLabel);

  var outputLabel = el('label', {class: 'composer-pipeline-option'}, tr('Delivery output '));
  var outputCodec = el('select');
  [['H264', 'H.264 via MistProcAV (recommended)'], ['AV1', 'AV1 via MistProcAV'], ['JPEG', 'MJPEG via MistProcAV'], ['raw', 'Raw UYVY (no encoder)']].forEach(function(option) {
    outputCodec.appendChild(el('option', {value: option[0]}, tr(option[1])));
  });
  outputCodec.value = ctx._composeData._outputCodec;
  outputCodec.disabled = !(mist.data.capabilities.processes && mist.data.capabilities.processes.AV);
  outputCodec.addEventListener('change', function() {
    ctx._composeData._outputCodec = this.value;
    ctx._composeData._outputCodecChanged = true;
    updateSummary();
  });
  outputLabel.appendChild(outputCodec);
  pipeline.appendChild(outputLabel);
  pipeline.appendChild(pipelineList);
  pipeline.appendChild(el('p', {class: 'composer-pipeline-note'}, tr('Audio is optional. Select a stream under Settings → Copy audio; it must provide PCM. With automatic preparation enabled, a PCM decoder is added to that configured source as well. For bitrate, presets, or multiple output renditions, configure MistProcAV in the Processes step.')));

  var designer = buildDesignerInline(ctx._composeData, composerCap || {}, {
    onChange: function() { renderPipeline(); updateSummary(); }
  });
  ...
  function updateSummary() {
    ...
    callbacks.markValid(n > 0);
  }
  updateSummary();
  renderPipeline();

  return {
    validate: function() { return ctx._composeData.sources.length > 0; },
...
function buildReviewStep($container, ctx, callbacks) {
  ...
  const introEl = el('p', {class: 'wizard-intro'}, tr("Review your configuration before saving. Click 'Edit' on any section to make changes."));
  ...
  if (scenario) {
    ...
    typeLine.appendChild(document.createTextNode(tr(scenario.title)));
    review.appendChild(typeLine);
  } else {
    review.appendChild(el('em', {class: 'text-muted'}, tr('Unknown')));
  }
  ...
  const tbl = el('table');
  const tr1 = el('tr');
  tr1.appendChild(el('td', null, tr('Name')));
  tr1.appendChild(el('td', null, ctx.name || tr('(not set)')));
  tbl.appendChild(tr1);

  const tr2 = el('tr');
  tr2.appendChild(el('td', null, tr('Source')));
  tr2.appendChild(el('td', null, ctx.source || tr('(not set)')));
  tbl.appendChild(tr2);

  if (ctx.always_on) {
    const tr3 = el('tr');
    tr3.appendChild(el('td', null, tr('Always on')));
    tr3.appendChild(el('td', null, tr('Yes')));
    tbl.appendChild(tr3);
  }
  ...
  if (ctx._composeData) {
    ...
    var trRes = el('tr');
    trRes.appendChild(el('td', null, tr('Resolution')));
    ...
    var trLayout = el('tr');
    trLayout.appendChild(el('td', null, tr('Layout')));
    trLayout.appendChild(el('td', null, layoutNames[compData.layout] || compData.layout));
    compTbl.appendChild(trLayout);

    var trSrc = el('tr');
    trSrc.appendChild(el('td', null, tr('Sources')));
    var srcNames = (compData.sources || []).map(function(s) { return s.stream || '(unnamed)'; });
    trSrc.appendChild(el('td', null, srcNames.length ? srcNames.join(', ') : tr('None')));
    compTbl.appendChild(trSrc);

    var trOutput = el('tr');
    trOutput.appendChild(el('td', null, tr('Delivery output')));
    trOutput.appendChild(el('td', null, compData._outputCodec === 'raw' ? tr('Raw UYVY') : tr(compData._outputCodec + ' via MistProcAV')));
    compTbl.appendChild(trOutput);
  ...
  if (ctx.processes && ctx.processes.length) {
    ...
    const noneEl = el('em', {class: 'text-muted'}, tr('None'));
  ...
  if (isPushSource(ctx.source)) {
    ...
    presetLine.textContent = tr('Preset: ') + preset.replace(/_/g, ' ');
    review.appendChild(presetLine);
    if (ctx.streamkeys && ctx.streamkeys.length) {
      let keysLine = el('div', {class: 'review-line review-subline'});
      keysLine.textContent = trn('%s stream key configured', '%s stream keys configured', ctx.streamkeys.length, ctx.streamkeys.length);
      review.appendChild(keysLine);
    } else if (!ctx.streamkey_only) {
      let warnLine = el('div', {class: 'review-warning'});
      warnLine.textContent = tr('No stream keys configured - anyone who knows the stream name can push to it.');
...
function streamConfigTab(tab, other, prev, $main, $pageHeader) {
  ...
  apiClient.send(function() {
    ...
    if (editing && stream && stream.header) {
      ...
      if (h2) h2.textContent = tr('New Stream');
    }
    ...
    const uriLabel = el('label', null, tr('Source:'));
...
function performSave(saveas, other, targetTab) {
  ...
  if (cleanSaveas._composeData) {
    ...
    applyComposerConfig(cleanSaveas, compData);
    if (mist.data.capabilities.processes && mist.data.capabilities.processes.AV) {
      relatedStreams = prepareComposerPipeline(cleanSaveas, compData, mist.data.streams || {}).relatedStreams;
    }
  } else {
    delete cleanSaveas._composeData;
  }

  streamSave(cleanSaveas, other || cleanSaveas.name, targetTab, null, relatedStreams);
}
