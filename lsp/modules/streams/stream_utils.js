...
function createStreamSlab(options) {
  ...
  const title = el(options.titleTag, {class: "stream-slab-title"}, tr(options.title));
  if (options.icon) {
...
export const stream = {
        ...
        onClick: function(tabNameCopy) {
          return function() {
            ...
            if (cont && (currenttab == "Edit") && (cont.getAttribute("data-changed") == "yes")) {
              if (!confirm(tr("Your settings have not been saved. Are you sure you want to navigate away?"))) {
                return;
              }
            }
  ...
  bigbuttons: function(streamname,currenttab){
    ...
    const settingsBtn = el('button', {class: 'settings'}, tr('Settings'));
    ...
    if (currenttab != "Status") {
      const statusBtn = el('button', {class: 'status'}, tr('Status'));
    ...
    if ((currenttab != "Preview") && !isFolderStream) {
      const previewBtn = el('button', {class: 'preview'}, tr('Preview'));
    ...
    if ((currenttab != "Embed") && !isFolderStream) {
      const embedBtn = el('button', {class: 'embed'}, tr('Embed'));
    ...
    const returnBtn = el('button', {class: 'cancel return'}, tr('Return'));
  ...
  findMist: function(on_success_callback,url_rewriter,fullsearch){
    ...
    function on_fail(urls) {
      if (urls.length == 1 && sockets._adminToken && urls[0].indexOf('/http/'+sockets._adminToken+'/') >= 0) {
        sockets.admin_proxy = null;
        return useDiscoveredHTTP();
      }
      ...
      if (urls.length) {
        ...
        helpBox.appendChild(el("h2", null, tr("Why am I seeing this?")));
        const p1 = el("p");
        p1.innerHTML = tr("If you think one of the urls above should have worked, try clicking the link. <br>If something went wrong, your browser may give you more information about what it is. <br>If you see something like this:");
        helpBox.appendChild(p1);
        const exampleBox = el("div", {style: {marginLeft: "2em"}});
        exampleBox.innerHTML = tr("<h1>Unsupported Media Type</h1>The server isn't quite sure what you wanted to receive from it.");
        helpBox.appendChild(exampleBox);
        const p2 = el("p");
        p2.innerHTML = tr(
          "then %s <i>can</i> be reached there but it couldn't be called from this page, possibly because of mixed content (if you're on an https website, your browser will refuse to load http content: create an https output) or CORS issues (if the HTTP output is accessible on a domain other than '%s' and that is not configured to allow cross-domain requests: check your proxy configuration).",
          APP_NAME,
          location.origin
        );
        ...
        attempts = el("span");
        attempts.textContent = tr("I've not tried anything yet. I'm cluessless. Please help.");
      }
      ...
      cont.innerHTML = '';
      const p = el("p");
      p.textContent = tr("Something went wrong: I could not locate %s's HTTP%s output URL. Where can it be reached?", APP_NAME, S);
      cont.appendChild(p);
      const descDiv = el("div", {class: "description"});
      descDiv.appendChild(el("p", null, tr("I've attempted:")));
    ...
    function on_success(url) {
      if (sockets._adminToken && url.indexOf('/http/'+sockets._adminToken+'/') >= 0) {
        sockets.admin_proxy = url;
      }
      else if (url != sockets.http_host) {
    ...
    if (!fullsearch) {
      sockets.ensureAdminHttpToken().then(function(token){
        cont.hidden = true;
        on_success(sockets.getApiUrl({pathname:'/http/'+token+'/'}));
      }).catch(useDiscoveredHTTP);
    }
    else {
      useDiscoveredHTTP();
    }
  ...
  status: function(streamname,options){
    ...
    if (statusDiv) { statusDiv.textContent = tr("Inactive"); statusDiv.hidden = true; }
    ...
    if (options.tags && (!options.tags.readonly)) {
      ...
      showIfInactive.textContent = tr("Transient tags can be added here once the stream is active.");
      ...
      tagBtn.textContent = tr("Add transient tag");
      tagBtn.addEventListener('click', function(){
        ...
        me.textContent = tr("Adding..");
        apiClient.send(function(d){
          me.textContent = tr("Add tag");
          inp.value = "";
        },save);
      });
  ...
  actions: function(currenttab,streamname){
    ...
    stopBtn.textContent = tr("Stop all sessions");
    stopBtn.addEventListener('click', function(){
      if (confirm(tr("Are you sure you want to disconnect all sessions (viewers, pushes and possibly the input) from this stream?"))) {
    ...
    invalBtn.textContent = tr("Invalidate sessions");
    invalBtn.addEventListener('click', function(){
      if (confirm(tr("Are you sure you want to invalidate all sessions for the stream '")+streamname+"'?\nThis will re-trigger the USER_NEW trigger.")) {
    ...
    nukeBtn.textContent = tr("Nuke stream");
    nukeBtn.addEventListener('click', function(){
      if (confirm(tr("Are you sure you want to completely shut down the stream '")+streamname+"'?\nAll viewers will be disconnected.")) {
        apiClient.send(function(){
    ...
    if (streamname[0] == "/") {
      ...
      function subscribeToThumbnail() {
        ...
        jpg.addEventListener('mouseenter', function(){
          if (image && stream) this.setAttribute("src",stream);
        });
        jpg.addEventListener('mouseleave', function(){
          if (image && stream) this.setAttribute("src",image);
        });
      ...
      if (mist.data.capabilities) subscribeToThumbnail();
      else apiClient.send(subscribeToThumbnail,{capabilities:true});
    }
  ...
  metadata: function(streamname,options){
          ...
          create: function(){
            ...
            if (options.tracktiming) {
              main.timing = dynamicUI.dynamic({
                ...
                add: {
                  ...
                  update: function(values){
                    ...
                    if (this.label.raw != values.idx+values.type+values.codec) {
                      this.label.textContent = tr("Track ")+values.idx+" ("+values.codec+")";
                      this.label.raw = values.idx+values.type+values.codec;
                      this.style.order = values.idx;
                    }
                    if (this.jitter.raw != values.jitter) {
                      this.jitter.setAttribute("title",tr("Jitter: ")+values.jitter+"ms");
                ...
                update: function(values){
                  const box = this.box.querySelector('.box');
                  if (!Object.keys(values).length) {
                    box.style.display = 'none';
                    this.box.left.style.display = 'none';
                    this.box.right.style.display = 'none';
                    this.box.label.textContent = 'No tracks';
                    return;
                  }
                  box.style.display = '';
                  this.box.left.style.display = '';
                  this.box.right.style.display = '';
                  const from = main.timing.bounds.firstms.max;
                  const to = main.timing.bounds.lastms.min;
              ...
              inclLabel.appendChild(el("span", null, tr("Include metadata in graph:")));
              inclLabel.appendChild(main.includemeta);
              main.appendChild(inclLabel);
              const timingLabel = el("label");
              timingLabel.appendChild(el("span", null, tr("Track timing:")));
            ...
            if (options.tracktable) {
                  ...
                  const tt = dynamicUI.dynamic({
                    create: function(id){
                      ...
                      const headers = {
                        ...
                        subtitle: {
                          vheader: 'Subtitles',
                          labels: ['Codec','Duration','Jitter','Avg bitrate','Peak bitrate','Language','Player track index','Track id','Process id','Created from','Masked']
                        },
                        meta: {
                          vheader: 'Metadata',
                      ...
                      update: function(track){
                        function getValues(track) {
                          ...
                          function formatTrackMask(mask){
                            const requesters = {1:'viewers', 2:'push outputs', 4:'processes'};
                            const masked = [];
                            for (const bit in requesters) {
                              if ((Number(mask) & Number(bit)) == 0) masked.push(requesters[bit]);
                            }
                            if (!masked.length) return 'Not hidden';
                            if (masked.length == Object.keys(requesters).length) return 'Hidden for everything';
                            return '\uD83D\uDD12 Hidden for '+masked.join(' and ');
                          }
                          function formatMinMax(min, max, formatter){
                            if (typeof min == 'undefined' || typeof max == 'undefined') return 'N/A';
                            const formatValue = formatter || function(value){ return value; };
                          ...
                          function formatIssues(issues){
                            ...
                            const list = el('ul');
                            issues.forEach(function(issue){ list.appendChild(el('li', null, issue)); });
                            return list;
                          }
                          function codecName(track){
                            return track.codec+(track.codecstring ? ' ('+track.codecstring+')' : '');
                          }
                          const type = track.codec == "subtitle" ? "subtitle" : track.type;
                          switch (type) {
                            case 'audio':
                            ...
                            case 'video':
                              return {
                                ...
                                body: [
                                  ...
                                  (track.fpks == 0 ? "variable" : format.addUnit(format.number(Math.round(track.fpks/10)/100,{round:false}),'fps')),
                                  ('language' in track ? track.language : 'unknown'),
                                  track.nth,
                                  track.trackid,
                                  track.pid == 0 ? 'none' : track.pid,
                                  ...
                                  formatMinMax(track.keys && track.keys.ms_min, track.keys && track.keys.ms_max, function(value){
                                    return format.addUnit(format.number(value),'ms');
                                  }),
                                  formatMinMax(track.keys && track.keys.frame_ms_min, track.keys && track.keys.frame_ms_max, function(value){
                                    return format.addUnit(format.number(value),'ms');
                                  }),
                                  formatMinMax(track.keys && track.keys.frames_min, track.keys && track.keys.frames_max, format.number),
                                  formatIssues(track.issues)
                                ]
                              }
                              break;
                            case 'subtitle':
              ...
              tracksLabel.appendChild(el("span", null, tr("Track metadata:")));
    ...
    function ondata(data) {
      ...
      if (data.error && (!meta.rawdata || meta.rawdata.type == "live")) {
        ...
        if (!mainDyn.parentNode) {
          meta.innerHTML = '';
          const btnDiv = el("div");
          const rawBtn = el("button", null, tr("Open raw json"));
          ...
          const freezeBtn = el("button", {title: "Pauze updating the stream metadata information below"}, tr("Freeze"));
          freezeBtn.addEventListener('click', function(){
            if (this.textContent == "Freeze") {
              this.textContent = tr("Frozen");
              this.style.background = "#bbb";
              mainDyn.freeze = true;
            }
            else {
              this.textContent = tr("Freeze");
    ...
    sockets.ws.stream.subscribe(streamname, function(type, data){
      if (type == 'streammeta') ondata(data);
      else if (type == 'error') ondata({error:data});
    });
  ...
  triggers: function(streamname,tab){
        ...
        for (const i in list) {
          ...
          for (let n = 0; n < trs.length; n++) {
            ...
            switch (n) {
              ...
              case 1: {
                td.textContent = trigger.streams.length ? trigger.streams.join(", ") : tr("All streams");
                break;
              }
              case 2: {
              ...
              case 3: {
                td.textContent = trigger.sync ? tr("Blocking") : tr("Non-blocking");
                break;
              }
              case 4: {
              ...
              case 5: {
                ...
                const editBtn = el("button", null, tr("Edit"));
                ...
                if (trigger.streams.length > 1) {
                  const removeFromBtn = el("button", null, tr("Remove from stream"));
                  ...
                  const removeBtn = el("button", null, tr("Remove"));
                  removeBtn.addEventListener('click', function(){
                    const t = this.closest(".buttons");
                    const type = t.getAttribute("data-type");
                    if (confirm(tr('Are you sure you want to delete this ') + type + ' automation rule?')) {
    ...
    const addBtn = el("button", null, tr("Add automation rule"));
  ...
  pushes: function(streamname){
    ...
    var newPushBtn = el('button', {'data-icon': 'plus'}, tr('New push'));
  ...
  accesslogs: function(streamname){
    ...
    const openRawBtn = el("button", null, tr("Open raw"));
  ...
  playercontrols: function(MistVideoObject,video){
    ...
    controls.appendChild(el("h3", null, tr("helloPlayer")));
    controls.appendChild(el("p", null, tr("Waiting for player..")));

    if (!video) { video = document.querySelector(".dashboard"); }

    if (!document.querySelector("link#devcss")) {
      const cssLink = el("link", {rel: "stylesheet", type: "text/css", href: sockets.getUrl()+"skins/dev.css", id: "devcss"});
    ...
    function init() {
      ...
      const rawlogsBtn = el("button", {class: "mistplayer-log-raw"}, tr("Open raw"));
      ...
      titleRow.appendChild(el("h3", {class: "title"}, tr("Stream Preview")));
      titleRow.appendChild(el("span", {class: "mistplayer-engine"}, tr("Engine: MistPlayer")));
  ...
  embedurls: function(streamname,misthost,urls){
    ...
    baseLabel.appendChild(el('span', {class: 'label'}, tr('Use base URL:')));
    ...
    const applyBtn = el('button', null, tr('Apply'));
    ...
    setTracks.textContent = tr('Loading..');
    const protocolurls = el('div', {style: {wordBreak: "break-all"}});
    protocolurls.textContent = tr('Loading..');
    ...
    const selfBtn = el('button', {type: 'button', class: 'embed-format-btn' + (embedSource === 'self' ? ' active' : '')}, tr('Self-hosted'));
    const cdnBtn = el('button', {type: 'button', class: 'embed-format-btn' + (embedSource === 'cdn' ? ' active' : '')}, tr('CDN'));
    const npmBtn = el('button', {type: 'button', class: 'embed-format-btn' + (embedSource === 'npm' ? ' active' : '')}, tr('npm'));

    const subToggle = el('div', {class: 'embed-format-toggle', style: {marginTop: '0.35em'}});
    const iifeBtn = el('button', {type: 'button', class: 'embed-format-btn' + (embedFormat === 'iife' ? ' active' : '')}, tr('IIFE'));
    const esmBtn = el('button', {type: 'button', class: 'embed-format-btn' + (embedFormat === 'esm' ? ' active' : '')}, tr('ESM'));
    ...
    cdnNote.textContent = tr('Also available via jsdelivr: https://cdn.jsdelivr.net/npm/@optimist-video/mistplayer-core/dist/player.iife.js');
    if (embedSource !== 'cdn') cdnNote.style.display = 'none';

    function updateFormatHelp() {
      if (embedSource === 'cdn') {
        formatHelp.textContent = tr('Load the player from a public CDN. You still need to point host at your MistServer instance.');
      } else if (embedSource === 'npm') {
        formatHelp.textContent = tr('Install via npm for use with bundlers (webpack, vite, esbuild, etc.). The ESM export gives you createPlayer plus full access to all player internals.');
      } else if (embedFormat === 'esm') {
        formatHelp.textContent = tr('ES Module - use with bundlers, modern apps, or <script type="module">. Gives you a clean API with queries, mutations & callbacks.');
      } else {
        formatHelp.textContent = tr('Classic script tag - works everywhere, no build step needed. Loads the player globally and calls mistPlay().');
      }
    }
    updateFormatHelp();
    ...
    playerInfo.innerHTML = tr('The player is built for <b>white-labelling</b> - use built-in themes, bring your own CSS, or replace any element with custom components. <a href="https://www.npmjs.com/package/@optimist-video/mistplayer-core" target="_blank" rel="noopener">npm</a> · <a href="https://docs.mistserver.org/" target="_blank" rel="noopener">Docs</a>');
    ...
    function displaySources(d,overwritebase) {
      ...
      if (overwritebase) {
        ...
        warnDiv.innerHTML = tr('Warning: the provided base URL <a href="%1$s">%1$s</a> could not be reached. These links are my best guess but will probably not work properly.', otherbase);
        build.push(warnDiv);
      }
    ...
    function connect2info(baseurl) {
      ...
      },streamname,baseurl.replace(/^http/,"ws")+ "json_" + encodeURIComponent(streamname) + ".js?inclzero=1&noinput=1",false);
