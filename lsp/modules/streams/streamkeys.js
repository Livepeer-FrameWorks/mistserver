...
export function streamkeys(streamname,onsave,opts){
  ...
  apiClient.send(function(d){ //request current stream keys + active streams
    ...
    const $addSection = formEngine.buildUI([el('h3', null, tr('Add stream key(s)')),streamFieldCfg,streamPickerCfg,{
        label: "Stream key(s)",
        type: "inputlist",
    ...
    let $streamkeysSummary = el('span', {}, tr("0 keys across 0 streams."));
    let $streamkeysEmpty = el('div', {class: 'streamkeys-empty'}, tr("No stream keys found."));
    ...
    const $listSection = formEngine.buildUI([el('h3', null, 'Current stream keys'+(streamname ? " for '"+streamname+"'" : '')),{
        ...
        headerRow.appendChild(el('th', {}, tr("Status")));
        headerRow.appendChild(el('th', {}, tr("Stream")));
        headerRow.appendChild(el('th', {}, tr("Key")));
        headerRow.appendChild(el('th', {}, tr("Type")));
        headerRow.appendChild(el('th', {}, tr("Actions")));
        ...
        function updateSummary(){
          ...
          $streamkeysSummary.textContent = tr('Keys: %s · Streams: %s', allRows.length, streamCount);
        }
        ...
        function formatStream(stream){
          ...
          $cont.addEventListener("contextmenu",function(e){
            ...
            editSpan1.innerHTML = stream.indexOf("+") < 0 ? tr("Edit stream") : tr("Edit <b>%s</b>", stream.split("+")[0]);
            const editSpan2 = el('span');
            editSpan2.innerHTML = stream.indexOf("+") < 0 ? tr("Create stream") : tr("Create <b>%s</b>", stream.split("+")[0]);
        ...
        function deleteKey(stream,key){
          if (!confirm(tr("Are you sure you want to remove the key '")+key+"'?")) {
            return;
          }
          apiClient.send(function(){
        ...
        function deleteStream(stream){
          ...
          if (!confirm(tr("Are you sure you want to remove all ")+keys.length+" keys of the stream '"+stream+"'?")) {
            return;
          }
          apiClient.send(function(){
    ...
    if ($streamkeysRows) {
      ...
      const titleSpan = el('span', {class: 'streamkeys-page-control-title'}, tr("Pagination"));
      ...
      if (selectEl) {
        ...
        if (selectContainer) {
          ...
          if (labelSpan) labelSpan.textContent = tr('Rows per page');
        }
      }
