...
export function wizard(options) {
  ...
  const btnBack = el('button', {'data-icon': 'return'}, tr('Back'));
  const btnNext = el('button', {class: 'save'}, tr('Next'));
  const btnSkip = el('button', null, tr('Skip'));
  const btnCancel = el('button', {class: 'cancel'}, tr('Cancel'));
  ...
  function buildProgressBar() {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const label = el('div', {class: 'wizard-step-label'}, tr(steps[i].label));
  ...
  function updateNav() {
    ...
    btnNext.textContent = isLast ? tr('Save') : tr('Next');
