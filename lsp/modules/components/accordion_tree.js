...
export function accordionTree(options) {
  ...
  function buildSection(sectionOpts, isNested) {
    ...
    const $titleEl = el('span', {class: 'accordion-title'}, sectionOpts.title ? tr(sectionOpts.title) : '');
    ...
    if (sectionOpts.subtitle) {
      $header.appendChild(
        el('span', {class: 'accordion-subtitle'}, tr(sectionOpts.subtitle))
      );
    }

    if (sectionOpts.summary) {
      $header.appendChild(
        el('span', {class: 'accordion-summary'}, tr(sectionOpts.summary))
      );
    }
        ...
        (function(action) {
          const $btn = el('button', {
            onclick: function(e) {
              e.stopPropagation();
              if (action.confirm) {
                if (!window.confirm(tr(action.confirm))) return;
              }
              if (action.onClick) action.onClick(e);
            }
          });
          if (action.icon) $btn.setAttribute('data-icon', action.icon);
          if (action.label) $btn.textContent = tr(action.label);
          if (action.title) $btn.setAttribute('title', tr(action.title));
  ...
  return {
    ...
    updateTitle: function(id, title) {
      const entry = sectionMap[id];
      if (entry) entry.$title.textContent = tr(title);
    },
