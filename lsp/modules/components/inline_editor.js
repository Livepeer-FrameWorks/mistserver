...
export function inlineEditor(options) {
  ...
  const $empty = el('div', {class: 'inline-editor-empty'}, tr(options.emptyMessage || 'No items configured.'));
  const $items = el('div', {class: 'inline-editor-items'});
  const $addBtn = el('button', {
    class: 'inline-editor-add',
    'data-icon': options.addIcon || 'plus'
  }, tr(options.addLabel || 'Add item'));
  ...
  function buildItemElement(index) {
    ...
    if (info.subtitle) {
      ...
      if (info.subtitle instanceof HTMLElement) {
        ...
        $sub.textContent = tr(info.subtitle);
      }
    ...
    (function(idx) {
      $actions.appendChild(
        el('button', {
          ...
          onclick: function(e) {
            e.stopPropagation();
            if (!window.confirm(tr('Remove this item?'))) return;
