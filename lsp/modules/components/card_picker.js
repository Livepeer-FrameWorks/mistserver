...
export function cardPicker(options) {
  ...
  if (options.intro) {
    container.insertBefore(
      el('p', {class: 'wizard-intro'}, tr(options.intro)),
  ...
  for (let g = 0; g < groups.length; g++) {
    ...
    for (let c = 0; c < cards.length; c++) {
      ...
      cardEl.appendChild(
        el('div', {class: 'wizard-card-title'}, tr(card.title))
      );

      if (card.desc) {
        cardEl.appendChild(el('div', {class: 'wizard-card-desc'}, tr(card.desc)));
      }

      if (isPromo && card.promoText) {
        cardEl.appendChild(el('div', {class: 'wizard-card-desc'}, tr(card.promoText)));
        if (card.promoLink) {
          cardEl.appendChild(
            el('a', {
      ...
      cardEl._cardTitle = ((card.title || '') + ' ' + (card.title ? tr(card.title) : '')).toLowerCase();
      cardEl._cardDesc = ((card.desc || '') + ' ' + (card.desc ? tr(card.desc) : '')).toLowerCase();
    ...
    if (collapsibleGroups) {
      groupTitle = el('button', {
        ...
        el('span', {class: 'wizard-group-title-text'}, tr(group.title)),
        el('span', {class: 'wizard-group-title-count'}, groupCards.length.toString())
      ]);
    } else {
      groupTitle = el('div', {class: 'wizard-group-title'}, tr(group.title));
    }
