...
import { calculateComposerPositions } from '../streams/composer_config.js';
...
const LAYOUT_SVGS = {
  ...
  lasagna: '<svg viewBox="0 0 48 36" width="48" height="36"><rect x="3" y="3" width="40" height="28" fill="none" stroke="currentColor" stroke-width="1"/><rect x="5" y="5" width="40" height="28" fill="none" stroke="currentColor" stroke-width="1"/><rect x="7" y="7" width="40" height="28" fill="var(--accentColor)" stroke="currentColor" stroke-width="1" opacity="0.4"/></svg>',
...
function calculatePositions(sources, resolution, layout) {
  return calculateComposerPositions(sources.length, resolution[0] + 'x' + resolution[1], layout);
}

function createScenePreview(resolutionValue, sourcesValue, layoutValue) {
  const section = el('section', {class: 'scene_preview'});
  const ns = 'http://www.w3.org/2000/svg';
  const svg = document.createElementNS(ns, 'svg');
  section.appendChild(svg);

  section.update = function(resolutionText, sources, layout) {
    const resolution = parseResolution(resolutionText);
    const cells = sources || [];
    const positions = calculatePositions(cells, resolution, layout || 'equal');
    svg.setAttribute('viewBox', '0 0 '+resolution[0]+' '+resolution[1]);
    svg.style.fontSize = (resolution[1] / 8) + 'px';
    svg.innerHTML = '';

    for (let i = 0; i < cells.length; i++) {
      const cell = cells[i];
      const position = (layout === 'none' && cell.x !== undefined) ? cell : positions[i];
      ...
      text.setAttribute('x', position.x + position.w / 2);
      text.setAttribute('y', position.y + position.h / 2);
  ...
  section.update(resolutionValue, sourcesValue, layoutValue);
  return section;
}
...
function buildDesignerInline(settings, cap, options) {
  ...
  srcSection.appendChild(el('h4', null, tr('Sources and labels')));
  ...
  function getStreamOptions() {
    ...
    if (mist.data.streams) {
      for (const s in mist.data.streams) opts.push(s);
    }
    if (Array.isArray(mist.data.active_streams)) {
      for (let i = 0; i < mist.data.active_streams.length; i++) opts.push(mist.data.active_streams[i]);
    }
  ...
  const streamListId = 'composer-streams-' + Math.random().toString(36).slice(2);
  const streamDatalist = el('datalist', {id: streamListId});
  const streamOptions = getStreamOptions();
  for (let i = 0; i < streamOptions.length; i++) {
    streamDatalist.appendChild(el('option', {value: streamOptions[i]}));
  }
  srcSection.appendChild(streamDatalist);
  ...
  function renderSourceList() {
    ...
    for (let i = 0; i < settings.sources.length; i++) {
      (function(idx) {
        ...
        const streamInput = el('input', {type: 'text', list: streamListId, placeholder: 'Stream name or .png path'});
        ...
        labelSel.appendChild(el('option', {value: 'name'}, tr('Show stream name')));
        labelSel.appendChild(el('option', {value: 'none'}, tr('No label')));
        labelSel.appendChild(el('option', {value: 'custom'}, tr('Custom label')));
        ...
        removeBtn.title = tr('Remove source');
    ...
    const addBtn = el('button', null, tr('Add source'));
  ...
  resSection.appendChild(el('h4', null, tr('Output resolution')));
  ...
  layoutSection.appendChild(el('h4', null, tr('Grid layout')));

  const layouts = [
    { id: 'equal', label: 'Standard grid' },
    { id: 'focussed', label: 'Focussed' },
    { id: 'lasagna', label: 'Layered' },
    ...
    (function(layout) {
      ...
      btn.addEventListener('click', function() {
        ...
        if (settings.layout === 'none' && layout.id !== 'none') {
          if (!confirm(tr('Switching away from freestyle will reset custom positions. Continue?'))) return;
        }
  ...
  const layoutHelp = el('p', null,
    ...
    tr("The 'layered' layout overlays all sources, which is useful for transparent or intermittently offline layers.") + " " +
    tr("In 'freestyle' mode, click a cell to select it, then drag to move and drag edges to resize.")
  );
  ...
  function updateCellNav() {
    ...
    const navLabel = el('span', null, tr('Select cell: '));
    ...
    for (let i = 0; i < settings.sources.length; i++) {
      (function(idx) {
        ...
        if (s.w / resolution[0] < 0.02 || s.h / resolution[1] < 0.02) {
          ...
          srcEl.title = s.stream || tr('Source %s', idx + 1);
        }
  ...
  previewEl.addEventListener('contextmenu', function(e) {
    ...
    if (idx < settings.sources.length - 1) {
      const fwdBtn = el('button', null, tr('Move forward'));
    ...
    if (idx > 0) {
      const bwdBtn = el('button', null, tr('Move backward'));
...
function openDesigner(settings, cap, onApply) {
  ...
  const applyBtn = el('button', {class: 'save'}, tr('Apply'));
...
export function composerEditor(item, cap, $area, editCallbacks) {
  ...
  function updateSummary() {
    ...
    const layoutNames = { equal: 'Standard grid', focussed: 'Focussed', lasagna: 'Layered', none: 'Freestyle' };
  ...
  const scenePreview = createScenePreview(editCopy.resolution, editCopy.sources, editCopy.layout);
  const designerBtn = el('button', null, tr('Open designer'));
  ...
  designerBtn.addEventListener('click', function() {
    openDesigner(editCopy, cap, function(updated) {
      ...
      updateSummary();
      scenePreview.update(editCopy.resolution, editCopy.sources, editCopy.layout);
    });
  });
  designerBtnWrap.appendChild(scenePreview);
