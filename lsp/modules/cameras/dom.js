...
dom.backButton = function(label, tab, param) {
  const btn = el('button', {'data-icon': 'return'}, tr(label));
...
dom.protocolBar = function(MistVideo, onAudioDetected) {
  ...
  var label = el('span', {class: 'ptz-protocol-label'}, tr('Protocol'));
  ...
  if (forced) {
    ...
    if (activeType !== forced) {
      ...
      fallback.textContent = tr('%s unavailable - fell back to %s', dom.protoLabel[forced], activeLabel);
...
dom.audioWarningBanner = function(codecs, onTranscode) {
  ...
  var text = el('span');
  text.textContent = tr('Audio (%s) - add Opus transcode for WHEP playback', codecs.join(', '));
  banner.appendChild(text);
  if (onTranscode) {
    var btn = el('button');
    btn.textContent = tr('Add Opus transcode');
    btn.addEventListener('click', function() {
      btn.disabled = true;
      btn.textContent = tr('Adding\u2026');
      onTranscode(function(ok) {
        if (ok) {
          text.textContent = tr('Opus transcode added.');
