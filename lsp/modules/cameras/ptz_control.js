...
function PTZControl() {}
  ...
  PTZControl.prototype.build = function() {
    ...
    intro.textContent = tr('Press and hold movement controls for continuous motion. Release to stop.');
    ...
    if (cameraStreams.length) {
      ...
      img.addEventListener('error', function() {
        ...
        fallback.textContent = tr('Snapshot unavailable');
      ...
      noPreview.textContent = tr('No live preview available.');
  ...
  PTZControl.prototype.renderSpeed = function(panel) {
    ...
    const lbl = el('label');
    lbl.textContent = tr('Motion speed');
  ...
  PTZControl.prototype.renderFocusIris = function(cam, camId, panel) {
    ...
    if (featsLower.indexOf('focus') >= 0) {
      ...
      const lbl = el('label');
      lbl.textContent = tr('Focus');
      section.appendChild(lbl);
      const nearBtn = el('button');
      nearBtn.textContent = tr('Near');
      const farBtn = el('button');
      farBtn.textContent = tr('Far');
  ...
  PTZControl.prototype.renderHome = function(camId, panel) {
    ...
    homeBtn.textContent = tr('Home Position');
  ...
  PTZControl.prototype.renderPresets = function(camId, panel) {
    ...
    const lbl = el('label');
    lbl.textContent = tr('Presets');
    section.appendChild(lbl);
    const note = el('div', {class: 'camera-note'});
    note.textContent = tr('Click to recall. Long press to save.');
