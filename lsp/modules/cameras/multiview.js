...
function multiviewPage(tab, other, prev, $main, $pageHeader) {
  ...
  if (!mist.data.camera_list) {
    $main.textContent = tr('Loading cameras...');
  ...
  if (!hasComposer) {
    ...
    const warnH3 = el('h3');
    warnH3.textContent = tr('Composer not available');
    const warnP = el('p');
    warnP.textContent = tr('The Composer process (MistProcComposer) is required to create multiview streams. Please ensure it is compiled and available.');
  ...
  camTitle.textContent = tr('Select cameras');
  ...
  previewTitle.textContent = tr('Layout preview');
  ...
  function updatePreview() {
    ...
    if (selectedIds.length === 0) {
      previewCanvas.textContent = tr('Select cameras above to preview the layout.');
      return;
    }
  ...
  createBtn.textContent = tr('Create multiview stream');
  const feedback = el('span', {class: 'multiview-feedback'});

  createBtn.addEventListener('click', function() {
    const selectedIds = Object.keys(selected);
    if (selectedIds.length < 2) {
      feedback.textContent = tr('Select at least 2 cameras.');
      feedback.className = 'multiview-feedback error';
      return;
    }

    createBtn.disabled = true;
    feedback.textContent = tr('Creating streams and composing...');
    ...
    function onAllReady() {
      ...
      apiClient.send(function(d) {
        ...
        if (d && d.streams && d.streams[streamName]) {
          feedback.textContent = tr('Multiview stream created! Opening stream…');
          feedback.className = 'multiview-feedback success';
          navto('Status', streamName);
        } else {
          feedback.textContent = tr('Stream creation sent. Check Streams page for result.');
          feedback.className = 'multiview-feedback success';
        }
      }, {addstream: addReq});
    }
