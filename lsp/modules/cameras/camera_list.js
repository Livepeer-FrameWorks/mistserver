...
function devicesPage(tab, other, prev, $main, $pageHeader) {
    ...
    addCamBtn.textContent = tr('Add camera');
    ...
    multiviewBtn.textContent = tr('Multiview');
    ...
    discoveryToggle.setAttribute('title', tr('Continuously discover devices via ONVIF, VISCA and NDI'));
    ...
    discoveryLabel.textContent = tr('Discovery');
    ...
    autoAddToggle.setAttribute('title', tr('Auto-add all devices as streams'));
    ...
    autoAddLabel.textContent = tr('Auto-add streams');
    ...
    autoAddToggle.addEventListener('click', function() {
      const willEnable = !this.classList.contains('toggled');
      if (!willEnable) {
        if (!confirm(tr('Disable auto-add? This will remove all auto-created camera streams.'))) return;
      }
      this.classList.toggle('toggled', willEnable);
      apiClient.send(function(d) {
        if (d && d.camera_config) {
    ...
    if (hasAV) {
      ...
      thumbToggle.setAttribute('title', tr('Enable MJPEG thumbnailing on auto-camera streams'));
      ...
      thumbLabel.textContent = tr('Thumbnailing');
    ...
    const emptyH3 = el('h3');
    emptyH3.textContent = tr('No devices discovered');
    const emptyP = el('p');
    emptyP.textContent = tr('Devices will appear automatically as they are found via ONVIF, VISCA, and NDI on your network.');
    const emptyBtn = el('button');
    emptyBtn.setAttribute('data-icon', 'plus');
    emptyBtn.textContent = tr('Add a camera');
    ...
    if (isThumbnails) {
      $devices = dynamicUI.dynamic({
        ...
        add: {
          create: function(id) {
            ...
            actionsBtn.textContent = tr('Actions');
      ...
      const sortUI = formEngine.buildUI([{
        ...
        'unit': (function() {
          ...
          reverseSpan.textContent = tr('Reverse');
      ...
      tableEl.layout = {
        ...
        name: function(d, id) {
          ...
          actionBtn.textContent = tr('Actions');
    ...
    context_menu.fill = function(id, e) {
      ...
      const actions = [
        ['Remove device', function() {
          if (confirm(tr('Remove this camera from the list?'))) {
            api.removeCamera(id).then(function() {
              delete current_devices[id];
              $devices.update(current_devices);
            })['catch'](function(err) {
              alert(tr('Failed to remove camera: ') + err.message);
            });
          }
      ...
      buttons: [
        {label: 'Add', type: 'save', 'function': function() {
          ...
          api.updateCamera(req).then(function() {
            ...
            alert(tr('Failed to add camera: ') + err.message);
          });
        }},
