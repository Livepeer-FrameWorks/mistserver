...
function buildConfigForm(ctx, capInfo) {
  ...
  if (capInfo && capInfo.actions && capInfo.actions.length) {
    const choices = [['', 'Legacy default response']];
    for (let i = 0; i < capInfo.actions.length; i++) {
      if (capInfo.actions[i] !== 'value') choices.push([capInfo.actions[i], capInfo.actions[i]]);
    }
    advanced.push({
      label: 'On handler failure',
      type: 'select',
      help: 'Outcome when a blocking handler is unavailable, times out, or returns an invalid action.',
      select: choices,
...
function buildReviewStep($container, ctx, callbacks) {
  ...
  if (ctx.onfail) {
    items.push({type: 'span', label: 'On handler failure', value: ctx.onfail});
  }
