      ...
      render: (function(ev, c, cp) {
        return function($container) {
          ...
          if (rules && rules.length) {
            info.appendChild(el('p', {style: 'font-weight: 600; margin-top: 8px;'}, rules.length + ' rule' + (rules.length === 1 ? '' : 's') + tr(' configured')));
          }
...
function buildStreamPicker(ctx, streamNames) {
  ...
  labelRow.appendChild(el('span', {class: 'automation-stream-picker-label'}, tr('Applies to')));
  const allBtn = el('button', {type: 'button', class: 'automation-stream-all'}, tr('All streams'));
  ...
  matchHelp.innerHTML = tr('<code>live</code> exact match only \u2022 <code>live+</code> all wildcard children \u2022 <code>#tag</code> any stream with this tag');
  wrapper.appendChild(matchHelp);

  const hintsRow = el('div', {class: 'var-reference stream-reference automation-stream-hints'});
  hintsRow.appendChild(el('span', {class: 'var-reference-label'}, tr('Streams')));
  const emptyHint = el('span', {class: 'stream-reference-empty'}, tr('No matching streams'));
  const hintBtns = [];
...
function buildConfigureStep($container, ctx, callbacks, caps) {
  ...
  header.appendChild(el('h3', {}, tr(cardInfo.title)));
  ...
  return {
    validate: function() {
      if (!ctx.url || !ctx.url.trim()) {
        alert(tr('Handler URL or path is required.'));
        return false;
      }
      return true;
    }
...
function buildReviewStep($container, ctx, callbacks) {
  ...
  $container.append(el('h3', {}, tr('Review your automation rule')));
  ...
  const formEl = disclosureForm({
    ...
    buttons: [
      ...
      { type: 'save', label: 'Save', 'function': function() {
        if (!ctx.url || !ctx.url.trim()) { alert(tr('Handler is required.')); return; }
      ...
      { label: 'Delete', 'function': function() {
        if (!confirm(tr('Delete this ') + event + ' automation rule?')) return;
        deleteRule(event, ruleIndex, function() {
          modal.close();
          if (onDone) onDone();
        });
...
function buildRuleMeta(rule) {
  ...
  if (rule.data.sync) {
    meta.appendChild(el('span', {class: 'automation-rule-badge automation-rule-badge--blocking'}, tr('Blocking')));
  }
...
registerTab(TAB_AUTOMATIONS, function(tab, other, prev, $main, $pageHeader) {
  ...
  emptyStateTitle = el('h3', {}, tr('No automation rules configured'));
  emptyStateText = el('p', {}, tr('Automation rules let you react to server events - authenticate viewers, redirect streams, track activity, and more.'));
  ...
  dangerZone.appendChild(el('button', {
    onclick: function() {
      if (!confirm(tr('Delete all configured automation rules? This cannot be undone.'))) return;
      mist.data.config.triggers = {};
      apiClient.send(function() { refreshRules(); }, { config: mist.data.config });
    }
  }, tr('Delete all automation rules')));
  ...
  function updateEmptyState(totalRules, visibleRules) {
    ...
    if (totalRules === 0) {
      ...
      emptyStateTitle.textContent = tr('No automation rules configured');
      emptyStateText.textContent = tr('Automation rules let you react to server events - authenticate viewers, redirect streams, track activity, and more.');
      emptyStateAction.setAttribute('data-icon', 'plus');
      emptyStateAction.textContent = tr('Create your first automation');
    ...
    if (visibleRules === 0) {
      ...
      emptyStateTitle.textContent = tr('No automation rules match the current filters');
      emptyStateText.textContent = trn(
        'You have %s automation rule configured. Clear filters to view it.',
        'You have %s automation rules configured. Clear filters to view them.',
        totalRules,
        totalRules
      );
      emptyStateAction.setAttribute('data-icon', 'x');
      emptyStateAction.textContent = tr('Clear filters');
