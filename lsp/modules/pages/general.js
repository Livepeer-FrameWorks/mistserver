...
function saveWithFeedback(btnEl, payload, targetTab) {
  ...
  btnEl._saving = true;
  btnEl.disabled = true;
  btnEl.textContent = tr("Saving..");
  apiClient.send(function() {
    btnEl.textContent = tr("Saved!");
    setTimeout(function() {
...
function playbackWeightsField(settings) {
  const defaults = {
    latency: 50,
    ttff: 50,
    cpu_server: 20,
  ...
  const presets = {
    '': defaults,
    latency: Object.assign({}, defaults, {ttff: 250, latency: 500, stability: 20}),
    ttff: Object.assign({}, defaults, {ttff: 500})
  };
  const weights = settings.weights;
  const custom = Object.assign({}, defaults, (weights && typeof weights === 'object') ? weights : {});
  const labels = {
    abr: ['Adaptive bitrate', 'Protocols supporting adaptive bitrate receive a better score.'],
    bw: ['Bandwidth required', 'Protocols requiring less bandwidth receive a better score.'],
    cpu_server: ['Server-side CPU usage', 'Protocols requiring less media-server CPU receive a better score.'],
    cpu_viewer_pwrd: ['Client-side CPU usage', 'Protocols requiring less CPU on a powered viewing device receive a better score.'],
    cpu_viewer_batt: ['Client-side CPU usage (battery)', 'Protocols requiring less CPU on a battery-powered viewing device receive a better score.'],
    latency: ['Latency', 'Protocols that can play closer to live receive a better score.'],
    ttff: ['Time to first frame', 'Protocols that begin playback sooner receive a better score.'],
    stability: ['Stability', 'Protocols expected to stutter or buffer less receive a better score.'],
    control: ['Controllability', 'Protocols supporting more playback controls receive a better score.'],
    permissibility: ['Permissibility', 'Protocols that work in more restrictive network environments receive a better score.'],
    recovery: ['Recoverability', 'Protocols that recover more reliably from playback errors receive a better score.']
  };
  const customFields = [];
  for (const key in defaults) {
    customFields.push({
      ...
      label: labels[key][0],
      help: labels[key][1],
      pointer: {main: custom, index: key},
      placeholder: defaults[key]
    });
  }
  const customUI = formEngine.buildUI(customFields);

  function matchesPreset(value, preset) {
    if (!value || typeof value !== 'object') return false;
    const keys = Object.keys(defaults);
    return keys.every(function(key){ return Number(value[key]) === Number(preset[key]); }) &&
      Object.keys(value).every(function(key){ return key in defaults; });
  }
  ...
  return {
    ...
    selectinput: [
      ...
      [{type: 'DOMfield', DOMfield: customUI}, 'Custom']
    ],
    getval: function(value, field) {
      const select = field.querySelector(':scope > select');
      if (!select || select.value === 'CUSTOM') {
        const result = {};
        const fields = field.querySelectorAll(':scope > label .field[name]');
        fields.forEach(function(customField){ result[customField.name] = getval(customField); });
        return Object.keys(result).length ? result : Object.assign({}, custom);
      }
      return Object.assign({}, presets[select.value] || defaults);
    },
    setval: function(value, extra, field) {
      const select = field.querySelector(':scope > select');
      let preset = null;
      if (typeof value === 'string' && value in presets) preset = value;
      if (value && typeof value === 'object') {
        for (const key in presets) {
          if (matchesPreset(value, presets[key])) { preset = key; break; }
        }
        Object.assign(custom, defaults, value);
      }
      select.value = preset === null ? 'CUSTOM' : preset;
      const fields = field.querySelectorAll(':scope > label .field');
      fields.forEach(function(customField) {
        if (customField.name in custom) setval(customField, custom[customField.name], ['initial']);
      });
      select.dispatchEvent(new Event('change', {bubbles: true}));
    },
    pointer: {main: settings, index: 'weights'},
    help: 'Playback sources are scored across these categories and sorted from highest to lowest expected performance.'
  };
}
    ...
    (function(theme) {
      ...
      var chip = el('button', {
        ...
        el('span', {class: 'theme-chip-text'}, [
          el('span', {class: 'theme-chip-label'}, tr(theme.label)),
          theme.modes.length > 1
            ? el('span', {class: 'theme-chip-modes'}, tr('dark / light'))
...
function renderExternalWritersTable($target, d) {
  ...
  var trUp = el("tr");
  var thName = el("th");
  thName.textContent = tr("Name");
  var thCmd = el("th");
  thCmd.textContent = tr("Command line");
  var thProto = el("th");
  thProto.textContent = tr("URI protocols handled");
  var thEmpty = el("th");
  ...
  for (var i of writerKeys) {
    ...
    tdP.textContent = uploader[2] ? uploader[2].join(", ") : tr("None");
    var tdA = el("td");
    var editBtn = el("button");
    editBtn.textContent = tr("Edit");
    ...
    removeBtn.addEventListener('click', function(){
      ...
      if (confirm(tr("Are you sure you want to remove the Uploader '")+name+"'?")) {
        apiClient.send(function(){
...
function renderJwksTable($target, d) {
  ...
  for (var entryIdx = 0; entryIdx < jwkEntries.length; entryIdx++) {
    ...
    (function(key, kidEl, entryIdx) {
      ...
      deleteJwkBtn.textContent = tr("Delete");
      deleteJwkBtn.addEventListener('click', function(){
        if (confirm(tr("Are you sure you want to delete this key?\n")+(typeof key == "string" ? key : kidEl.textContent))) {
          apiClient.send(function(){
            navto("General");
          },{
  ...
  var thKeyId = el("th");
  thKeyId.textContent = tr("Key id or url to key set");
  var thKeyType = el("th");
  thKeyType.textContent = tr("Key type");
  var thCanPermit = el("th");
  thCanPermit.textContent = tr("Can permit");
  ...
  $main.appendChild(accordionTree({page: 'General', mode: 'advanced', groupId: 'general-advanced', sections: [
    {
      title: 'General settings',
      subtitle: 'Name, debug level, access logs',
      keywords: ['server name', 'human readable name', 'debug level', 'access log', 'trusted proxies', 'fallback stream', 'prometheus', 'playback source priority', 'weights'],
      requires: ['config'],
      render: function($inner) {
        var s_general = {
          serverid: mist.data.config.serverid,
        ...
        $inner.appendChild(formEngine.buildUI([pageIntro("Core identity and logging settings. Set a human-readable name, control debug log verbosity (level 3 recommended for production), and optionally enable access logging. If "+APP_NAME+" is behind a reverse proxy, add its address to Trusted Proxies so viewer IPs are reported correctly - requests from localhost bypass authentication entirely."),{
          ...
          LTSonly: true
        },playbackWeightsField(s_general),{
          type: 'buttons',
          buttons: [{
            type: 'save',
            label: 'Save',
    ...
    },
    {
      ...
      render: function($inner) {
        ...
        newVarBtn.textContent = tr("New variable");
    ...
    },
    {
      ...
      keywords: ['latitude', 'longitude', 'location', 'geo', 'proximity', 'server tags', 'load balancer tags'],
      requires: ['config'],
      render: function($inner) {
        var s_balancer = {
          location: "location" in mist.data.config ? mist.data.config.location : {},
          tags: Array.isArray(mist.data.tags) ? mist.data.tags.slice() : []
        };
        var balancerUI = formEngine.buildUI([
          pageIntro("Geo coordinates and location name are used by the load balancer for proximity-based routing. These settings have no effect if you're not using a load balancer."),
          {
            ...
            help: "This setting is only useful when "+APP_NAME+" is combined with a load balancer. Tags may be used to choose which server receives a user."
          },{
            type: 'buttons',
            buttons: [{
              type: 'save',
              label: 'Save',
              'function': function(ele){
                saveWithFeedback(ele,{config: {location: s_balancer.location}, tags: s_balancer.tags},'General');
              }
            }]
          }
        ]);
        $inner.appendChild(balancerUI);
        apiClient.send(function(d){
          if (!Array.isArray(d.tags)) return;
          s_balancer.tags = d.tags.slice();
          var tagsField = $inner.querySelector('.field[name="tags"]');
          if (tagsField) setval(tagsField, s_balancer.tags, ['initial']);
        },{tags:true});
      }
    },
    {
    ...
    },
    {
      ...
      render: function($inner) {
        ...
        addJwkBtn.textContent = tr("Add JWK");
        addJwkBtn.addEventListener('click', function(){
          showEditJWKPopup("");
        });
        var jwksDiv = el("div");
        jwksDiv.textContent = tr("Loading..");
        $inner.appendChild(formEngine.buildUI([
...
function showEditJWKPopup(other) {
  ...
  function buildForm(d){
    ...
    const h3Keys = el("h3");
    h3Keys.textContent = tr("Keys");

    const uploadJwksBtn = el("button");
    uploadJwksBtn.setAttribute("data-icon", "up");
    uploadJwksBtn.textContent = tr("Upload JWKS file");
    ...
    uploadJwksBtn.addEventListener('click', function(){
      ...
      function showErr(err) {
        ...
        setTimeout(function(){ me.setAttribute("data-icon", "up"); me.textContent = tr("Upload JWKS file"); },5e3);
      }
    ...
    const h3Grant = el("h3");
    h3Grant.textContent = tr("Grant permissions");
...
registerTab('Stream keys', function(tab, other, prev, $main, $pageHeader) {
  ...
  returnBtn.textContent = tr("Back to Streams");
