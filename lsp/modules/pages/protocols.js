    ...
    entries.push({
      ...
      render: (function(connId, c, h) {
        return function($container) {
          ...
          if (instances.length) {
            $container.appendChild(el('p', {style: 'font-weight: 600; margin-top: 8px;'}, instances.length + ' instance' + (instances.length === 1 ? '' : 's') + tr(' configured')));
          } else {
            $container.appendChild(el('p', {style: 'color: var(--text-muted, #999); margin-top: 8px;'}, tr('Not currently enabled')));
          }
        };
      })(id, cap, hint),
...
function getPortField(connId) {
  const cap = mist.data.capabilities.connectors[connId];
  if (!cap) return null;
  if (cap.required && cap.required.port) return cap.required.port;
  if (cap.optional && cap.optional.port) return cap.optional.port;
  return null;
}

function getPortSummary(connId, instances) {
  const field = getPortField(connId);
  if (!field) return null;

  const ports = [];
  for (let i = 0; i < instances.length; i++) {
    let port = instances[i].data.port;
    if (port === undefined || port === null || port === '') port = field.default;
    if (port === undefined || port === null || port === '') continue;
    if (ports.indexOf(port) < 0) ports.push(port);
  }

  if (!instances.length && field.default !== undefined && field.default !== null && field.default !== '') {
    return 'Default port ' + field.default;
  }
  if (!ports.length) return null;
  return (ports.length === 1 ? 'Port ' : 'Ports ') + ports.join(', ');
}
      ...
      { type: 'save', label: 'Add instance', 'function': function() {
        ...
        apiClient.send(function() {
          ...
          if (countAfter > countBefore) {
            ...
            feedback.textContent = tr('Instance was not added - the server rejected it as a duplicate. Change at least one setting to differentiate this instance.');
...
function buildInstanceSection(container, connId, cap, inst, index, total, modal, onDone, startExpanded, hasHint, actionsToFooter) {
  ...
  if (total > 1 || index === 0) {
    buttons.push({
      label: 'Remove',
      'function': function() {
        if (!confirm(tr('Remove this instance of ') + getFriendly(connId) + '?')) return;
...
function buildCard(connId, onRefresh, mode, ctxMenu) {
  ...
  const portSummary = getPortSummary(connId, instances);
  const urlPattern = mode === 'advanced' ? getUrlPattern(connId) : null;
  const deps = mode === 'advanced' ? getDeps(connId) : null;

  if (urlPattern || deps || portSummary) {
    ...
    if (portSummary) {
      meta.appendChild(el('span', {class: 'proto-card-port'}, portSummary));
    }
