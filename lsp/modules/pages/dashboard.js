...
function buildSubNav(views, activeKey, onChange) {
  ...
  for (let i = 0; i < views.length; i++) {
    const view = views[i];
    const btn = el('button', {class: 'subnav-btn'}, tr(view.label));
    btn._key = view.key;
...
function renderOverview(container) {
  ...
  function updateSystemDetails() {
    ...
    if (hasMem) {
      ...
      phys.appendChild(el('h4', null, tr('Physical memory')));
      ...
      noMem.appendChild(el('h4', null, tr('Physical memory')));
    ...
    if (mem.shmtotal > 0) {
      ...
      shm.appendChild(el('h4', null, tr('Shared memory')));
    ...
    if (mem.swaptotal > 0) {
      ...
      swap.appendChild(el('h4', null, tr('Swap')));
...
function renderStreamDetail(container) {
  ...
  function updateStreamInfo() {
    ...
    if (mist.data.active_streams && mist.data.active_streams.indexOf(selectedStream) > -1) {
      info.appendChild(el('span', {class: 'cam-pill enabled'}, tr('Online')));
    } else {
      info.appendChild(el('span', {class: 'cam-pill disabled'}, tr('Offline')));
    }
  }
...
function renderViewerAnalytics(container) {
  ...
  tableSection.appendChild(el('h3', null, tr('Active Viewers')));
  ...
  function updateViewerTable() {
    ...
    if (!cd || !cd.fields) {
      ...
      noDataRow.appendChild(el('td', null, tr('No viewer data available.')));
...
function renderCustom(container) {
  ...
  form.appendChild(el('h3', null, tr('Add data set')));
  ...
  const addBtn = el('button', null, tr('Add data set'));

  const addToLabel = el('label', null, tr('Add to: '));
  addToLabel.appendChild(graphSelect);
  form.appendChild(addToLabel);

  const graphIdLabel = el('label', {class: 'graph-id-label'}, tr('Graph name: '));
  graphIdLabel.appendChild(graphId);
  form.appendChild(graphIdLabel);

  const dtLabel = el('label', null, tr('Data type: '));
  dtLabel.appendChild(datatypeSelect);
  form.appendChild(dtLabel);

  const originDiv = el('div');
  originDiv.appendChild(el('span', null, tr('Data origin:')));
  ...
  function renderGraph(gid) {
    ...
    removeBtn.addEventListener('click', function() {
      if (confirm(tr('Remove graph "') + gid + '"?')) {
  ...
  addBtn.addEventListener('click', function() {
    ...
    if (targetGraph === 'new') {
      ...
      if (newId in graphs) {
        alert(tr('A graph with this name already exists.'));
        return;
      }
