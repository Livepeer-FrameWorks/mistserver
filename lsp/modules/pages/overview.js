...
defineSection({
  ...
  render: function($container) {
    ...
    section.appendChild(h.infoRow('Version', el('span', null, mist.data.config.version || tr('Unknown'))));
...
defineSection({
  ...
  render: function($container) {
    ...
    if (!on.length) onRow.textContent = tr('None.');
    section.appendChild(el('div', {class: 'proto-heading'}, tr('Enabled')));
    ...
    if (off.length) {
      ...
      section.appendChild(el('div', {class: 'proto-heading'}, tr('Disabled')));
...
registerTab('Overview', function(tab, other, prev, $main, $pageHeader, $pageFooter) {
  ...
  versioncheck.textContent = tr('Loading..');
  ...
  activeproducts.textContent = tr('Unknown');
  ...
  serverH3.textContent = tr('Server');
  ...
  const protoH3 = el('h3');
  protoH3.textContent = tr('Protocols');
  ...
  protoHeadingOn.textContent = tr('Enabled');
  ...
  protoHeadingOff.textContent = tr('Disabled');
  ...
  problemsH3.textContent = tr('Recent problems');
  ...
  const saveBtn = headerButton("disk", "Save", tr("Force an immediate save to the config.json %s uses to save your settings.", APP_NAME));
  ...
  const downloadBtn = headerButton("down", "Download", tr("Download the current %s configuration file as a backup.", APP_NAME));
  ...
  const uploadBtn = headerButton("up", "Upload", tr("Upload a %s configuration file to restore a backup or import from another instance. Your current config will be overwritten!", APP_NAME));
  ...
  function update_update(d) {
    function update_progress(d) {
      ...
      versioncheck.textContent = tr("Updating..")+perc;
      add_logs(d.log);
    ...
    if (!("update" in d) || (d.update && !('uptodate' in d.update))) {
      versioncheck.textContent = tr('Unknown, checking..');
        apiClient.send(function(d){
          if ("update" in d) {
            update_update(d);
          }
      ...
      versioncheck.textContent = tr('Unknown, checking..');
        apiClient.send(function(d){
          if (("update" in d) && (d.update)) {
            ...
            versioncheck.textContent = tr("Failed to check for updates.");
          }
        },{checkupdate:true});
      return;
    }
      ...
      versioncheck.textContent = tr('Your version is up to date.');
      ...
      versioncheck.textContent = tr('Updating..');
      update_progress(d);
    }
    else {
      versioncheck.textContent = "";
      const updateNotice = el("span", {class: 'red'});
      updateNotice.textContent = tr(
        'On %s version %s became available.',
      ...
      if (!d.update.url || (d.update.url.slice(-4) != ".zip")) {
        ...
        rollingBtn.textContent = tr('Rolling update');
        rollingBtn.style.fontSize = '1em';
        rollingBtn.style.marginLeft = '1em';
        rollingBtn.addEventListener('click', function(){
          if (confirm(tr('Are you sure you want to execute a rolling update?'))) {
            versioncheck.classList.add('orange');
            versioncheck.classList.remove('red');
            versioncheck.textContent = tr('Rolling update command sent..');
      ...
      a.textContent = tr("Manual download");
      a.protocol = "https:";
  ...
  if ("license" in mist.data.config) {
    if (("active_products" in mist.data.config.license) && (Object.keys(mist.data.config.license.active_products).length)) {
      ...
      thProduct.textContent = tr("Product");
      const thUpdates = el("th");
      thUpdates.textContent = tr("Updates until");
      const thUse = el("th");
      thUse.textContent = tr("Use until");
      const thInstances = el("th");
      thInstances.textContent = tr("Max. simul. instances");
      ...
      activeproducts.textContent = tr("None. ");
    }
    const moreLink = el("a");
    moreLink.textContent = tr("More details");
  ...
  function enterStats() {
    ...
    if (protocols.on.length) {
      ...
      protocols_on.textContent = tr('None.');
    }
    ...
    if (!httpsFound) {
      ...
      warn.textContent = tr('HTTPS is not enabled. Enable HTTPS or use a reverse proxy for secure playback.');
    ...
    if ('capabilities' in mist.data) {
      ...
      if (protocols.off.length) {
        ...
        protocols_off.textContent = tr('None.');
      }
    }
    else {
      protocols_off.textContent = tr('Loading..');
    }
    updateHints();
  }
  updateViewers();
