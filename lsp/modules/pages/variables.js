...
function buildEmptyState() {
  return el('div', {class: 'variables-empty-state'}, [
    ...
    el('h3', {}, tr('No custom variables yet')),
    el('p', {}, tr('Create one to reuse values in stream sources, push targets, and file paths.'))
  ]);
}
...
function renderTable($container, opts) {
  opts = opts || {};
  $container.innerHTML = tr('Loading..');
  ...
  apiClient.send(function(d) {
    ...
    const table = el('table', {}, [
      el('thead', {},
        el('tr', {}, [
          el('th', {}, tr('Variable')),
          el('th', {}, tr('Latest value')),
          el('th', {}, tr('Command or url')),
          el('th', {}, tr('Check interval')),
          el('th', {}, tr('Last checked')),
          el('th')
        ])
      ),
      tbody
    ...
    for (const i in d.variable_list) {
      try {
        ...
        const lastCheckedCell = el('td', {
          title: hasTarget
            ? (v.lastunix > 0 ? tr('At %s', format.dateTime(v.lastunix, 'long')) : tr('Not yet'))
            : ''
        });
        if (hasTarget) {
          lastCheckedCell.innerHTML = v.lastunix > 0
            ? tr('%s ago', format.duration(new Date().getTime() * 1e-3 - v.lastunix))
            : tr('Not yet');
        }
        ...
        tbody.appendChild(
          el('tr', { class: 'variable', 'data-name': i }, [
            ...
            targetCell,
            el('td', {},
              hasTarget ? (v.interval == 0 ? 'Once' : format.duration(v.interval)) : tr('Never')
            ),
            lastCheckedCell,
            el('td', {}, [
              el('button', {
              ...
              el('button', {
                onclick: function() {
                  const name = this.closest('tr').getAttribute('data-name');
                  if (confirm(tr('Are you sure you want to remove the custom variable $') + name + '?')) {
  ...
  function build(saveas, saveas_dyn) {
    ...
    modal = uiHelpers.openFormModal({
      ...
      form: [
      {
        ...
        'function': function() {
          let b = [el('span', {}, tr('Invalid variable type'))];
          switch (this.value) {
            case 'value': {
              b = [{
                type: 'str',
