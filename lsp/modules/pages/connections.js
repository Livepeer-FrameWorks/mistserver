  ...
  return dynamicUI.dynamic({
    create: function () {
      ...
      COLUMNS.forEach(function (col) {
        headerRow.appendChild(el('th', { class: col.numeric ? 'header numeric' : 'header', 'data-index': col.label }, tr(col.label)));
      });
...
function renderConnections(tab, other, prev, $main) {
  ...
  function applyFilter(isInitial) {
    ...
    apiClient.send(function (d) {
      ...
      if (amount > COUNT_WARN) {
        ...
        if (!confirm(tr('Your current filters would yield ') + amount + ' results. Are you sure you want to query all of them?')) { state.active = false; return; }
      }
  ...
  $form.appendChild(formEngine.buildUI([
    el('h3', null, tr('Filter connections')),
