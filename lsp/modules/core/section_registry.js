...
function buildHaystack(entry) {
  var parts = [entry.title || '', entry.title ? tr(entry.title) : ''];
  if (entry.subtitle) parts.push(entry.subtitle, tr(entry.subtitle));
  if (entry.page) parts.push(entry.page);
  if (entry.keywords) {
    for (var i = 0; i < entry.keywords.length; i++) {
      parts.push(entry.keywords[i], tr(entry.keywords[i]));
    }
  }
...
export function searchSections(query, opts) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    var titleLower = (entry.title || '').toLowerCase() + ' ' + (entry.title ? tr(entry.title).toLowerCase() : '');
    var subtitleLower = (entry.subtitle || '').toLowerCase() + ' ' + (entry.subtitle ? tr(entry.subtitle).toLowerCase() : '');
