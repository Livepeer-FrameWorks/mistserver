...
import { getAvailableLocales, getLocale, setLanguage, tr } from './i18n.js';
...
AppShell.getModeHint = function() {
  ...
  if (isAdvanced) {
    return tr('Advanced mode is active. Switch to Guided for a simplified UI.');
  }
  return tr('Guided mode is active. Switch to Advanced for full controls.');
};
...
AppShell.init = function() {
  ...
  for (const code in locales) {
    languageSelect.appendChild(new Option(locales[code], code));
  }
...
AppShell.updateThemeLabel = function(t) {
  ...
  const isDark = (t === 'dark');
  btn.setAttribute('title', isDark ? tr('Dark theme') : tr('Light theme'));
  btn.setAttribute('aria-pressed', isDark ? 'true' : 'false');
  btn.setAttribute('aria-label', isDark ? tr('Switch to light theme') : tr('Switch to dark theme'));
};
...
AppShell.updateModeLabel = function(m) {
  ...
  const btn = q('#mode-toggle');
  btn.setAttribute('title', isAdvanced ? tr('Advanced mode') : tr('Guided mode'));
  btn.setAttribute('aria-pressed', isAdvanced ? 'true' : 'false');
  btn.setAttribute('aria-label', isAdvanced ? tr('Switch to guided mode') : tr('Switch to advanced mode'));
};
