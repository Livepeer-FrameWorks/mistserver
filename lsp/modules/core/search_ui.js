...
export function initSearchUI() {
  ...
  $searchInput = el('input', {
    ...
    placeholder: tr('Search settings, protocols...'),
...
registerTab('Search', function(tab, other, prev, $main, $pageHeader) {
  ...
  if (!query.trim()) {
    ...
    $main.appendChild(el('div', {class: 'search-empty-state'}, [
      ...
      el('h3', null, tr('Search MistServer settings')),
      el('p', null, tr('Press Ctrl+K or start typing to find settings, protocols, and more.'))
    ]));
    return;
  }
  ...
  if (!results.length) {
    ...
    $main.appendChild(el('div', {class: 'search-empty-state'}, [
      el('span', {class: 'search-empty-icon', 'data-icon': 'search'}),
      el('h3', null, tr('No results for \u201c%s\u201d', query)),
      el('p', null, tr('Try different keywords or check the spelling.'))
    ]));
    return;
  }
  ...
  for (var i = 0; i < results.length; i++) {
    (function(result, idx) {
      ...
      header.appendChild(el('span', {class: 'search-result-badge'}, tr(section.page)));
      header.appendChild(el('span', {class: 'search-result-title'}, tr(section.title)));
      if (section.subtitle) {
        header.appendChild(el('span', {class: 'search-result-subtitle'}, tr(section.subtitle)));
      }
