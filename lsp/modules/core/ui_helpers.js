...
export const uiHelpers = {
  statCard: function(icon, value, label) {
    return el('div', {class: 'overview-stat-card'}, [
      ...
      el('div', {class: 'stat-content'}, [
        value,
        el('div', {class: 'stat-label'}, tr(label))
      ])
    ]);
  },
  ...
  infoRow: function(label, value) {
    const row = el('div', {class: 'overview-info-row'}, [
      el('span', {class: 'info-label'}, tr(label)),
  ...
  unavailableText: function(reason) {
    return el('span', {class: 'text-muted'}, tr(reason || 'Not available'));
  },
  ...
  reviewSection: function(title, stepId, callbacks) {
    return el('div', {class: 'review-section-header'}, [
      el('h4', {class: 'review-section-title'}, tr(title)),
  ...
  setPageTitle: function(pageHeader, title, subtitle) {
    ...
    if (typeof title !== 'undefined') {
      titleEl.textContent = title ? tr(title) : '';
    }
  ...
  createSubtabHeader: function(options) {
    ...
    const titleEl = el(titleTag, null, options.title ? tr(options.title) : '');
    titleGroup.appendChild(titleEl);
    if (options.subtitle) {
      titleGroup.appendChild(el('p', {class: 'subtab-subtitle'}, tr(options.subtitle)));
    }
      ...
      for (let i = 0; i < tabs.length; i++) {
        ...
        if (tabInfo.disabled) {
          ...
          if (tabInfo.disabledTitle) {
            attrs.title = tr(tabInfo.disabledTitle);
          }
        } else {
          attrs.tabindex = '0';
        }
        const tabText = tabInfo.label || tabInfo.tab;
        const tabEl = el('li', attrs, tabText ? tr(tabText) : '');
  ...
  createSmartSelect: function(options) {
    ...
    const labelText = tr(options.label || 'Select');
    ...
    const search = el('input', {
      ...
      type: 'search',
      placeholder: tr(options.searchPlaceholder || 'Type to filter...')
    });
    const select = el('select', {class: 'smart-select-dropdown'});
    const quickLabel = el('div', {class: 'smart-select-quick-label'}, tr(options.quickLabel || 'Top picks'));
    ...
    function normalizeOption(input) {
      if (input && typeof input === 'object' && !Array.isArray(input)) {
        ...
        const sourceLabel = ('label' in input) ? String(input.label) : value;
        return {
          value: value,
          label: tr(sourceLabel),
          search: ((('search' in input) ? String(input.search) : '') + ' ' + sourceLabel).toLowerCase()
        };
      }
    ...
    function renderSelect() {
      ...
      if (allowEmpty) {
        const defaultOpt = el('option', null, tr(options.emptyLabel || 'Select...'));
        defaultOpt.value = '';
      ...
      if (!matchCount && allowEmpty) {
        const noMatch = el('option', null, tr(options.noMatchLabel || 'No matches'));
        noMatch.value = '';
    ...
    function renderQuickPicks() {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        const btn = el('button', {
          ...
          title: tr(pick.title || pick.label),
  ...
  openCopyFallback: function(options) {
    ...
    const title = tr(options.title || 'Copy to clipboard');
    const label = tr(options.label || 'Text');
    ...
    const help = options.help || (
      tr('Automatic copying failed%s. Instead you can manually copy from the field below.', err ? ' (' + err + ')' : '')
    );
  ...
  openConfirm: function(options) {
    ...
    if (!actions.length) {
      actions = [{
        type: 'cancel',
        label: tr(options.cancelLabel || 'Cancel'),
        close: true
      }, {
        type: 'save',
        label: tr(options.confirmLabel || 'Confirm'),
        close: true,
    ...
    function wrapButtons(defs) {
      ...
      for (let i = 0; i < defs.length; i++) {
        ...
        for (const key in def) {
          if (key === 'onClick' || key === 'function' || key === 'close') continue;
          wrapped[key] = key === 'label' ? tr(def[key]) : def[key];
        }
  ...
  createShell: function(options) {
    ...
    const api = {
      ...
      setTitle: function(content) {
        setContent(titleEl, typeof content === 'string' ? tr(content) : content);
        refreshVisibility();
        return api;
      },
      setSubtitle: function(content) {
        setContent(subtitleEl, typeof content === 'string' ? tr(content) : content);
        refreshVisibility();
        return api;
      },
  ...
  openModal: function(options) {
    ...
    const shell = uiHelpers.createShell({
      ...
      title: options.title,
      subtitle: options.subtitle,
      body: ('body' in options) ? options.body : el('div', null, tr(options.loadingText || 'Loading...')),
      footer: options.footer,
      headerActions: options.headerActions
    });
  ...
  dashboardSectionHeader: function(icon, title) {
    return el('h3', {class: 'dashboard-section-header', 'data-icon': icon}, tr(title));
  }
};
