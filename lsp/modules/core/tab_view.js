  ...
  showTab: function(tab,other,prev) {
    ...
    if (mist.user.loggedin) {
      if (!('ui_settings' in mist.data)) {
        UI.elements.main.textContent = tr('Loading..');
    ...
    for (let pi = 0; pi < plains.length; pi++) {
      if (plains[pi].closest('.button').getAttribute('data-tab') === tab) {
    ...
    const shell = uiHelpers.createPageShell({
      title: tr(tab),
      subtitle: UI.pageSubtitles[tab] ? tr(UI.pageSubtitles[tab]) : false
    });
    ...
    switch (tab) {
      ...
      case 'Logs': {
        ...
        const intro = formEngine.buildUI([{type:'help',classes:['page-intro'],help:tr('Server log output. Use the actions to inspect or export log contents.')}]);
      ...
      default:
        if (getTabHandler(tab)) {
          ...
          $pageBody.append(el('p', null, tr('This tab does not exist.')));
        }
        break;
    }
