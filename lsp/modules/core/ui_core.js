...
pagecontrol: function(page_ele,default_page_size){
  ...
  prevBtn.textContent = tr("Previous");
  ...
  labelSpan.textContent = tr("Items per page:");
...
sortableItems: function(item_container,getVal,options){
  ...
  item_container.sort = function(sortby,sortdir){
    ...
    try {
      const compare = new Intl.Collator(getLocale(),{numeric:true, sensitivity:'accent'}).compare;
...
buildMenu: function(){
  function createButton(j,button) {
    ...
    plainSpan.textContent = tr(j);
    const highlightedSpan = el('span', {class: 'highlighted'});
    highlightedSpan.textContent = tr(j);

    const labelSpan = el('span', {class: 'nav-label'});
    labelSpan.appendChild(plainSpan);
    labelSpan.appendChild(highlightedSpan);
