...
send: function(callback,sendData,opts){
  ...
  if (!opts.hide) {
    const cancelLink = el('a', null, tr('Cancel request'));
    cancelLink.addEventListener('click', function(){
      controller.abort();
    });
    const msgEl = UI.elements.connection.msg;
    msgEl.classList.remove('red');
    msgEl.textContent = tr('Data sent, waiting for a reply..');
  ...
  fetch(mist.user.host, {
    ...
    switch (d.authorize.status) {
      case 'OK':
        ...
        const keep = ['config','capabilities','ui_settings','LTS','active_streams','browse','log','totals','bandwidth','tags','variable_list','external_writer_list','streamkeys','camera_list','clients'];
        for (const i in save) {
          if (keep.indexOf(i) == -1) {
            delete save[i];
          }
        ...
        UI.elements.connection.status.textContent = tr('Connected');
        ...
        msgEl.appendChild(
          el("span", {"class": "status-last-seen"},
            tr('Updated %s', format.time((new Date).getTime()/1000))
          )
        );

        msgEl.appendChild(
          el("span", {"class": "status-recent-label"}, tr('Recent logs'))
        );
        ...
        if (d.log && d.log.length) {
          ...
          logsContainer.appendChild(
            el("div", {"class": "status-log-line status-log-line-empty"}, [
              el("span", {"class": "status-log-msg"}, tr('No recent logs.'))
            ])
          );
        }
        ...
        if (d.authorize.challenge == mist.user.authstring) {
          if (mist.user.password != '') {
            UI.elements.connection.msg.textContent = tr('The credentials you provided are incorrect.');
    ...
    if (!opts.hide) {
      ...
      if (err.name === 'AbortError') {
        statusEl = el('i', null, tr('The connection timed out or was aborted. '));
      } else {
        statusEl = el('i', {style: {textTransform: 'capitalize'}}, (err.message || tr('Connection error')) + '. ');
      }
      const retryLink = el('a', null, tr('Send server request again'));
      retryLink.addEventListener('click', function(){
        apiClient.send(callback,sendData,opts);
      });
      const msgEl = UI.elements.connection.msg;
      msgEl.classList.add('red');
      msgEl.textContent = tr('An error occurred while attempting to communicate with %s:', APP_NAME);
      msgEl.appendChild(el('br'));
