  ...
  for (const i in elements) {
    ...
    if (e.type == 'buttons') {
      ...
      for (const j in e.buttons) {
        ...
        if (!button) continue;
        const b = el('button', null, tr(button.label));
        b._opts = button;
        if ('css' in button) {
    ...
    labelSpan.innerHTML = ('label' in e ? tr(e.label)+':' : '');
    ...
    switch (e.type) {
      ...
      case 'select':
        ...
        for (const j in e.select) {
          ...
          if (typeof e.select[j] == 'string') {
            option.textContent = tr(e.select[j]);
          }
          else {
            option.value = e.select[j][0];
            option.textContent = tr(e.select[j][1]);
          }
      ...
      case 'radioselect':
        ...
        for (const i in e.radioselect) {
          ...
          radioSpan.innerHTML = tr(e.radioselect[i][1]);
          ...
          if (e.radioselect[i].length > 2) {
            ...
            for (const j in e.radioselect[i][2]) {
              ...
              if (e.radioselect[i][2][j] instanceof Array) {
                ...
                opt.innerHTML = tr(e.radioselect[i][2][j][1]);
              }
              else {
                opt.innerHTML = tr(e.radioselect[i][2][j]);
              }
              sel.appendChild(opt);
            }
          }
      ...
      case 'checklist':
        ...
        for (const i in e.checklist) {
          ...
          checkLabel.textContent = tr(e.checklist[i][1]);
      ...
      case "selectinput":
        ...
        for (const i in e.selectinput) {
          ...
          if (typeof e.selectinput[i] == "string") {
            opt.textContent = tr(e.selectinput[i]);
          }
          else {
            opt.textContent = tr(e.selectinput[i][1]);
      ...
      case "sublist": {
        ...
        curvals.appendChild(el("span", null, tr("None.")));
    ...
    if ('placeholder' in e) {
      $field.setAttribute('placeholder',tr(e.placeholder));
    }
    if ('default' in e) {
    ...
    if ('qrcode' in e) {
      ...
      qrButton.addEventListener('click', function(){
        ...
        function renderNow(){
          ...
          if (!qrRenderer) {
            qrDiv.textContent = tr('QR renderer unavailable');
            return;
          }
          qrDiv.innerHTML = '';
          qrRenderer(qrDiv, {
    ...
    if (('clipboard' in e) && (document.queryCommandSupported('copy'))) {
      ...
      copyButton.addEventListener('click', function(){
        ...
        copy(text).then(function(){
          me.textContent = tr('Copied!');
          setTimeout(function(){
            me.textContent = tr('Copy');
          },5e3);
        }).catch(function(e){
    ...
    switch (e.type) {
      case 'browse':
        ...
        browseBtn.textContent = tr('Browse');
        ...
        browseBtn.addEventListener('click', function(){
          ...
          browseLabel.appendChild(el('span', {class: 'label'}, tr('Current folder:')));
          ...
          function browse(path){
            folderContents.textContent = tr('Loading..');
            apiClient.send(function(d){
              ...
              upFolder.setAttribute('title',tr('Folder up'));
      ...
      case 'hostlimited':
        ...
        bwOptBlack.value = '-';
        bwOptBlack.textContent = tr('Blacklist');
        subUI.blackwhite.appendChild(bwOptBlack);
        const bwOptWhite = el('option');
        bwOptWhite.value = '+';
        bwOptWhite.textContent = tr('Whitelist');
        ...
        switch (e.type) {
          case 'geolimited':
            ...
            defaultOpt.value = '';
            defaultOpt.textContent = tr('[Select a country]');
    ...
    if ('help' in e) {
      ...
      let helpLabel = e.label ? tr('Show help for %s', tr(e.label)) : tr('Show help');
      let balloon = el('span', {class: 'ih_balloon', id: helpId, role: 'tooltip', popover: 'auto'});
      if (typeof e.help === 'string') {
        balloon.innerHTML = tr(e.help);
      } else if (e.help instanceof HTMLElement) {
...
convertBuildOptions: function(input,saveas) {
    ...
    if ("influences" in ele) {
      ...
      obj.observe = function(isVisible){
        ...
        styles.forEach(function(s){
          ...
          else if (s._content && (s.innerHTML !== s._content)) {
  ...
  for (const j in type) {
    if (input[type[j]] && Object.keys(input[type[j]]).length) {
      const heading = el('h4');
      heading.textContent = tr('%s parameters', format.capital(type[j]));
      build.push(heading);
