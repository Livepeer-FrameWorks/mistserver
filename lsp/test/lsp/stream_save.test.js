...
test('streamSave includes explicitly prepared related streams', () => {
  let sent = null;
  const ctx = loadModule('modules/streams/stream_save.js');
  ctx.mist.data = {
    streams: {},
  ...
  ctx.apiClient.send = (callback, data) => {
    sent = data;
    callback();
  };
  ctx.navto = () => {};
  ...
  const related = {
    camera: {
      ...
      processes: {AV_composer_decode_video: {process: 'AV', codec: 'UYVY'}}
    }
  };
  ctx.streamSave({name: 'wall', source: 'compose'}, 'wall', 'Streams', null, related);

  assert.equal(sent.addstream.wall.source, 'compose');
  assert.equal(sent.addstream.camera.processes.AV_composer_decode_video.codec, 'UYVY');
  assert.equal(ctx.mist.data.streams.camera.source, 'rtsp://camera/live');
});
