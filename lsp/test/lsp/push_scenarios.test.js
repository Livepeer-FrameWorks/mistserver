...
test('loadStreamList falls back to configured streams without logging out on lookup failure', () => {
  let callbackStreams = null;
  let requestOptions = null;
  const ctx = loadModule('modules/pushes/push_scenarios.js', {
    apiClient: {
      send(callback, request, options) {
        requestOptions = options;
        options.onError(new Error('temporary lookup failure'));
      }
    },
    findInput() { return null; }
  });
  ctx.mist.data.streams = {
    alpha: { source: 'push://' },
    beta: { source: 'rtsp://camera/live' }
  };

  ctx.pushScenarios.loadStreamList(function(streams) { callbackStreams = streams; });

  assert.equal(requestOptions.noLoginOnError, true);
  assert.equal(requestOptions.hide, true);
  assert.deepEqual(plain(callbackStreams), ['alpha', 'beta']);
});

test('loadStreamList completes when an optional folder browse fails', () => {
  let callbackStreams = null;
  const requests = [];
  const ctx = loadModule('modules/pushes/push_scenarios.js', {
    apiClient: {
      send(callback, request, options) {
        ...
        if (request.active_streams) {
          callback({ active_streams: ['live+alice'] });
        } else {
          options.onError(new Error('folder unavailable'));
        }
      }
    },
    findInput(name) { return name === 'Folder' ? { source_match: '/folder/*' } : null; },
    mistHelpers: {
      inputMatch(pattern, value) { return pattern === '/folder/*' && value.indexOf('/folder/') === 0; }
    }
  });
  ctx.mist.data.streams = { archive: { source: '/folder/media' } };

  ctx.pushScenarios.loadStreamList(function(streams) { callbackStreams = streams; });

  assert.equal(requests.length, 2);
  assert.equal(requests[1].options.noLoginOnError, true);
  assert.deepEqual(plain(callbackStreams), ['archive', 'archive+', 'live+', 'live+alice']);
});
