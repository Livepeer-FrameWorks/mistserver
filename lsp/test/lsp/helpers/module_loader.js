...
function createBaseContext() {
  ...
  const ctx = {
    ...
    Option: function(label, value) { this.label = label; this.textContent = label; this.value = value; },
    msg(message) { return message; },
    tr(message) {
      const args = Array.prototype.slice.call(arguments, 1);
      let next = 0;
      return typeof message === 'string' ? message.replace(/%(?:(\d+)\$)?([%s])/g, function(match, position, kind) {
        if (kind === '%') return '%';
        const value = position ? args[Number(position) - 1] : args[next++];
        return value === undefined || value === null ? '' : String(value);
      }) : message;
    },
    trn(singular, plural, count) { return (count === 1 ? singular : plural).replace('%s', String(count)); },
    getLocale() { return 'en'; },
    getAvailableLocales() { return { en: 'English', 'de-DE': 'Deutsch', 'es-ES': 'Español' }; },
    setLanguage() {},
    deepExtend,
    log() {},
    MD5(v) { return 'md5:' + v; },
  ...
  ctx.window.location.reload = function() {};
  ctx.global = ctx;
  return ctx;
}
...
function loadModule(relPath, overrides) {
  ...
  src = src.replace(/^(\s*)export\s+async\s+function\s+([A-Za-z_$][\w$]*)/gm, (_, indent, name) => {
