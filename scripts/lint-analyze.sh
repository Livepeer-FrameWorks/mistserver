...
printf "%-45s %s\n" "Linter" "Count"
printf "%-45s %s\n" "---------------------------------------------" "-----"
grep -oE "\[[-a-zA-Z]+\]" "$REPORT_FILE" | sort | uniq -c | sort -rn | while read -r count linter; do
    printf "%-45s %5d\n" "$linter" "$count"
done
echo ""
...
while IFS= read -r line; do
    if [[ "$line" =~ ^"=== COMPONENT:" ]]; then
        ...
        current_component=${line#=== COMPONENT: }
...
printf "%-60s %s\n" "File" "Warnings"
printf "%-60s %s\n" "------------------------------------------------------------" "--------"
grep -oE "^[^:]+\.cpp:[0-9]+:[0-9]+:" "$REPORT_FILE" | cut -d: -f1 | sort | uniq -c | sort -rn | head -20 | while read -r count file; do
    printf "%-60s %5d\n" "$file" "$count"
done
echo ""
