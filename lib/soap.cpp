...
namespace SOAP {
  ...
  Message::Message(const std::string & actionUri, Version ver)
    : doc(), version(ver), action(actionUri) {
  ...
  Message::Message(Message && other) noexcept
    : doc(std::move(other.doc)), envelope(std::move(other.envelope)),
      header(std::move(other.header)), body(std::move(other.body)), version(other.version),
      action(std::move(other.action)), namespaces(std::move(other.namespaces)) {
    // The libxml2 tree moved with `doc`; re-point the cached node handles at our own doc so
    // their owner back-pointer does not dangle to the moved-from message's document.
    envelope.rebindOwner(&doc);
    header.rebindOwner(&doc);
    body.rebindOwner(&doc);
  }
  ...
  void Message::addHeader(const std::string & name, const std::string & value, bool mustUnderstand) {
    ...
    if (colonPos == std::string::npos) {
      throw std::runtime_error("Header name must include namespace prefix");
    }
  ...
  void Message::addSecurityHeader(const SecurityHeader & security) {
    ...
    passwordNode.setTextContent(generatePasswordDigest(
      security.usernameToken.nonce, security.usernameToken.created,
  ...
  Message Message::parse(const std::string & xml) {
    ...
    // Now lock while modifying the message's internal state
    {
      ...
      // `root`/`header`/`body` were bound to the now-moved-from local `parsedDoc`. The libxml2
      // tree moved into msg.doc (still valid), but these node handles' owner back-pointer still
      // references the stack-local document, which is destroyed when this function returns.
      // Re-point them at msg.doc so they stay valid for the returned message.
      msg.envelope.rebindOwner(&msg.doc);
      msg.header.rebindOwner(&msg.doc);
      msg.body.rebindOwner(&msg.doc);
  ...
  namespace onvif {
    ...
    void DiscoveryFactory::createDiscoveryBody(Message & msg, const DiscoveryOptions & options,
      ...
      if (!options.scopes.empty()) {
        ...
        if (options.matchBy && !options.matchByUri.empty()) {
          scopes.setAttribute("MatchBy", options.matchByUri);
        }
      }
    ...
    void ONVIFMessageFactory::addServiceNamespaces(Message & msg, ServiceType service) const {
      ...
      switch (service) {
        case ServiceType::Device:
          msg.addNamespace("tds", "http://www.onvif.org/ver10/device/wsdl");
          break;
        case ServiceType::Media:
          msg.addNamespace("trt", "http://www.onvif.org/ver10/media/wsdl");
          break;
        case ServiceType::PTZ:
          msg.addNamespace("tptz", "http://www.onvif.org/ver20/ptz/wsdl");
          break;
        ...
        case ServiceType::Imaging:
          msg.addNamespace("timg", "http://www.onvif.org/ver20/imaging/wsdl");
          break;
        case ServiceType::Discovery: msg.addWSDiscoveryNamespaces(); break;
      }
    }

    void ONVIFMessageFactory::addStandardHeaders(Message & msg, const std::string & action,
    ...
    Message ONVIFMessageFactory::createMessage(ServiceType service, const std::string & action,
                                               const std::string & to) const {
  ...
  std::string generatePasswordDigest(const std::string & nonce, const std::string & created,
