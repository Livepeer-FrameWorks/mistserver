...
namespace ONVIF {
  ...
  /**
   * @brief Mask the text content of every Password element in an XML/SOAP string for safe logging.
   * @note Covers both cleartext <tt:Password> and attributed WS-Security <wsse:Password Type=...>.
  ...
  class Device : public ::Device::Base {
    public:
      ...
      void setTLSConfig(const std::string &policy, const std::string &caFile) {
        const std::string nextPolicy = policy.empty() ? "opportunistic" : policy;
        if (tlsPolicy != nextPolicy || tlsCaFile != caFile) tlsStatus.clear();
        tlsPolicy = nextPolicy;
        tlsCaFile = caFile;
      }
      std::string getTLSStatus() const { return tlsStatus; }
      ...
      Result<std::vector<ONVIFEvent>> pullMessages(const std::string & subscriptionEndpoint,
                                                    int timeoutSec = 5, int messageLimit = 10);
    ...
    protected:
      ...
      std::string path;
      uint16_t port;
      std::string scheme = "http"; // transport scheme: "http" (default) or "https"
      std::string tlsPolicy = "opportunistic";
      std::string tlsCaFile;
      mutable std::string tlsStatus;
      std::string username;
      std::string password;
      std::string name;
      std::string uri;
