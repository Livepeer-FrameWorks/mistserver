...
namespace XML {
  ...
  Node Node::getChildNS(const std::string & namespaceURI, const std::string & localName) const {
    ...
    if (ownerDoc) {
      ...
      while (c) {
        ...
        if (c->type == XML_ELEMENT_NODE && xmlStrcmp(c->name, BAD_CAST localName.c_str()) == 0 &&
            c->ns && c->ns->href && xmlStrcmp(c->ns->href, BAD_CAST namespaceURI.c_str()) == 0) {
  ...
  std::map<std::string, std::string> Node::getAttributes() const {
    ...
    for (xmlAttr *attr = node->properties; attr; attr = attr->next) {
      if (attr->name && attr->children && attr->children->content) {
        attrs[reinterpret_cast<const char *>(attr->name)] =
  ...
  Document::Document(const std::string & xml) : doc(nullptr), ctx(nullptr) {
    ...
    // SOAP/WS-Discovery never requires a DTD. Reject it outright so neither local nor remote
    // external entities can be reached, independent of libxml2 version-specific parser behavior.
    if (xml.find("<!DOCTYPE") != std::string::npos) {
      throw std::runtime_error("DTD declarations are not allowed");
    }

    // Parse XML with security flags to prevent XXE attacks. Deliberately NOT setting
    // XML_PARSE_NOENT (so internal entities are not substituted/expanded - this defuses
    // billion-laughs / entity-expansion blowups) nor XML_PARSE_HUGE (keeps libxml2's built-in
    // depth/size limits). DTD declarations are rejected above and network loading is disabled.
  ...
  Document::Document(Document && other) noexcept
    : doc(other.doc), ctx(other.ctx), lastError(std::move(other.lastError)),
  ...
  void Document::registerNamespacesInContext(xmlNodePtr node) {
    ...
    if (node->ns && node->ns->prefix && node->ns->href) {
      xmlXPathRegisterNs(ctx, node->ns->prefix, node->ns->href);
    }

    // Register namespaces from attributes
    for (xmlAttrPtr attr = node->properties; attr; attr = attr->next) {
      if (attr->ns && attr->ns->prefix && attr->ns->href) {
        xmlXPathRegisterNs(ctx, attr->ns->prefix, attr->ns->href);
      }
    }
  }
  ...
  void Document::registerNamespacesRecursive(xmlNodePtr node) {
    ...
    for (xmlNodePtr child = node->children; child; child = child->next) {
      registerNamespacesRecursive(child);
    }
  }
