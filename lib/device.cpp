...
namespace Device {
  ...
  JSON::Value DeviceInfo::toJSON() const {
    ...
    for (const auto & stream : streams) {
      ...
      streamObj["resolution"] = stream.resolution;
      streamObj["framerate"] = stream.framerate;
      ...
      for (const auto & option : stream.options) { optionsObj[option.first] = option.second; }
      streamObj["options"] = optionsObj;
    ...
    for (const auto & pair : protocols) {
      ...
      protoObj["endpointId"] = pair.second.endpointId;
      protoObj["address"] = pair.second.address;
      protoObj["port"] = (int)pair.second.port;
      protoObj["scheme"] = pair.second.scheme;
      protoObj["transport"] = pair.second.transport;
      protoObj["framing"] = pair.second.framing;
      protoObj["path"] = pair.second.path;
      protoObj["username"] = pair.second.username;
      protoObj["password"] = pair.second.password;
      protoObj["tlsPolicy"] = pair.second.tlsPolicy;
      protoObj["tlsCaFile"] = pair.second.tlsCaFile;
      protoObj["tlsStatus"] = pair.second.tlsStatus;
      protoObj["lastError"] = pair.second.lastError;
      protoObj["associationSource"] = pair.second.associationSource;

      JSON::Value alternatesArray;
      for (const auto & endpoint : pair.second.alternateEndpoints) { alternatesArray.append(endpoint); }
      if (pair.second.alternateEndpoints.empty()) { alternatesArray.append(); alternatesArray.shrink(0); }
      protoObj["alternateEndpoints"] = alternatesArray;
      ...
      JSON::Value resolutionsArray;
      for (const auto & resolution : pair.second.capabilities.supportedResolutions) {
        resolutionsArray.append(resolution);
      }
      if (pair.second.capabilities.supportedResolutions.empty()) {
        resolutionsArray.append();
        resolutionsArray.shrink(0);
      }
      capsObj["supportedResolutions"] = resolutionsArray;

      JSON::Value frameratesArray;
      for (const auto & framerate : pair.second.capabilities.supportedFramerates) {
        frameratesArray.append(framerate);
      }
      if (pair.second.capabilities.supportedFramerates.empty()) {
        frameratesArray.append();
        frameratesArray.shrink(0);
      }
      capsObj["supportedFramerates"] = frameratesArray;

      JSON::Value audioFormatsArray;
      for (const auto & format : pair.second.capabilities.supportedAudioFormats) {
        audioFormatsArray.append(format);
      }
      if (pair.second.capabilities.supportedAudioFormats.empty()) {
        audioFormatsArray.append();
        audioFormatsArray.shrink(0);
      }
      capsObj["supportedAudioFormats"] = audioFormatsArray;
    ...
    for (const auto & mod : analytics.supportedModules) {
      ...
      if (mod.parameters.empty()) { paramsArray.append(); paramsArray.shrink(0); }
      modObj["parameters"] = paramsArray;
      supportedModulesArray.append(modObj);
    }
    if (analytics.supportedModules.empty()) { supportedModulesArray.append(); supportedModulesArray.shrink(0); }
    ...
    if (analytics.activeModules.empty()) { activeModulesArray.append(); activeModulesArray.shrink(0); }
    ...
    for (const auto & rule : analytics.supportedRules) {
      ...
      if (rule.parameters.empty()) { paramsArray.append(); paramsArray.shrink(0); }
      ruleObj["parameters"] = paramsArray;
      supportedRulesArray.append(ruleObj);
    }
    if (analytics.supportedRules.empty()) { supportedRulesArray.append(); supportedRulesArray.shrink(0); }
    ...
    if (analytics.activeRules.empty()) { activeRulesArray.append(); activeRulesArray.shrink(0); }
    ...
    if (analytics.objectClassifications.empty()) { classificationsArray.append(); classificationsArray.shrink(0); }
  ...
  DeviceInfo DeviceInfo::fromJSON(const JSON::Value & json) {
    ...
    if (json.isMember("streams")) {
      ...
      for (size_t i = 0; i < streamList.size(); i++) {
        ...
        if (streamObj.isMember("resolution")) stream.resolution = streamObj["resolution"].asString();
        if (streamObj.isMember("framerate")) stream.framerate = streamObj["framerate"].asString();
        ...
        if (streamObj.isMember("options") && streamObj["options"].isObject()) {
          jsonForEachConst(streamObj["options"], option) { stream.options[option.key()] = option->asString(); }
        }
    ...
    if (json.isMember("protocols")) {
      ...
      for (size_t i = 0; i < protocolsList.size(); i++) {
        ...
        if (protoObj.isMember("endpointId")) config.endpointId = protoObj["endpointId"].asString();
        ...
        if (protoObj.isMember("scheme")) config.scheme = protoObj["scheme"].asString();
        if (protoObj.isMember("transport")) config.transport = protoObj["transport"].asString();
        if (protoObj.isMember("framing")) config.framing = protoObj["framing"].asString();
        if (protoObj.isMember("path")) config.path = protoObj["path"].asString();
        ...
        if (protoObj.isMember("tlsPolicy")) config.tlsPolicy = protoObj["tlsPolicy"].asString();
        if (protoObj.isMember("tlsCaFile")) config.tlsCaFile = protoObj["tlsCaFile"].asString();
        if (protoObj.isMember("tlsStatus")) config.tlsStatus = protoObj["tlsStatus"].asString();
        if (protoObj.isMember("lastError")) config.lastError = protoObj["lastError"].asString();
        if (protoObj.isMember("associationSource")) {
          config.associationSource = protoObj["associationSource"].asString();
        }
        if (protoObj.isMember("alternateEndpoints")) {
          const JSON::Value & endpoints = protoObj["alternateEndpoints"];
          for (size_t j = 0; j < endpoints.size(); ++j) {
            config.alternateEndpoints.push_back(endpoints[j].asString());
          }
        }
        ...
        if (protoObj.isMember("capabilities")) {
          ...
          if (capsObj.isMember("supportedResolutions")) {
            const JSON::Value & values = capsObj["supportedResolutions"];
            for (size_t j = 0; j < values.size(); ++j) {
              config.capabilities.supportedResolutions.push_back(values[j].asString());
            }
          }
          if (capsObj.isMember("supportedFramerates")) {
            const JSON::Value & values = capsObj["supportedFramerates"];
            for (size_t j = 0; j < values.size(); ++j) {
              config.capabilities.supportedFramerates.push_back(values[j].asString());
            }
          }
          if (capsObj.isMember("supportedAudioFormats")) {
            const JSON::Value & values = capsObj["supportedAudioFormats"];
            for (size_t j = 0; j < values.size(); ++j) {
              config.capabilities.supportedAudioFormats.push_back(values[j].asString());
            }
          }
        }
    ...
    if (json.isMember("analytics")) {
      ...
      if (aObj.isMember("analyticsServiceUrl")) info.analytics.analyticsServiceUrl = aObj["analyticsServiceUrl"].asString();
