...
namespace ONVIF {
  ...
  // getting UTC time in ISO8601 format
  ...
  // Mask the text content of any Password element so SOAP bodies can be logged without leaking
  // authentication material: both the cleartext ONVIF user password (<tt:Password>secret</...>
  // from CreateUsers/SetUser) and the WS-Security digest (<wsse:Password Type="...">digest</...>,
  // still auth material). Handles arbitrary namespace prefixes and attributes.
  std::string redactPasswords(const std::string & xml) {
    std::string out = xml;
    size_t pos = 0;
    while ((pos = out.find("Password", pos)) != std::string::npos) {
      size_t nameEnd = pos + 8; // past "Password"
      char after = nameEnd < out.size() ? out[nameEnd] : '\0';
      size_t lt = out.rfind('<', pos);
      // Must be an element name (next char ends the name) on an opening tag (not "</...Password>"
      // and not an attribute value like Type="...PasswordDigest").
      bool isName = (after == '>' || after == ' ' || after == '\t' || after == '\r' || after == '\n' || after == '/');
      bool opening = (lt != std::string::npos && !(lt + 1 < out.size() && out[lt + 1] == '/'));
      if (!isName || !opening) {
        pos = nameEnd;
        continue;
      }
      size_t gt = out.find('>', nameEnd); // end of the opening tag
      if (gt == std::string::npos) break;
      if (out[gt - 1] == '/') { // self-closing <...Password/>
        pos = gt + 1;
        continue;
      }
      size_t contentEnd = out.find('<', gt + 1);
      if (contentEnd != std::string::npos && contentEnd > gt + 1) {
        out.replace(gt + 1, contentEnd - (gt + 1), "***");
      }
      pos = gt + 1;
    }
    return out;
  ...
  // Clamp a float into [lo, hi]; also normalizes NaN to lo.
  ...
  static std::string urlHost(const std::string &host) {
    return host.find(':') == std::string::npos ? host : "[" + host + "]";
  }

  // ONVIF Device Implementation
  Device::Device(const std::string & host, uint16_t port, const std::string & path, const std::string & scheme)
    : ::Device::Base(), host(host), path(path.empty() ? "/onvif/device_service" : path), port(port),
      scheme(scheme.empty() ? "http" : scheme),
      uri((scheme.empty() ? "http" : scheme) + "://" + urlHost(host) + ":" + std::to_string(port) +
          (path.empty() ? "/onvif/device_service" : path)), connected(false),
      messageFactory(SOAP::Version::SOAP_1_2) {
    HIGH_MSG("Creating ONVIF device at %s://%s:%d%s", this->scheme.c_str(), host.c_str(), port, path.c_str());
  }
    ...
    try {
      ...
      return extractResponse<DeviceInfo>(response, "GetDeviceInformationResponse", [](const XML::Document & doc) -> Result<DeviceInfo> {
        try {
          ...
          // safely get text content and log it
  ...
  Result<bool> Device::absoluteMove(const std::string & profileToken, float pan, float tilt, float zoom) {
    ...
    // ONVIF normalized space: pan/tilt in [-1,1], zoom in [0,1]. Reject out-of-range/NaN.
    pan = clampf(pan, -1.0f, 1.0f);
    tilt = clampf(tilt, -1.0f, 1.0f);
    zoom = clampf(zoom, 0.0f, 1.0f);
  ...
  Result<bool> Device::relativeMove(const std::string & profileToken, float pan, float tilt, float zoom) {
    if (!connected) { return Result<bool>{false, Error{Error::Type::Transport, "Not connected"}}; }
    // Relative translations are normalized to [-1,1] in each axis. Reject out-of-range/NaN.
    pan = clampf(pan, -1.0f, 1.0f);
    tilt = clampf(tilt, -1.0f, 1.0f);
    zoom = clampf(zoom, -1.0f, 1.0f);
  ...
  Result<bool> Device::continuousMove(const std::string & profileToken, float pan, float tilt, float zoom) {
    if (!connected) { return Result<bool>{false, Error{Error::Type::Transport, "Not connected"}}; }
    // Continuous velocities are normalized to [-1,1] in each axis. Reject out-of-range/NaN.
    pan = clampf(pan, -1.0f, 1.0f);
    tilt = clampf(tilt, -1.0f, 1.0f);
    zoom = clampf(zoom, -1.0f, 1.0f);
    ...
    try {
      ...
      if (!endpointOverride.empty()) {
        ...
        if (it != services.end()) {
          ...
          serviceUrl = scheme + "://" + urlHost(host) + ":" + std::to_string(port) + path;
      ...
      std::istringstream requestStream(redactPasswords(request));
      std::string line;
      ...
      // Honor the device's configured transport: if this device is https but a discovered service
      // URL (XAddr) is http, upgrade it rather than silently downgrading credentials/PTZ to cleartext.
      if (scheme == "https" && url.protocol != "https") {
        if (url.port.empty() || url.port == "80") { url.port = "443"; }
        url.protocol = "https";
      }

      // Requests are often built before GetCapabilities supplies service-specific XAddrs. Keep
      // WS-Addressing To synchronized with the actual HTTP endpoint selected above.
      std::string wireRequest = request;
      try {
        XML::Document requestDoc(request);
        XML::Node to = requestDoc.evaluateXPath("//*[local-name()='Header']/*[local-name()='To']");
        if (to.isValid()) {
          to.setTextContent(url.getUrl());
          wireRequest = requestDoc.toString(false);
        }
      } catch (const std::exception &e) {
        return Result<std::string>{false, Error{Error::Type::Validation,
          std::string("Failed to update WS-Addressing endpoint: ") + e.what()}};
      }

      Socket::Connection conn;
      // Use TLS when the (possibly upgraded) service URL is https so credentials are actually
      // encrypted (arg 3 is nonblock, arg 4 is with_ssl, arg 5 the SNI/verification hostname).
      const bool useTls = (url.protocol == "https");
      const bool verifyTls = useTls && tlsPolicy != "insecure";
      conn.open(url.host, url.getPort(), false, useTls, url.host, verifyTls, tlsCaFile);
      if (!conn && useTls && tlsPolicy == "opportunistic") {
        const std::string verifyError = conn.getError();
        if (verifyError.find("TLS certificate validation failed") != std::string::npos ||
            verifyError.find("TLS CA load failed") != std::string::npos) {
          WARN_MSG("ONVIF: TLS certificate validation failed for %s; retrying encrypted TLS without verification",
                   url.host.c_str());
          tlsStatus = "encrypted-unverified";
          conn.open(url.host, url.getPort(), false, true, url.host, false, "");
        }
      }
      if (!conn) {
      ...
      if (!useTls) tlsStatus = "plaintext";
      else if (tlsStatus != "encrypted-unverified") tlsStatus = verifyTls ? "verified" : "encrypted-unverified";

      // Build HTTP request using Parser
      HTTP::Parser httpRequest;
      httpRequest.method = "POST";
      httpRequest.url = "/" + url.getEncodedPath();
      if (!url.args.empty()) httpRequest.url += "?" + url.args;
      httpRequest.protocol = "HTTP/1.1";
      httpRequest.SetHeader("Host", urlHost(url.host) + ":" + std::to_string(url.getPort()));
      httpRequest.SetHeader("Content-Type", "application/soap+xml; charset=utf-8; action=\"" + action + "\"");
      httpRequest.SetHeader("Content-Length", std::to_string(wireRequest.length()));
      // HTTP Basic transmits credentials as reversible base64, so only ever send it over TLS
      // (RFC 7617). Over plaintext we rely on the WS-Security digest already in the SOAP header.
      if (!username.empty() && !password.empty()) {
        if (useTls) {
          ...
          VERYHIGH_MSG("ONVIF: not sending HTTP Basic over plaintext http; relying on WS-Security digest");
        }
      }
      httpRequest.SetHeader("Connection", "close");
      httpRequest.SetBody(wireRequest);
      ...
      if (!handled) {
        if (handled.error.type == Error::Type::Authentication && useDigestAuth) {
          ...
          // Retry once with digest disabled (same transport as the primary request)
          Socket::Connection conn2;
          const bool retryVerify = useTls && tlsStatus != "encrypted-unverified" && tlsPolicy != "insecure";
          conn2.open(url.host, url.getPort(), false, useTls, url.host, retryVerify, tlsCaFile);
          if (conn2) {
            HTTP::Parser retry;
            retry.method = "POST";
            retry.url = "/" + url.getEncodedPath();
            if (!url.args.empty()) retry.url += "?" + url.args;
            retry.protocol = "HTTP/1.1";
            retry.SetHeader("Host", urlHost(url.host) + ":" + std::to_string(url.getPort()));
            retry.SetHeader("Content-Type", "application/soap+xml; charset=utf-8; action=\"" + action + "\"");
            retry.SetHeader("Content-Length", std::to_string(wireRequest.length()));
            // Only send HTTP Basic over TLS (RFC 7617); over plaintext this retry has no usable
            // credential, which is the intended secure-by-default behavior.
            if (!username.empty() && !password.empty() && useTls) {
              std::string auth = username + ":" + password;
              std::string encoded = Encodings::Base64::encode(auth);
              retry.SetHeader("Authorization", "Basic " + encoded);
            }
            retry.SetHeader("Connection", "close");
            retry.SetBody(wireRequest);
  ...
  Result<void> Device::sendRequestVoid(const std::string & service, const std::string & action, const std::string & request,
  ...
  Result<bool> Device::createUser(const User & user) {
    if (!connected) { return Result<bool>{false, Error{Error::Type::Transport, "Not connected"}}; }
    if (scheme != "https") {
      WARN_MSG("ONVIF: creating user '%s' over plaintext http - the password is transmitted in cleartext. "
               "Configure this ONVIF endpoint with scheme 'https' to protect credentials.",
               user.username.c_str());
    }
  ...
  Result<bool> Device::setUser(const User & user) {
    if (!connected) { return Result<bool>{false, Error{Error::Type::Transport, "Not connected"}}; }
    if (scheme != "https") {
      WARN_MSG("ONVIF: updating user '%s' over plaintext http - the password is transmitted in cleartext. "
               "Configure this ONVIF endpoint with scheme 'https' to protect credentials.",
               user.username.c_str());
    }
  ...
  bool Discovery::sendCommand(const std::string & deviceId, const ::Device::PTZCommand & cmd) {
    static const std::string prefix = "onvif:";
    if (deviceId.compare(0, prefix.size(), prefix) != 0) return false;
    HTTP::URL endpoint(deviceId.substr(prefix.size()));
    if (endpoint.host.empty() || (endpoint.protocol != "http" && endpoint.protocol != "https")) return false;
    std::string endpointPath = "/" + endpoint.getEncodedPath();
    if (!endpoint.args.empty()) endpointPath += "?" + endpoint.args;

    // Create temporary device and send command
    Device device(endpoint.host, endpoint.getPort(), endpointPath, endpoint.protocol);
  ...
  std::unique_ptr<::Device::Base> Discovery::createDevice(const ::Device::DeviceInfo & info) const {
    ...
    if (protoIt != info.protocols.end()) {
      ...
      if (!protoIt->second.scheme.empty()) scheme = protoIt->second.scheme;
    }
    // Default to the scheme's well-known port when none was explicitly configured.
    if (!port) { port = (scheme == "https") ? 443 : 80; }
    const std::string path = protoIt != info.protocols.end() && !protoIt->second.path.empty()
                               ? protoIt->second.path
                               : "/onvif/device_service";
    auto dev = std::unique_ptr<Device>(new Device(host, port, path, scheme));
    if (protoIt != info.protocols.end()) dev->setTLSConfig(protoIt->second.tlsPolicy, protoIt->second.tlsCaFile);
  ...
  bool Discovery::parseProbeMatch(const std::string & response, ::Device::DeviceInfo & info) {
    try {
      // Accept any SOAP envelope regardless of XML declaration or namespace prefix. WS-Discovery
      // replies are SOAP-over-UDP and frequently omit the <?xml?> declaration and use varied
      // prefixes (s:, soap:, SOAP-ENV:), so gate on the Envelope element itself.
      if (response.find("Envelope") == std::string::npos) {
        WARN_MSG("ONVIF: Response does not contain a SOAP Envelope");
        return false;
      }
      ...
      for (const auto & ns : namespaces) {
        ...
        // Register the discovered WS-Discovery namespace under the prefix used by our XPath helpers.
      ...
      std::string endpointReference = doc.evaluateXPathString(
        "string(//*[local-name()='ProbeMatch']/*[local-name()='EndpointReference']/*[local-name()='Address'])");
      ...
      types = trim(types);
      scopes = trim(scopes);
      endpointReference = trim(endpointReference);
      ...
      if (!xaddrs.empty()) {
        ...
        for (const auto & address : addresses) {
          ...
          onvifConfig.address = url.host;
          onvifConfig.port = url.getPort();
          onvifConfig.scheme = url.protocol.empty() ? "http" : url.protocol;
          onvifConfig.path = "/" + url.getEncodedPath();
          if (!url.args.empty()) onvifConfig.path += "?" + url.args;
          if (onvifConfig.path == "/") onvifConfig.path = "/onvif/device_service";
          onvifConfig.alternateEndpoints.assign(addresses.begin(), addresses.end());
          if (!endpointReference.empty()) {
            std::string identity = endpointReference;
            const std::string uuidPrefix = "urn:uuid:";
            if (identity.substr(0, uuidPrefix.size()) == uuidPrefix) identity.erase(0, uuidPrefix.size());
            onvifConfig.endpointId = "onvif:" + identity;
          } else {
            onvifConfig.endpointId = "onvif:" + url.getUrl();
          }

          info.name = url.host;
          info.host = url.host;
          info.id = onvifConfig.endpointId;
    ...
    try {
      switch (cmd.action) {
        ...
        case ::Device::PTZAction::Preset:
          if (!cmd.presetToken.empty()) {
            if (cmd.storePreset) {
              return setPreset(defaultProfileToken, cmd.presetToken, "Preset " + cmd.presetToken);
            }
            return gotoPreset(defaultProfileToken, cmd.presetToken);
          }
          break;
        ...
        case ::Device::PTZAction::WhiteBalance: {
          ...
          if (cmd.args.count("color_temp")) {
            settings.whiteBalance.crGain = cmd.args.at("color_temp");
          }
          return setImagingSettings(vsToken, settings);
        }
        default:
          ERROR_MSG("ONVIF: Unsupported PTZ command: %d", static_cast<int>(cmd.action));
          return false;
      }
  ...
  ::Device::AnalyticsCapabilities Device::queryAnalytics(const std::string & profileToken) const {
    ...
    try {
      auto request = buildRequest("tan:GetSupportedAnalyticsModules",
        ...
        if (result) {
          auto parseResult = extractResponse<std::vector<::Device::AnalyticsModule>>(
            ...
            [](const XML::Document & doc) -> Result<std::vector<::Device::AnalyticsModule>> {
              ...
              auto nodes = doc.evaluateXPathAll("//*[local-name()='AnalyticsModule' or local-name()='AnalyticsModuleDescription']");
    ...
    // Get active rules
    try {
      auto request = buildRequest("tan:GetRules",
        [&](XML::Document & doc, XML::Node & root) {
          auto tokenNode = doc.createNode("tan:ConfigurationToken");
          tokenNode.setTextContent(configToken);
          root.addChild(tokenNode);
      ...
      if (request) {
        ...
        if (result) {
          auto parseResult = extractResponse<std::vector<::Device::AnalyticsRule>>(
            ...
            [](const XML::Document & doc) -> Result<std::vector<::Device::AnalyticsRule>> {
      ...
      MEDIUM_MSG("ONVIF: Failed to query analytics rules: %s", e.what());
    }
  ...
  Result<ImagingSettings> Device::getImagingSettings(const std::string & videoSourceToken) const {
    ...
    return extractResponse<ImagingSettings>(response, "GetImagingSettingsResponse",
      [](const XML::Document & doc) -> Result<ImagingSettings> {
        ...
        if (!node.isValid()) return Result<ImagingSettings>{false, Error{Error::Type::Validation, "No ImagingSettings node"}};
  ...
  Result<std::vector<ONVIFEvent>> Device::pullMessages(const std::string & subscriptionEndpoint,
                                                        int timeoutSec, int messageLimit) {
  ...
  ::Device::DeviceInfo Device::queryCapabilities() const {
    ...
    try {
      ...
      onvifConfig.endpointId = "onvif:" + uri;
      onvifConfig.scheme = scheme;
      onvifConfig.transport = "tcp";
      onvifConfig.framing = "soap";
      onvifConfig.path = path;
      onvifConfig.username = username;
      onvifConfig.password = password;
      onvifConfig.tlsPolicy = tlsPolicy;
      onvifConfig.tlsCaFile = tlsCaFile;
      onvifConfig.tlsStatus = tlsStatus;
      ...
      if (profilesResult) {
        ...
        if (!profiles.empty()) {
          ...
          if (caps.analytics && services.count("analytics") && !services.at("analytics").empty()) {
            ...
            HIGH_MSG("ONVIF: Analytics: hasAnalytics=%d, modules=%zu, rules=%zu",
                     info.analytics.hasAnalytics,
                     info.analytics.supportedModules.size(),
                     info.analytics.activeRules.size());
          }
        }
      } else {
  ...
  SOAP::Message Device::createMessage(const std::string & action, SOAP::onvif::ServiceType service) const {
    auto msg = messageFactory.createMessage(service, action,
                                            scheme + "://" + urlHost(host) + ":" + std::to_string(port) + path);
  ...
  bool Discovery::startAsyncDiscovery(::Device::DiscoveryCallback callback, uint32_t timeoutMs) {
    ...
    if (!setupAsyncSocket()) return false;
    return true;
  }
  ...
  bool Discovery::setupAsyncSocket() {
    try {
  ...
  void Discovery::processSocketData() {
    ...
    try {
      while (socket->Receive()) {
        ...
        const std::string packet(dataPtr, respSize);

        ::Device::DeviceInfo info;
        if (parseProbeMatch(packet, info)) {
          bool found = false;
          for (const auto & dev : asyncDevices) {
            auto existing = dev.protocols.find("onvif");
            auto incoming = info.protocols.find("onvif");
            if (existing != dev.protocols.end() && incoming != info.protocols.end() &&
                existing->second.endpointId == incoming->second.endpointId) {
              found = true;
              break;
            }
          }
