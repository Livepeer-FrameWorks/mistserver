...
namespace XML {
  ...
  class Node {
    public:
      ...
      /**
       * @brief Re-point this node's owning-document back-pointer.
       * @param doc New owner document
       * @note The underlying xmlNodePtr is unaffected. Use after a Document has been moved: the
       *       libxml2 tree survives the move, but cached Node handles still reference the old
       *       Document object, so their owner must be re-pointed to avoid a dangling access.
       */
      void rebindOwner(Document *doc) { ownerDoc = doc; }
