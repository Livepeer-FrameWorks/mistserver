...
namespace h264{
  ...
  static void skipScalingList(Utils::bitstream & bs, size_t listSize) {
    size_t lastScale = 8;
    size_t nextScale = 8;
  ...
  sequenceParameterSet::sequenceParameterSet(const char *ptr, size_t len) {
    valid = false;
    chars.sep_col_plane = false;
    ...
    for (size_t i = 1; i < len; i++) {
      if (i + 2 < len && (memcmp(ptr + i, "\000\000\003", 3) == 0)) { // Emulation prevention bytes
    ...
    if (bs.size() < 24) { return; } // static size data
    chars.profile = bs.get(8);
    bs.skip(8); // constraints
    chars.level = bs.get(8);
    bs.getUExpGolomb(); // ID
    if (chars.profile == 100 || chars.profile == 110 || chars.profile == 122 || chars.profile == 244 ||
        chars.profile == 44 || chars.profile == 83 || chars.profile == 86 || chars.profile == 118 || chars.profile == 128) {
      // chroma format idc
    ...
    chars.log2_max_frame_num = bs.getUExpGolomb() + 4;
    chars.cnt_type = bs.getUExpGolomb();
    if (!chars.cnt_type) {
      chars.log2_max_order_cnt = bs.getUExpGolomb() + 4;
    } else if (chars.cnt_type == 1) {
      bs.get(1); // bool delta_pic_order_always_zero
      bs.getExpGolomb(); // int64_t offset_for_non_ref_pic
      bs.getExpGolomb(); // int64_t offset_for_top_to_bottom_field
      int64_t num_ref_frames_in_pic_order_cnt_cycle = bs.getUExpGolomb();
      for (size_t i = 0; i < num_ref_frames_in_pic_order_cnt_cycle; ++i) { bs.getExpGolomb(); }
    }
    if (!bs.size()) { return; }
    chars.max_ref_frames = bs.getUExpGolomb(); // max_num_ref_frames
    chars.gaps = bs.get(1); // gaps in frame num allowed
    widthInMbs = bs.getUExpGolomb() + 1;
    heightInMapUnits = bs.getUExpGolomb() + 1;
    if (!bs.size()) { return; }
    chars.mbs_only = (bs.get(1) == 1); // Gets used in height calculation
    if (!chars.mbs_only) { bs.skip(1); }
    bs.skip(1);
    // cropping flag
    if (bs.get(1)) {
    ...
    chars.fps = 0;
    if (bs.get(1)) {
      // Skipping all the parameters we don't use
      if (bs.get(1)) {
      ...
      // Decode timing info
      if (bs.get(1)) {
        if (bs.size() < 65) { return; }
        uint32_t unitsInTick = bs.get(32);
        uint32_t timeScale = bs.get(32);
        chars.fps = (double)timeScale / (2 * unitsInTick);
        bs.skip(1);
      }
    }

    chars.width = (widthInMbs * 16) - (cropHorizontal * 2);
    chars.height = ((chars.mbs_only ? 1 : 2) * heightInMapUnits * 16) - (cropVertical * 2);

    if (sar_width != sar_height) {
      if (sar_width > sar_height) {
        chars.width = ((chars.width * sar_width) / sar_height);
      } else {
        chars.height = ((chars.height * sar_height) / sar_width);
      }
    }
    valid = true;
  }

  initData::initData(const char *ptr, size_t len) {
    // H264 init data is defined in ISO 14496-15
    // Unfortunately this spec is not free, but the data format is:
    //   uint8_t configurationVersion (must be set to 1)
    //   uint8_t AVCProfileIndication (profile code per H264 spec)
    //   uint8_t profile_compatibility (constraint bits, per H264 spec)
    //      100000 = Baseline profile constraints applied
    //      010000 = Main profile constraints applied
    //      001000 = Extended profile constraints applied
    //      000100 = High profile constraints applied
    //      000010 = Multiview high profile constraints applied
    //      000001 = Stereo high profile constraints applied
    //   uint8_t AVCLevelIndication (level code per H264 spec)
    //   6 reserved bits that should all be set to 1
    //   2 bits: NALUnitLength - 1 (only 1, 2 or 4 are allowed)
    //   3 reserved bits that should all be set to 1
    //   5 bits: SPS count, followed by SPS count times:
    //     uint16_t SPS length
    //     length bytes of SPS data
    //   uint8_t PPS count, followed by PPS count times:
    //     uint16_t PPS length
    //     length bytes of PPS data
    valid = false;
    if (len < 7) { // Min length assuming zero SPS and zero PPS
      return;
    }
    profile = ptr[1];
    constraints = ptr[2];
    level = ptr[3];
    NALULen = (ptr[4] & 0x3) + 1;
    size_t spsCount = (ptr[5] & 0x1F);
    size_t offset = 6;
    for (size_t i = 0; i < spsCount; ++i) {
      if (len < offset + 2) {
        INFO_MSG("H264 init error: SPS #%zu does not fit %zub into %zub", i, offset, len);
        return;
      }
      size_t spsLen = Bit::btohs(ptr + offset);
      if (len < offset + 2 + spsLen) {
        INFO_MSG("H264 init error: SPS #%zu length %zu does not fit %zub into %zub", i, spsLen, offset, len);
        return;
      }
      // Extract width/height/fps from the SPS
      sequenceParameterSet sps(ptr + offset + 2, spsLen);
      if (sps) {
        const SPSMeta & M = sps.chars;
        if (M.width * M.height >= width * height) {
          width = M.width;
          height = M.height;
        }
        if (M.fps > fps) { fps = M.fps; }
      }
      offset += 2 + spsLen;
    }
    if (len < offset + 1) {
      INFO_MSG("H264 init error: PPS count does not fit %zub into %zub", offset, len);
      return;
    }
    size_t ppsCount = ptr[offset];
    ++offset;
    for (size_t i = 0; i < ppsCount; ++i) {
      if (len < offset + 2) {
        INFO_MSG("H264 init error: PPS #%zu does not fit %zub into %zub", i, offset, len);
        return;
      }
      size_t ppsLen = Bit::btohs(ptr + offset);
      if (len < offset + 2 + ppsLen) {
        INFO_MSG("H264 init error: PPS #%zu length %zu does not fit %zub into %zub", i, ppsLen, offset, len);
        return;
      }
      // pps.push_back(sequenceParameterSet(ptr + offset + 2, ppsLen));
      offset += 2 + ppsLen;
    }
  ...
  std::string initFromAnnexB(const char *ptr, size_t len) {
    std::deque<std::string> sps;
    std::deque<std::string> pps;
    const char *nalStart = ptr;
    const char *end = ptr + len;
    const char *nextStart = nalu::scanAnnexB(ptr, len);
    if (!nextStart) {
      WARN_MSG("No start codes in Annex B H264 init data");
      return "";
    }

    while (nalStart < end) {
      if (!nextStart) { nextStart = end; }
      const char *nalEnd = nalu::nalEndPosition(nalStart, nextStart - nalStart);
      size_t nalSize = nalEnd - nalStart;
      if (nalSize) {
        uint8_t nalType = nalStart[0] & 0x1F;
        if (nalType == 7) { sps.push_back(std::string(nalStart, nalSize)); }
        if (nalType == 8) { pps.push_back(std::string(nalStart, nalSize)); }
      }
      if (nextStart == end || nextStart + 3 >= end) { break; }
      nalStart = nextStart + 3;
      nextStart = nalu::scanAnnexB(nalStart, end - nalStart);
    }

    bool oversized = false;
    for (const std::string &unit : sps) { oversized |= unit.size() > 0xFFFF; }
    for (const std::string &unit : pps) { oversized |= unit.size() > 0xFFFF; }
    if (sps.empty() || pps.empty() || sps.size() > 31 || pps.size() > 255 || sps.front().size() < 4 || oversized) {
      WARN_MSG("Annex B H264 init data contains %zu SPS and %zu PPS; invalid", sps.size(), pps.size());
      return "";
    }
    ...
    result += sps.front()[3];
    result += (char)0xFF;
    result += (char)(0xE0 + sps.size());
    for (const std::string &unit : sps) {
      result += (char)(unit.size() >> 8);
      result += (char)(unit.size() & 0xFF);
      result += unit;
    }
    result += (char)pps.size();
    for (const std::string &unit : pps) {
      result += (char)(unit.size() >> 8);
      result += (char)(unit.size() & 0xFF);
      result += unit;
    }
