 ...
 *
 * The NDI runtime is loaded at runtime via dlopen() and the SDK loader table (no link-time
...
namespace NDI {
   ...
   * @return Pointer to the loaded NDI function table, or nullptr if the NDI runtime is unavailable
  ...
  class Device : public ::Device::Base {
    private:
      ...
      // Scratch buffers for audio format conversion (reused across frames; ResizeablePointer keeps
      // capacity so steady-state frames never reallocate).
      Util::ResizeablePointer audioScratch16;    // host-native interleaved 16-bit
      Util::ResizeablePointer audioScratchFloat; // planar 32-bit float (FLTP)
      Util::ResizeablePointer audioScratchBE;    // Mist interleaved big-endian 16-bit PCM
      ...
      // Format/capability conversion utilities
    ...
    public:
      ...
      /**
       * @brief Convert Mist interleaved big-endian 16-bit PCM to NDI planar float and send it.
       * @param pcmBE Interleaved big-endian signed 16-bit PCM samples
       * @param bytes Size of pcmBE in bytes
       * @param sampleRate Sample rate in Hz
       * @param channels Channel count
       * @note Uses the SDK's NDIlib_util_audio_from_interleaved_16s_v3 (correct dB reference-level
       *       scaling) after swapping Mist's big-endian samples to host-native order.
       */
      bool sendAudioPCM16BE(const uint8_t *pcmBE, size_t bytes, int sampleRate, int channels);

      /**
       * @brief Convert a received NDI (planar float) audio frame to Mist interleaved big-endian
       *        16-bit PCM.
       * @param frame Received NDI audio frame (FLTP)
       * @param[out] outBytes Number of PCM bytes produced
       * @return Pointer to an internal buffer holding the PCM (valid until the next call), or
       *         nullptr on failure.
       * @note Uses the SDK's NDIlib_util_audio_to_interleaved_16s_v3 (correct dB reference-level
       *       scaling) then swaps host-native samples to Mist's big-endian order.
       */
      const uint8_t *audioFrameToPCM16BE(const NDIlib_audio_frame_v3_t & frame, size_t & outBytes);
       ...
       * @param value Focus value (0.0 to 1.0)
       * @note Uses NDIlib_recv_ptz_focus
       */
      bool ptzFocus(float value);

      /**
       * @brief Enable auto focus
       * @note Uses NDIlib_recv_ptz_auto_focus
       */
      bool ptzAutoFocus();

      /**
       * @brief Store the current PTZ position as a preset
       * @param preset Preset number (0-based)
       * @note Uses NDIlib_recv_ptz_store_preset
       */
      bool ptzStorePreset(int preset);

      /**
       * @brief Recall a stored PTZ preset
       * @param preset Preset number (0-based)
       * @param speed Movement speed towards the preset (0.0 to 1.0)
       * @note Uses NDIlib_recv_ptz_recall_preset
       */
      bool ptzRecallPreset(int preset, float speed);
  ...
  class Discovery : public ::Device::Discovery {
    public:
       ...
       * @param target The exact NDI source name
    ...
    public:
      /**
       * @brief Register a callback invoked with sources that have disappeared since the last poll.
       * @note NDI sources are ephemeral; the controller uses this to drop stale NDI-only cameras.
       */
      void setRemovalCallback(::Device::DiscoveryCallback cb) { removalCallback = cb; }
    ...
    private:
      ...
      ::Device::DiscoveryCallback removalCallback;
