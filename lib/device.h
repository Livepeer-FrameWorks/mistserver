...
namespace Device {
  ...
  struct ProtocolConfig {
      ...
      std::string framing;
      std::string path;
      std::vector<std::string> alternateEndpoints;
      std::string username;
      std::string password;
      // HTTPS verification policy: "opportunistic" (default), "required", or "insecure".
      std::string tlsPolicy = "opportunistic";
      std::string tlsCaFile;
      // Runtime diagnostics. These are exposed through the API but are not credentials.
      std::string tlsStatus;
      std::string lastError;
      // How this endpoint became part of its logical camera: "manual" or "inferred".
      std::string associationSource;
  ...
  struct PTZCommand {
      ...
      std::string presetToken; // Opaque protocol-specific preset token
      bool storePreset = false;
  };
