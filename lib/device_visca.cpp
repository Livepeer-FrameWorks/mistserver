 ...
 *
 * Transport Modes:
 * - Raw UDP: VISCA payloads on port 1259
...
namespace VISCA {
  ...
  namespace {
    std::string makeEndpointId(const std::string & address, uint16_t port) {
      return "visca:" + (address.find(':') == std::string::npos ? address : "[" + address + "]") + ":" +
             std::to_string(port);
    }

    bool parseEndpointId(const std::string & endpointId, std::string & address, uint16_t & port) {
      static const std::string prefix = "visca:";
      if (endpointId.compare(0, prefix.size(), prefix) != 0) return false;
      const std::string endpoint = endpointId.substr(prefix.size());
      std::string portText;
      if (!endpoint.empty() && endpoint[0] == '[') {
        const size_t close = endpoint.find(']');
        if (close == std::string::npos || close + 1 >= endpoint.size() || endpoint[close + 1] != ':') return false;
      ...
      if (address.empty() || portText.empty()) return false;
      try {
        size_t consumed = 0;
        const unsigned long parsed = std::stoul(portText, &consumed, 10);
        if (consumed != portText.size() || parsed == 0 || parsed > 65535) return false;
  ...
  Device::Device(const std::string & addr, uint16_t p, ViscaTransport transport, ViscaFraming framing)
    : address(addr), port(p), transportType(transport), connected(false), connectionTimeout(1000), sequenceNum(0),
      activeSequence(0), framingType(framing) {
    ...
    if (port == 0) {
      port = framingType == ViscaFraming::OverIP ? 52381 : 1259;
  ...
  bool Device::connect() {
    ...
    if (!createSocket()) {
      WARN_MSG("VISCA: Failed to open configured %s endpoint %s:%d",
               transportType == ViscaTransport::TCP ? "TCP" : "UDP", address.c_str(), port);
      return false;
    }
    connected = true;
    if (!initialize()) {
      WARN_MSG("VISCA: Configured endpoint did not answer at %s:%d", address.c_str(), port);
      disconnect();
      return false;
    }
    return true;
  ...
  ::Device::DeviceInfo Device::queryCapabilities() const {
    ...
    // Basic device info
    info.id = makeEndpointId(address, port);
    info.host = address;
    ...
    info.ptzFeatures = {"pan_tilt", "zoom", "focus",    "iris",     "whitebalance", "preset",
                        "home",     "stop", "absolute", "relative", "continuous",   "exposure",
                        "gain",     "flip"};
    ...
    viscaConfig.transport = transportType == ViscaTransport::TCP ? "tcp" : "udp";
    viscaConfig.framing = framingType == ViscaFraming::OverIP ? "visca-ip" : "raw";
    viscaConfig.endpointId = info.id;
    ...
    viscaConfig.capabilities.supportedCommands = {"pan_tilt",      "zoom",          "focus",         "iris",
                                                  "whitebalance",  "exposure",      "gain",          "flip",
                                                  "preset_store",  "preset_recall", "preset_clear",
                                                  "home",          "stop",          "absolute_move", "direct_zoom"};
      ...
      switch (cmd.action) {
        ...
        case ::Device::PTZAction::Preset:
          if (!cmd.presetToken.empty()) {
            size_t consumed = 0;
            unsigned long preset = std::stoul(cmd.presetToken, &consumed, 10);
            if (consumed != cmd.presetToken.size() || preset > 127) {
              lastError = ErrorInfo(Error::InvalidCommand, "VISCA preset token must be a decimal value from 0 to 127");
              return false;
            }
            return cmd.storePreset ? setPreset(static_cast<uint8_t>(preset))
                                   : gotoPreset(static_cast<uint8_t>(preset));
          }
          break;
        ...
        case ::Device::PTZAction::Iris:
          if (cmd.args.find("value") != cmd.args.end()) {
            return setIris(static_cast<int>(cmd.args.at("value")));
          }
          break;
  ...
  bool Device::sendCommand(const uint8_t *cmd, size_t len, uint32_t timeoutMs) {
    const bool inquiry = len > 1 && cmd[1] == 0x09;
    return transact(cmd, len, inquiry, inquiry ? &lastResponse : nullptr, timeoutMs);
  }
  ...
  bool Device::createUDP() {
    ...
    udpConn = std::unique_ptr<Socket::UDPConnection>(new Socket::UDPConnection(false, family));
    udpConn->SetDestination(address, port);
    if (!udpConn->bind(0)) {
      lastError = ErrorInfo(Error::SocketError, "Failed to bind VISCA UDP socket");
      udpConn.reset();
      return false;
    }
    return true;
  ...
  bool Device::initialize() {
    ...
    std::string response;
    if (!transact(inquiry, sizeof(inquiry), true, &response, 2000)) {
  ...
  bool Device::sendPacket(const uint8_t *payload, size_t len) {
    if (!isConnected()) {
      lastError = ErrorInfo(Error::NotConnected, "Device not connected");
      return false;
    }
    ...
    if (framingType == ViscaFraming::OverIP) {
      packet.resize(len + 8);
      packet[0] = 0x01;
      packet[1] = 0x10;
      packet[2] = (len >> 8) & 0xFF;
      packet[3] = len & 0xFF;
      activeSequence = sequenceNum++;
      packet[4] = (activeSequence >> 24) & 0xFF;
      packet[5] = (activeSequence >> 16) & 0xFF;
      packet[6] = (activeSequence >> 8) & 0xFF;
      packet[7] = activeSequence & 0xFF;
  ...
  bool Device::extractFrame(std::string &response) {
    if (framingType == ViscaFraming::Raw) {
      size_t end = receiveBuffer.find(static_cast<char>(0xFF));
      if (end == std::string::npos) return false;
      response = receiveBuffer.substr(0, end + 1);
      receiveBuffer.erase(0, end + 1);
      return true;
    }
    if (receiveBuffer.size() < 8) return false;
    const uint8_t *p = reinterpret_cast<const uint8_t *>(receiveBuffer.data());
    const size_t payloadLen = (static_cast<size_t>(p[2]) << 8) | p[3];
    if (payloadLen > 4096) {
      receiveBuffer.clear();
      lastError = ErrorInfo(Error::InvalidResponse, "VISCA-over-IP payload is too large");
      return false;
    }
    if (receiveBuffer.size() < payloadLen + 8) return false;
    const std::string packet = receiveBuffer.substr(0, payloadLen + 8);
    receiveBuffer.erase(0, payloadLen + 8);
    return decodeOverIpPacket(packet, activeSequence, response);
  }

  bool Device::receiveFrame(std::string &response, uint32_t timeoutMs) {
    if (!isConnected()) {
      lastError = ErrorInfo(Error::NotConnected, "Device not connected");
      return false;
    }
    ...
    while (true) {
      if (extractFrame(response)) return validateResponse(response);
      bool hasData = false;
      ...
      if (hasData) {
        if (transportType == ViscaTransport::TCP) {
          ...
          if (buffer.size()) receiveBuffer.append(buffer.remove(buffer.size()));
        } else if (transportType == ViscaTransport::UDP) {
          receiveBuffer.assign(reinterpret_cast<const char *>((const void *)udpConn->data), udpConn->data.size());
        }
        if (extractFrame(response)) return validateResponse(response);
      }
  ...
  bool Device::transact(const uint8_t *cmd, size_t len, bool inquiry, std::string *response, uint32_t timeoutMs) {
    if (!isConnected()) {
      lastError = ErrorInfo(Error::NotConnected, "Device not connected");
      return false;
    }
    try {
      lastCommand.assign(reinterpret_cast<const char *>(cmd), len);
      lastResponse.clear();
      receiveBuffer.clear();
      if (!sendPacket(cmd, len)) return false;
      ...
      while (true) {
        ...
        if (elapsed >= timeoutMs) {
          lastError = ErrorInfo(Error::Timeout, acknowledged ? "VISCA completion timeout" : "VISCA response timeout");
          return false;
        }
        std::string frame;
        if (!receiveFrame(frame, static_cast<uint32_t>(timeoutMs - elapsed))) return false;
        const uint8_t type = static_cast<uint8_t>(frame[1]);
        if ((type & 0xF0) == 0x60) {
          char errorType[64];
          snprintf(errorType, sizeof(errorType), "VISCA device returned error 0x%02X", type);
          lastError = ErrorInfo(Error::InvalidResponse, errorType);
          return false;
        }
        if ((type & 0xF0) == 0x40) {
          acknowledged = true;
  ...
  bool Device::getStatus(int & pan, int & tilt, int & zoom) {
    ...
    std::string response;
    if (!transact(cmd, sizeof(cmd), true, &response, 1000)) { return false; }
  ...
  bool Device::absoluteMove(int pan, int tilt, uint8_t speed) {
    ...
    uint8_t cmd[] = {0x81, 0x01, 0x06, 0x02, speed, tiltSpeed,
                     ...
                     static_cast<uint8_t>(tilt & 0x0F), 0xFF};
    return sendCommand(cmd, sizeof(cmd));
  }

  bool Device::directZoom(uint16_t position) {
    // VISCA zoom positions run 0x0000 (wide) to 0x7AC0 (max tele incl. digital); reject anything
    // beyond that rather than packing a bogus value into the command nibbles.
    if (position > 0x7AC0) {
      WARN_MSG("VISCA: directZoom position 0x%04X out of range, clamping to 0x7AC0", position);
      position = 0x7AC0;
    }
    // VISCA direct zoom: 81 01 04 47 0p 0q 0r 0s FF
    uint8_t cmd[] = {0x81, 0x01, 0x04, 0x47,
                     static_cast<uint8_t>((position >> 12) & 0x0F),
                     static_cast<uint8_t>((position >> 8) & 0x0F),
                     static_cast<uint8_t>((position >> 4) & 0x0F),
                     static_cast<uint8_t>(position & 0x0F), 0xFF};
    return sendCommand(cmd, sizeof(cmd));
  }
  ...
  bool Device::getZoomPosition(uint16_t & position) {
    ...
    if (response.length() >= 7 && (uint8_t)response[1] == 0x50) {
      position = ((uint8_t)response[2] & 0x0F) << 12 |
                 ((uint8_t)response[3] & 0x0F) << 8 |
                 ((uint8_t)response[4] & 0x0F) << 4 |
                 ((uint8_t)response[5] & 0x0F);
      return true;
    }
  ...
  bool Device::setAddress(const std::string & newAddress, uint16_t newPort) {
    ...
    if (!port) port = framingType == ViscaFraming::OverIP ? 52381 : 1259;
  ...
  bool Discovery::sendCommand(const std::string & deviceId, const ::Device::PTZCommand & cmd) {
    ...
    if (it != connectedDevices.end() && it->second && it->second->isConnected()) {
      return it->second->sendPTZ(cmd);
    }

    std::string address;
    uint16_t port = 0;
    if (!parseEndpointId(deviceId, address, port)) return false;
  ...
  bool Discovery::sendCommand(const ::Device::DeviceInfo &info, const ::Device::PTZCommand &cmd) {
    auto pit = info.protocols.find("visca");
    if (pit == info.protocols.end()) return false;
    const std::string key = pit->second.endpointId;
    auto cached = connectedDevices.find(key);
    if (cached != connectedDevices.end() && cached->second && cached->second->isConnected()) {
      return cached->second->sendPTZ(cmd);
    }
    std::unique_ptr<::Device::Base> base = createDevice(info);
    Device *device = dynamic_cast<Device *>(base.release());
    if (!device) return false;
    std::unique_ptr<Device> owned(device);
    if (!owned->connect()) return false;
    bool result = owned->sendPTZ(cmd);
    connectedDevices[key] = std::move(owned);
    return result;
  }
  ...
  bool Discovery::setTally(const ::Device::DeviceInfo &info, bool on) {
    auto pit = info.protocols.find("visca");
    if (pit == info.protocols.end()) return false;
    const std::string key = pit->second.endpointId;
    auto cached = connectedDevices.find(key);
    if (cached != connectedDevices.end() && cached->second && cached->second->isConnected()) {
      return cached->second->setBacklight(on);
    }
    std::unique_ptr<::Device::Base> base = createDevice(info);
    Device *device = dynamic_cast<Device *>(base.release());
    if (!device) return false;
    std::unique_ptr<Device> owned(device);
    if (!owned->connect()) return false;
    bool result = owned->setBacklight(on);
    connectedDevices[key] = std::move(owned);
    return result;
  }
  ...
  std::unique_ptr<::Device::Base> Discovery::createDevice(const ::Device::DeviceInfo & info) const {
    ...
    uint16_t port = 52381;
    ViscaTransport transport = ViscaTransport::UDP;
    ViscaFraming framing = ViscaFraming::OverIP;
    ...
    if (protoIt != info.protocols.end()) {
      ...
      if (protoIt->second.transport == "tcp") transport = ViscaTransport::TCP;
      if (protoIt->second.framing == "raw") framing = ViscaFraming::Raw;
    }
    return std::unique_ptr<Device>(new Device(address, port, transport, framing));
  }
  ...
  bool Discovery::parseDiscoveryResponse(const std::string & response, ::Device::DeviceInfo & info) {
    ...
    if ((responseType & 0xF0) == 0x60) return false;
    ...
    const uint16_t responsePort = info.protocols.count("visca") && info.protocols["visca"].port
                                    ? info.protocols["visca"].port
                                    : static_cast<uint16_t>(isViscaOverIP ? 52381 : 1259);
    info.id = makeEndpointId(info.host, responsePort);
    ...
    viscaConfig.port = responsePort;
    viscaConfig.transport = "udp";
    viscaConfig.framing = isViscaOverIP ? "visca-ip" : "raw";
    viscaConfig.endpointId = info.id;
    viscaConfig.capabilities.hasPTZ = true;
    viscaConfig.capabilities.hasAudio = false;
    viscaConfig.capabilities.hasMetadata = false;
  ...
  bool Discovery::setupAsyncSocket() {
    try {
      ...
      // Raw VISCA and VISCA-over-IP ports
      std::vector<uint16_t> ports = {52381, 1259};

      // VISCA IF_Clear command (over-IP discovery packet header) and raw inquiry
      const uint8_t rawInquiry[] = {0x81, 0x09, 0x04, 0x00, 0xFF};
      ...
      auto sendToBroadcast = [&](const std::string & bcast) {
        for (uint16_t port : ports) {
          ...
          if (port == 52381) {
            ...
            discoverySocket->SendNow(reinterpret_cast<const char *>(rawInquiry), sizeof(rawInquiry));
          }
  ...
  void Discovery::processSocketData() {
    ...
    try {
      while (discoverySocket->Receive()) {
        ...
        const uint16_t senderPort = remoteAddr.port();

        size_t respSize = discoverySocket->data.size();
        if (respSize == 0) continue;

        const std::string packet(reinterpret_cast<const char *>((const void *)discoverySocket->data), respSize);
        if (packet.size() < 3) continue;

        ::Device::DeviceInfo info;
        // Set host/id before parse so parser can rely on it
        info.host = sender;
        info.protocols["visca"].port = senderPort;
        if (parseDiscoveryResponse(packet, info)) {
          bool found = false;
          for (const auto & dev : asyncDevices) {
            auto existing = dev.protocols.find("visca");
            auto incoming = info.protocols.find("visca");
            if (existing != dev.protocols.end() && incoming != info.protocols.end() &&
                existing->second.address == incoming->second.address &&
                existing->second.port == incoming->second.port &&
                existing->second.framing == incoming->second.framing) {
              found = true;
              break;
            }
          }
