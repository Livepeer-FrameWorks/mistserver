...
namespace Foghorn {
  ...
  bool readUint16(const char *data, size_t size, size_t offset, uint16_t & value) {
    if (!data || offset + 2 > size) { return false; }
    value = (((uint8_t)data[offset]) << 8) | (uint8_t)data[offset + 1];
    return true;
  }
  ...
  bool makePunchRequest(std::string & data, const std::string & protocol, const std::string & path,
    ...
    if (protocol.size() > 0xFF || path.size() > 0xFFFF || additionalData.size() > 0xFFFF) { return false; }
    data.assign("FOGH0123456789ABCDEF\002", 21);
    data += (char)protocol.size();
    data += protocol;
    appendUint16(data, path.size());
    data += path;
    appendUint16(data, additionalData.size());
    data += additionalData;
    appendUint16(data, localPort);
    data += (char)flags;
    calcHash(data, passphrase);
    return true;
  }

  bool parseAnnouncement(const char *data, size_t size, Announcement & announcement) {
    if (!data || size < 24 || data[20] != 0) { return false; }
    size_t offset = 21;
    const size_t protocolLength = (uint8_t)data[offset++];
    if (offset + protocolLength + 2 > size) { return false; }
    announcement.protocol.assign(data + offset, protocolLength);
    offset += protocolLength;

    uint16_t pathLength = 0;
    if (!readUint16(data, size, offset, pathLength)) { return false; }
    offset += 2;
    if (offset + pathLength > size) { return false; }
    announcement.path.assign(data + offset, pathLength);
    offset += pathLength;

    announcement.hasLocalPort = false;
    announcement.localPort = 0;
    announcement.hasFlags = false;
    announcement.flags = 0;
    if (offset == size) { return true; }
    if (offset + 2 > size || !readUint16(data, size, offset, announcement.localPort)) { return false; }
    announcement.hasLocalPort = true;
    offset += 2;
    if (offset == size) { return true; }
    if (offset + 1 != size) { return false; }
    announcement.hasFlags = true;
    announcement.flags = (uint8_t)data[offset];
    return true;
  }

  bool parseRelayRequest(const char *data, size_t size, RelayRequest & request) {
    if (!data || size < 29 || data[20] != 2) { return false; }
    size_t offset = 21;
    size_t protocolLength = (uint8_t)data[offset++];
    if (offset + protocolLength + 2 > size) { return false; }
    request.protocol.assign(data + offset, protocolLength);
    offset += protocolLength;

    uint16_t pathLength = 0;
    if (!readUint16(data, size, offset, pathLength)) { return false; }
    offset += 2;
    if (offset + pathLength + 2 > size) { return false; }
    request.path.assign(data + offset, pathLength);
    offset += pathLength;

    uint16_t additionalLength = 0;
    if (!readUint16(data, size, offset, additionalLength)) { return false; }
    offset += 2;
    if (offset + additionalLength + 3 != size) { return false; }
    request.additionalData.assign(data + offset, additionalLength);
    offset += additionalLength;
    if (!readUint16(data, size, offset, request.localPort)) { return false; }
    request.flags = (uint8_t)data[offset + 2];
    return true;
  }

  bool parsePunchInstruction(const char *data, size_t size, PunchInstruction & instruction) {
    if (!data || size < 29 || data[20] != 3) { return false; }
    uint16_t additionalLength = 0;
    if (!readUint16(data, size, 21, additionalLength)) { return false; }
    size_t addressOffset = 23 + additionalLength;
    if (addressOffset + 6 > size) { return false; }

    size_t addressRemainder = size - addressOffset - 2;
    size_t hostLength = 0;
    instruction.hasSuggestedPort = false;
    if (addressRemainder == 4 || addressRemainder == 16) {
      hostLength = addressRemainder;
    } else if (addressRemainder == 6 || addressRemainder == 18) {
      hostLength = addressRemainder - 2;
      instruction.hasSuggestedPort = true;
      readUint16(data, size, addressOffset + 2 + hostLength, instruction.suggestedPort);
    } else {
      return false;
    }

    instruction.additionalData.assign(data + 23, additionalLength);
    readUint16(data, size, addressOffset, instruction.port);
    Socket::hostBytesToStr(data + addressOffset + 2, hostLength, instruction.host);
    return instruction.host.size();
  }
  ...
  void List::setProtocol(const std::string & p) {
    size_t protocolLength = std::min<size_t>(p.size(), 0xFF);
    protocolString.assign("\000", 1);
    protocolString[0] = protocolLength;
    protocolString.append(p.data(), protocolLength);
  }
  ...
  void List::add(const std::string & u) {
    ...
    if (url.path.size() > 0xFFFF) {
      WARN_MSG("Ignoring Foghorn URL with an oversized path");
      return;
    }
  ...
  void List::publish(Socket::UDPConnection & uSock, const uint64_t currTime) {
    ...
    if (lastPublish >= currTime) { return; }
    ...
    if (lastOpenNotice && lastOpenNotice + 180 >= currTime) { bits |= 1; }
  ...
  bool List::parsePacket(Socket::UDPConnection & udpSrv, const uint64_t currTime) {
    ...
    for (it = instances.begin(); it != instances.end(); ++it) {
      ...
      if (udpSrv.data[20] == 1) { // Mapping response
        if (udpSrv.data.size() == 28) {
          if (std::string(udpSrv.data + 22, udpSrv.data.size() - 22) != it->second.porthost4) {
            ...
            if (!it->second.url.path.size()) {
              INFO_MSG("Verified IPv4 with %s:%" PRIu16 " at %s:%" PRIu16, it->second.url.host.c_str(),
                       it->second.url.getPort(), host.c_str(), port);
            } else {
              INFO_MSG("Published IPv4 to %s:%" PRIu16 " as %s at %s:%" PRIu16 " (%s)", it->second.url.host.c_str(),
                       it->second.url.getPort(), it->second.url.path.c_str(), host.c_str(), port,
                       stateToString(it->second.state4));
            }
          }
          if (it->second.state4 != udpSrv.data[21]) {
            INFO_MSG("IPv4 at %s:%" PRIu16 " changed from %s to %s", it->second.url.host.c_str(),
                     it->second.url.getPort(), stateToString(it->second.state4), stateToString(udpSrv.data[21]));
            it->second.state4 = udpSrv.data[21];
          }
        ...
        if (udpSrv.data.size() == 40) {
          if (std::string(udpSrv.data + 22, udpSrv.data.size() - 22) != it->second.porthost6) {
            ...
            Socket::hostBytesToStr(udpSrv.data + 24, udpSrv.data.size() - 24, host);
            if (!it->second.url.path.size()) {
              INFO_MSG("Verified IPv6 with %s:%" PRIu16 " at %s:%" PRIu16, it->second.url.host.c_str(),
                       it->second.url.getPort(), host.c_str(), port);
            } else {
              INFO_MSG("Published IPv6 to %s:%" PRIu16 " as %s at %s:%" PRIu16 " (%s)", it->second.url.host.c_str(),
                       it->second.url.getPort(), it->second.url.path.c_str(), host.c_str(), port,
                       stateToString(it->second.state6));
            }
          }
          if (it->second.state6 != udpSrv.data[21]) {
            INFO_MSG("IPv6 at %s:%" PRIu16 " changed from %s to %s", it->second.url.host.c_str(),
                     it->second.url.getPort(), stateToString(it->second.state6), stateToString(udpSrv.data[21]));
            it->second.state6 = udpSrv.data[21];
          }
      ...
      if (udpSrv.data[20] == 3) { // Request to connect to user
        PunchInstruction instruction;
        if (!parsePunchInstruction(udpSrv.data, udpSrv.data.size(), instruction)) { return false; }
        std::string localIP = udpSrv.getLocalAddr().host();
        uint32_t localPort = udpSrv.getLocalAddr().port();
        if (instruction.hasSuggestedPort) { localPort = instruction.suggestedPort; }

        std::string connIdent = localIP + ":" + JSON::Value(localPort).asString() + "/" + instruction.host + ":" +
          JSON::Value(instruction.port).asString();
        if (recentMap.count(connIdent)) { return false; }
        recentMap[connIdent] = currTime;
        punchReq.host = instruction.host;
        punchReq.port = instruction.port;
        punchReq.additionalData = instruction.additionalData;
      ...
      if (udpSrv.data[20] == 4 && udpSrv.data.size() >= 23) { // Ping
        ...
        if (std::string(udpSrv.data + 23, udpSrv.data.size() - 23) == "OPEN_PORT") { lastOpenNotice = currTime; }
  ...
  const PunchRequest & List::getPunchData() const {
    return punchReq;
  }

  bool List::insertRecent(const std::string & localIP, const uint16_t localPort, const std::string & remoteIP,
                          const uint16_t remotePort, const uint64_t currTime) {
    std::string connIdent =
      localIP + ":" + JSON::Value(localPort).asString() + "/" + remoteIP + ":" + JSON::Value(remotePort).asString();
    if (recentMap.count(connIdent)) { return false; }
    recentMap[connIdent] = currTime;
    return true;
  }
  ...
  Puncher::Puncher(const HTTP::URL & target, const std::string & protocol, const std::string & additionalData) {
    ...
    // Allocate data
    pData.reset(new PunchData());
    pData->sock = new Socket::UDPConnection();
    pData->addrs = Socket::getAddrs(trgt.host, trgt.getPort(), AF_UNSPEC, false);
    if (!pData->addrs.size() || !pData->sock->setDestination(pData->addrs.front())) {
      pData.reset();
      return;
    }
    lPort = 0;
    if (trgt.args.size()) {
      ...
      if (args.count("lport")) { lPort = JSON::Value(args["lport"]).asInt(); }
    }
    ...
    if (!Foghorn::makePunchRequest(pData->reqStr, protocol, trgt.path, additionalData, lPort, 0, trgt.user)) {
      pData.reset();
    }
  }
  ...
  bool Puncher::start() {
    if (!pData || !pData->sock || !pData->addrs.size()) { return false; }
    size_t attempt = 0;
    while (attempt < 32) {
      ...
      if (attempt) {
        ...
        delete pData->sock;
        pData->sock = new Socket::UDPConnection();
        if (!pData->sock->setDestination(pData->addrs.front()) || !pData->sock->bind(lPort)) {
          ++attempt;
          continue;
        }
          ...
          while (pData->sock->Receive()) {
            if (Foghorn::checkHash(pData->sock->data, trgt.user)) {
              ...
              if (pData->sock->data[20] == 3 && pData->sock->data.size() >= 23) {
                PunchInstruction instruction;
                if (!parsePunchInstruction(pData->sock->data, pData->sock->data.size(), instruction)) {
                ...
                targetPort = instruction.port;
                targetHost = instruction.host;
                if (instruction.hasSuggestedPort) { lPort = instruction.suggestedPort; }
                if (lPort == 0xFFFF) {
                  ...
                  std::shared_ptr<PunchData> threadData = pData;
                  std::thread punchThread([threadData]() {
                    uint64_t startTime = Util::bootSecs();
                    threadData->bgSock->setIgnoreSendErrors(true);
                    while (!threadData->connected && startTime + 10 > Util::bootSecs()) {
                      Util::sleep(100);
                      if (!threadData->connected) {
                        for (const auto & jt : threadData->addrs) {
                          if (threadData->bgSock->setDestination(jt)) {
                            threadData->bgSock->SendNow(threadData->reqStr);
                          }
                        }
                      }
                    }
                    threadData->bgSock->setIgnoreSendErrors(false);
                    INFO_MSG("Exiting hole punching thread (%s)", threadData->connected ? "connected" : "failed");
                  });
                  punchThread.detach();
