...
namespace {
  ...
  bool chunked11ReuseWithSharedParser() {
    ...
    if (!ok) {
      std::cerr << "shared parser did not accept the next request after a chunked response"
                << std::endl;
    }
    connection.close();
    close(sockets[1]);
  ...
  bool closesAsRequested(const std::string &protocol, const std::string &requestConnection,
                         bool buffered, const char *description) {
    int sockets[2];
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, sockets)) { return false; }
    Socket::Connection connection(sockets[0]);
    HTTP::Parser request;
  ...
  bool responseCanForceClose(bool buffered) {
    int sockets[2];
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, sockets)) { return false; }
    Socket::Connection connection(sockets[0]);
    HTTP::Parser request;
    ...
    const std::string wire = receiveAvailable(sockets[1]);
    const bool ok = !connection.connected() && contains(wire, "Connection: close", "response close header") &&
                    excludes(wire, "Transfer-Encoding: chunked", "chunking on a closing response");
    close(sockets[1]);
    return ok;
  }
  ...
  bool bufferedEmptyIsDelimited() {
    ...
    response.Chunkify("", connection);
    const std::string wire = receiveAvailable(sockets[1]);
    const bool ok = connection.connected() && contains(wire, "Content-Length: 0", "empty response length");
    connection.close();
    close(sockets[1]);
    return ok;
  }
  ...
  bool buildResponseKeepsZeroLength() {
    HTTP::Parser response;
...
int main() {
  bool responseTestsOk = true;
#define CHECK_RESPONSE_TEST(expression) \
  do { \
    if (!(expression)) { \
      std::cerr << "failed: " #expression << std::endl; \
      responseTestsOk = false; \
    } \
  } while (false)
  ...
  CHECK_RESPONSE_TEST(responseCanForceClose(false));
  CHECK_RESPONSE_TEST(responseCanForceClose(true));
  CHECK_RESPONSE_TEST(closesAsRequested("HTTP/1.1", "CLOSE", true,
                                        "case-insensitive close header"));
  CHECK_RESPONSE_TEST(noContentIsSelfDelimited());
  CHECK_RESPONSE_TEST(bufferedStatusIsPreserved());
  CHECK_RESPONSE_TEST(bufferedEmptyIsDelimited());
