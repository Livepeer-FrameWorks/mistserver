...
bool testResponseValidationTable() {
  ...
  const std::vector<ResponseCase> cases = {
    ...
    {"completion_socket_1", std::string("\x90\x51\xEE", 3), true, 0x51},
    {"error", std::string("\x90\x60\x02", 3), true, 0x60},
    ...
    {"ack_socket_7", std::string("\x90\x47\xFF", 3), true, 0x47},
    {"wrong_type", std::string("\x90\x70\xFF", 3), false, 0x00},
  };
...
bool testQueryCapabilitiesFeatures() {
  ...
  bool ok = true;
  ok &= expect(info.id == "visca:10.1.2.3:52381", "id should be the VISCA endpoint identity");
  ok &= expect(info.protocols["visca"].endpointId == info.id, "protocol endpoint identity should match");
...
bool testQueryCapabilitiesSupportedCommands() {
  ...
  if (info.protocols.count("visca") == 1) {
    ...
    ok &= expect(hasValue(cfg.capabilities.supportedCommands, "absolute_move"),
                 "commands should include absolute_move");
...
bool testCreateDeviceFromProtocolEndpoint() {
  ...
  if (viscaDevice) {
    ...
    ok &= expect(viscaDevice->getTransportType() == VISCA::ViscaTransport::TCP, "transport should be explicit");
    ok &= expect(viscaDevice->getFramingType() == VISCA::ViscaFraming::Raw, "framing should be explicit");
  }
  return ok;
}
...
bool testSetAddressValidationAndTrim() {
  ...
  ok &= expect(device.getTransportType() == VISCA::ViscaTransport::UDP, "setAddress should preserve UDP");
  return ok;
}
...
bool testSetAddressPortRules() {
  ...
  ok &= expect(device.getTransportType() == VISCA::ViscaTransport::UDP, "setAddress must not infer transport from port");
  return ok;
}

bool testOverIpDecoder() {
  const std::string payload("\x90\x51\xFF", 3);
  std::string packet("\x01\x11\x00\x03\x00\x00\x00\x2A", 8);
  packet += payload;
  std::string decoded;
  bool ok = true;
  ok &= expect(VISCA::Device::decodeOverIpPacket(packet, 42, decoded), "valid framed response should decode");
  ok &= expect(decoded == payload, "decoded payload mismatch");
  ok &= expect(!VISCA::Device::decodeOverIpPacket(packet, 41, decoded), "wrong sequence must be rejected");
  ok &= expect(!VISCA::Device::decodeOverIpPacket(packet.substr(0, 9), 42, decoded),
...
int main(int argc, char **argv) {
  ...
  if (testCase == "over_ip_decoder") { return testOverIpDecoder() ? 0 : 1; }
