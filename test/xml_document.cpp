...
bool testDocumentCopyConstructorDeepCopy() {
  ...
  bool ok = true;
  ok &= expect(original.getRoot().getChild("child").getTextContent() == "after",
               "original value should update after mutation");
  ok &= expect(copy.getRoot().getChild("child").getTextContent() == "before",
               "copy-constructed document should remain independent");
  return ok;
}
...
bool testDocumentCopyAssignmentDeepCopy() {
  ...
  bool ok = true;
  ok &= expect(source.getRoot().getChild("value").getTextContent() == "two",
               "source should reflect mutation");
  ok &= expect(target.getRoot().getChild("value").getTextContent() == "one",
               "copy-assigned target should remain independent");
  return ok;
}
...
bool testExternalEntityDisabled() {
  try {
    XML::Document doc("<!DOCTYPE root [<!ENTITY xxe SYSTEM 'file:///etc/passwd'>]><root>&xxe;</root>");
    return expect(doc.getRoot().getTextContent().find("root:") == std::string::npos,
                  "external entity content must not be loaded");
  } catch (...) {
    return true; // Rejecting the document outright is also safe.
  }
}
...
int main(int argc, char **argv) {
  ...
  if (testCase == "external_entity_disabled") { return testExternalEntityDisabled() ? 0 : 1; }
