...
bool testExtractCleanIPTable() {
  ...
  const std::vector<TestCase> cases = {
    ...
    // IPv6 canonicalization: differently-written forms of one address must dedupe to one key
    {"2001:0db8:0000:0000:0000:0000:0000:0001", "2001:db8::1"},
    {"2001:DB8:0:0:0:0:0:1", "2001:db8::1"},
    {"https://[2001:0db8:0000::1]:443/", "2001:db8::1"},
    {"Camera-ONE.Local:52381", "camera-one.local"},
  };
...
bool testUpdateDeviceInfoMerge() {
  ...
  onvifOld.type = "onvif";
  onvifOld.address = "10.1.1.10";
  onvifOld.port = 0;
  onvifOld.transport = "tcp";
  onvifOld.username = "olduser";
  onvifOld.password = "oldpass";
  onvifOld.tlsPolicy = "required";
  onvifOld.tlsCaFile = "/camera/ca.pem";
  onvifOld.associationSource = "manual";
  ...
  onvifNew.port = 80;
  onvifNew.transport = "udp";
  onvifNew.tlsPolicy = "opportunistic";
  ...
  ok &= expect(existing.protocols["onvif"].address == "10.1.1.20", "advertised endpoint address should refresh");
  ok &= expect(existing.protocols["onvif"].port == 80, "empty protocol port should be filled");
  ok &= expect(existing.protocols["onvif"].transport == "tcp", "manual transport must survive rediscovery");
  ok &= expect(existing.protocols["onvif"].tlsPolicy == "required", "TLS policy must survive rediscovery");
  ok &= expect(existing.protocols["onvif"].tlsCaFile == "/camera/ca.pem", "custom CA must survive rediscovery");
  ...
  ok &= expect(existing.protocols["onvif"].capabilities.supportedCommands.size() == 2,
               "supportedCommands should deduplicate");
...
bool testUpdateDeviceInfoStatusRule() {
  ...
  Controller::updateDeviceInfo(existing, incoming);
  ok &= expect(existing.status == "discovered", "rediscovery should restore disconnected status");
  return ok;
}
...
bool testCreatePTZCommandPresetAndOtherActions() {
  ...
  presetArgs["token"] = "opaque-profile-token";
  presetArgs["store"] = true;
  ok &= expect(Controller::createPTZCommand("preset", presetArgs, preset), "preset should parse from token");
  ok &= expect(preset.presetToken == "opaque-profile-token", "preset token should remain opaque");
  ok &= expect(preset.storePreset, "store should map to storePreset");

  Device::PTZCommand badPreset;
  JSON::Value badPresetArgs;
  badPresetArgs["index"] = 1;
  ok &= expect(!Controller::createPTZCommand("preset", badPresetArgs, badPreset),
               "numeric/index preset API should be rejected");
...
bool testProtocolEndpointIds() {
  Device::DeviceInfo dev;
  dev.id = "source name";
  dev.host = "2001:0db8::1";
  Device::ProtocolConfig onvif;
  onvif.type = "onvif";
  onvif.address = "2001:db8::1";
  onvif.scheme = "https";
  onvif.path = "/onvif/device_service";
  Device::ProtocolConfig visca;
  visca.type = "visca";
  visca.address = "2001:db8::1";
  visca.port = 52381;
  Device::ProtocolConfig ndi;
  ndi.type = "ndi";
  bool ok = true;
  ok &= expect(Controller::protocolEndpointId("onvif", onvif, dev) ==
                 "onvif:https://[2001:db8::1]:443/onvif/device_service", "ONVIF fallback identity mismatch");
  ok &= expect(Controller::protocolEndpointId("visca", visca, dev) == "visca:[2001:db8::1]:52381",
               "VISCA identity mismatch");
  ok &= expect(Controller::protocolEndpointId("ndi", ndi, dev).empty(),
               "NDI identity must not be inferred from display or host fields");
  ndi.endpointId = "ndi:exact source";
  ok &= expect(Controller::protocolEndpointId("ndi", ndi, dev) == ndi.endpointId,
               "explicit endpoint identity must win");
  return ok;
}
...
int main(int argc, char **argv) {
  ...
  if (testCase == "protocol_endpoint_ids") { return testProtocolEndpointIds() ? 0 : 1; }
