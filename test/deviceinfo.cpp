...
bool testEmptyJsonArrays() {
  ...
  ok &= expect(json["ptzFeatures"].isArray() && json["ptzFeatures"].size() == 0,
               "ptzFeatures should serialize as empty array");
  ...
  ok &= expect(
    json["analytics"]["objectClassifications"].isArray() && json["analytics"]["objectClassifications"].size() == 0,
...
bool testAnalyticsRoundtripPopulated() {
  ...
  onvif.alternateEndpoints.push_back("https://10.0.0.20:8899/onvif/device_service");
  onvif.tlsPolicy = "required";
  onvif.tlsCaFile = "/tmp/camera-ca.pem";
  onvif.associationSource = "manual";
  ...
  ok &= expect(parsed.analytics.analyticsServiceUrl == info.analytics.analyticsServiceUrl,
               "analyticsServiceUrl should round-trip");
  ...
  ok &= expect(parsed.protocols["onvif"].capabilities.supportedCommands.size() == 1,
               "supportedCommands should round-trip");
  ok &= expect(parsed.protocols["onvif"].endpointId == onvif.endpointId, "endpointId should round-trip");
  ok &= expect(parsed.protocols["onvif"].scheme == "https", "scheme should round-trip");
  ok &= expect(parsed.protocols["onvif"].transport == "tcp", "transport should round-trip");
  ok &= expect(parsed.protocols["onvif"].framing == "soap", "framing should round-trip");
  ok &= expect(parsed.protocols["onvif"].path == "/onvif/device_service", "path should round-trip");
  ok &= expect(parsed.protocols["onvif"].alternateEndpoints.size() == 1,
               "alternate endpoints should round-trip");
  ok &= expect(parsed.protocols["onvif"].tlsPolicy == "required", "TLS policy should round-trip");
  ok &= expect(parsed.protocols["onvif"].tlsCaFile == onvif.tlsCaFile, "TLS CA file should round-trip");
  ok &= expect(parsed.protocols["onvif"].associationSource == "manual",
               "association source should round-trip");
  return ok;
}
...
bool testStreamRoundtrip() {
  ...
  ok &= expect(parsed.streams[0].resolution == "1920x1080", "stream resolution should round-trip");
  ok &= expect(parsed.streams[0].framerate == "29.97", "stream framerate should round-trip");
  ok &= expect(parsed.streams[0].options["profile"] == "main", "stream options should round-trip");
  return ok;
}
...
bool testProtocolCapabilityFlagsRoundtrip() {
  ...
  ok &= expect(hasValue(roundtrip.capabilities.supportedCommands, "absolute_move"),
               "supportedCommands should include absolute_move");
  ok &= expect(hasValue(roundtrip.capabilities.supportedFormats, "control"),
               "supportedFormats should include control");
  return ok;
}
