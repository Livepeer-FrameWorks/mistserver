  ...
  blueprints: {
      ...
      if (MistVideo.player.api) {
        var videovalues = {
          ...
          "tracks playing": function(){
            if (MistVideo.reporting && MistVideo.reporting.stats && MistVideo.reporting.stats.d && MistVideo.reporting.stats.d.tracks) {
              var trackidxs = MistVideo.reporting.stats.d.tracks;
              if (MistVideo.info && MistVideo.info.meta && MistVideo.info.meta.tracks) {
                var out = {};
                for (var i in MistVideo.info.meta.tracks) {
                  var track = MistVideo.info.meta.tracks[i];
                  if (trackidxs.indexOf(track.idx) > -1) out[track.type] = track;
                }
                return processMultiOutput(out,function(track){ return track.codec+" ("+track.idx+")"; });
              }
              return trackidxs.join(", ");
            }
            return "";
          },
          ...
          "current bitrate": function(){
            ...
            if (MistVideo.player.monitor && ("currentBps" in MistVideo.player.monitor)) {
              out = MistVideo.player.monitor.currentBps;
            }
            if (MistVideo.player.api && "currentBps" in MistVideo.player.api) {
              out = MistVideo.player.api.currentBps;
            }
          ...
          "Max bitrate": function(){
            if (MistVideo.player.api && "maxBps" in MistVideo.player.api) {
              var out = MistVideo.player.api.maxBps;
          ...
          "framerate in": function(){
            if (MistVideo.player.api && "framerate_in" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.framerate_in(),{
                "default": function(v){ return MistUtil.format.number(v)+"p/s"; },
          ...
          "Display buffer": function(){
            if (MistVideo.player.api && "displayBuffer" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.displayBuffer,function(v){ return MistUtil.format.number(v)+"ms"; });
            }
          },
          "Decode Time": function(){
            if (MistVideo.player.api && "decodeTime" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.decodeTime,function(v){ return MistUtil.format.number(v)+"ms"; });
            }
          },
          "Display Time": function(){
            if (MistVideo.player.api && "displayTime" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.displayTime,function(v){ return MistUtil.format.number(v)+"ms"; });
            }
          },
          "Audio/Video sync": function(){
            if (MistVideo.player.api && "sync" in MistVideo.player.api) return MistUtil.format.number(MistVideo.player.api.sync)+"ms";
          },
          "Earliness": function(){
            if (MistVideo.player.api && "earliness" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.earliness,function(v){ return MistUtil.format.number(v)+"ms"; });
            }
          },
          "Server delay": function(){
            if (MistVideo.player.api && "server_delay" in MistVideo.player.api) return MistUtil.format.number(MistVideo.player.api.server_delay)+"ms";
          },
          "Local jitter": function(){
            if (MistVideo.player.api && "local_jitter" in MistVideo.player.api) return MistUtil.format.number(MistVideo.player.api.local_jitter)+"ms";
          },
          "Timestamp shift": function(){
            if (MistVideo.player.api && "shift" in MistVideo.player.api) {
              return processMultiOutput(MistVideo.player.api.shift,function(v){ return MistUtil.format.number(v)+"μs"; });
            }
          },
          "Buffering": function(){
            if (MistVideo.player.api && ("buffer_manager" in MistVideo.player.api)) {
              var bm = MistVideo.player.api.buffer_manager;
              if (bm) {
          ...
          "Speed": function(){
            if (MistVideo.player.api && ("buffer_manager" in MistVideo.player.api)) {
              if (!MistVideo.player.api.buffer_manager) return 0;
              var speed = MistVideo.player.api.buffer_manager.timing.speed;
      ...
      var menu = createMenu(MistVideo, {
        ...
        onChange: function(val) {
          ...
          if (MistVideo.options.forcePlayer != MistVideo.playerName) {
            if (MistVideo.options.startCombo) MistVideo.options.startCombo = false;
    ...
    debuglevel: function(){
      ...
      select.addEventListener("change",function(){
        MistVideo.player.debugging = this.value == "false" ? false : (this.value == "true" ? true : this.value);
      });
      select.value = MistVideo.player.debugging === true ? "true" : (MistVideo.player.debugging || "false");
      container.appendChild(select);
