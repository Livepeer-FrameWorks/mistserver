...
export const playbackBlueprints = {
    progress: function(){
      ...
      function getBufferWindow() {
        if ("on_time" in MistVideo.player.api) {
          var ot = MistVideo.player.api.on_time;
    ...
    speaker: function(){
      ...
      MistVideo.playerState.on("muted", function(isMuted){
        ...
        try { localStorage['mistMuted'] = isMuted ? "1" : "0"; } catch (e) {}
      });
      ...
      var initevent = MistUtil.event.addListener(MistVideo.video,"loadedmetadata",function(){
        if (!MistVideo.options.muted && ('localStorage' in window) && (localStorage != null) && ('mistMuted' in localStorage)) {
          MistVideo.api.muted = (localStorage['mistMuted'] == "1");
        }
