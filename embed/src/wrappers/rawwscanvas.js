...
p.prototype.build = function (MistVideo,callback) {
  ...
  player.onDecoderLoad = function() {
    ...
    function init() {
       ...
       * 10       2        offset (when frame should be outputted compared to timestamp) [ms]
       * */
  ...
  if ("libde265" in window) {
    ...
    var scripttag = MistUtil.scripts.insert(MistVideo.urlappend(mistplayers.rawwscanvas.scriptsrc(MistVideo.options.host)),{
      onerror: function(e){
