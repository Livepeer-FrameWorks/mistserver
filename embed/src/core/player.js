...
import { mistplayers } from './registry.js';
import { MistSkin } from '../ui/skin-loader.js';
import { MistUI } from '../ui/controls.js';
import { PlayerState } from './state.js';
import { locales } from './locales.js';

export function MistPlayer() {};
export function mistPlay(streamName,options) {
  return new MistVideo(streamName,options);
}
export function MistVideo(streamName,options) {
  var MistVideo = this;
  ...
  options = MistUtil.object.extend({
    ...
    translations: false   //override UI strings for i18n, e.g. { play: "Afspelen" } or "nl"
  },options);
  ...
  this.translate = function(key, fallback) {
    ...
    if (key in defaultStrings) { return defaultStrings[key]; }
    return fallback || key;
  };

  this.playerState = new PlayerState(this);

  // Fullscreen support — extracted from settings.js for programmatic access
  var fsCache = null;
  this.fullscreen = {
    get supported() {
      if (!MistVideo.container && !MistVideo.video) return false;
      var f = MistVideo.fullscreen._detect();
      return f !== false;
    },
    get active() {
      var f = MistVideo.fullscreen._detect();
      if (f) return f.element() === f.target();
      return !!(MistVideo.container && MistVideo.container.hasAttribute('data-fullscreen'));
    },
    _detect: function() {
      ...
      var targets = [function(){ return MistVideo.container; }, function(){ return MistVideo.video; }];
      for (var j = 0; j < targets.length; j++) {
        for (var i = 0; i < requestfuncs.length; i++) {
          var el = targets[j]();
          if (el && requestfuncs[i] in el) {
            fsCache = {
              request: function() { return fsCache.target()[requestfuncs[i]](); },
              cancel: function() { return document[cancelfuncs[i]](); },
              element: function() { return document[elementfuncs[i]]; },
              event: eventnames[i],
              target: targets[j]
            };
            return fsCache;
    ...
    request: function() {
      ...
      if (MistVideo.container) {
        ...
        var keydownHandler = function(e) {
          if (e.key === 'Escape') MistVideo.fullscreen.exit();
        };
        MistVideo.fullscreen._escHandler = keydownHandler;
    ...
    exit: function() {
      ...
      if (MistVideo.container && MistVideo.container.hasAttribute('data-fullscreen')) {
        MistVideo.container.removeAttribute('data-fullscreen');
        MistUtil.event.send('fakefullscreenchange', null, document);
        if (MistVideo.player && MistVideo.player.resizeAll) MistVideo.player.resizeAll();
        if (MistVideo.fullscreen._escHandler) {
          document.removeEventListener('keydown', MistVideo.fullscreen._escHandler);
          MistVideo.fullscreen._escHandler = null;
        }
      }
    },
    toggle: function() {
      if (MistVideo.fullscreen.active) MistVideo.fullscreen.exit();
      else MistVideo.fullscreen.request();
    },
    _escHandler: null
  };
  ...
  this.pip = {
    get supported() {
      return !!(document.pictureInPictureEnabled && MistVideo.video && ('requestPictureInPicture' in MistVideo.video));
    },
    get active() {
      return !!document.pictureInPictureElement;
    },
    request: function() {
      if (MistVideo.video && ('requestPictureInPicture' in MistVideo.video)) {
        return MistVideo.video.requestPictureInPicture();
      }
    ...
    toggle: function() {
      if (MistVideo.pip.active) return MistVideo.pip.exit();
      return MistVideo.pip.request();
    }
  };

  // Audio Gain (volume boost above 100%)
  this.gain = {
    _ctx: null,
    _node: null,
    _source: null,
    ...
    init: function() {
      if (MistVideo.gain._ctx) return;
      if (!MistVideo.video) return;
      try {
        var ctx = new (window.AudioContext || window.webkitAudioContext)();
        var source = ctx.createMediaElementSource(MistVideo.video);
        var gainNode = ctx.createGain();
        source.connect(gainNode);
        gainNode.connect(ctx.destination);
        gainNode.gain.value = MistVideo.gain._value;
        MistVideo.gain._ctx = ctx;
        MistVideo.gain._node = gainNode;
        MistVideo.gain._source = source;
      } catch(e) {
        MistVideo.log("AudioContext init failed: "+e,"error");
      }
    }
  };

  // Capability detection for the active wrapper
  this.getCapabilities = function() {
    var api = MistVideo.player && MistVideo.player.api ? MistVideo.player.api : null;
    return {
      ...
      fullscreen:   MistVideo.fullscreen.supported,
      pip:          MistVideo.pip.supported,
  ...
  function onStreamInfo(d) {
    ...
    if (MistVideo.playerState && d.meta && d.meta.tracks) {
      MistVideo.playerState.set('tracks', MistUtil.tracks.parse(d.meta.tracks));
    }
    ...
    if (MistVideo.choosePlayer()) {
      ...
      MistVideo.player.build(MistVideo,function(video){
        ...
        //bind reactive state to the video element
        MistVideo.playerState.bind(video);

        //normalized playback API (safe wrappers around player.api)
        var _api = function() { return MistVideo.player && MistVideo.player.api ? MistVideo.player.api : null; };
        MistVideo.api = {
          get currentTime() { var a = _api(); return a ? a.currentTime : 0; },
          set currentTime(v) { var a = _api(); if (a) a.currentTime = v; },
          get duration() { var a = _api(); return a ? a.duration : 0; },
          get paused() { var a = _api(); return a ? a.paused : true; },
          get volume() { var a = _api(); return a ? a.volume : 1; },
          set volume(v) { var a = _api(); if (a) a.volume = v; },
          get muted() { var a = _api(); return a ? a.muted : false; },
          set muted(v) { var a = _api(); if (a) a.muted = v; },
          get buffered() { var a = _api(); return a ? a.buffered : null; },
          play: function() { var a = _api(); return a && a.play ? a.play() : Promise.resolve(); },
          pause: function() { var a = _api(); if (a && a.pause) a.pause(); },

          get playbackRate() { var a = _api(); return (a && 'playbackRate' in a) ? a.playbackRate : 1; },
          set playbackRate(v) { var a = _api(); if (a && 'playbackRate' in a) a.playbackRate = v; },

          get loop() { var a = _api(); return (a && 'loop' in a) ? a.loop : false; },
          set loop(v) {
            var a = _api();
            if (a && 'loop' in a) a.loop = v;
            if (MistVideo.playerState) MistVideo.playerState.set('loop', !!v);
          },
          ...
          setTrack: function(type, trackid) {
            ...
            if (type === 'subtitle') {
              if (trackid === '' || trackid === 'none' || trackid === null) {
                if ('setWSSubtitle' in a) { a.setWSSubtitle(undefined); }
                else if ('setSubtitle' in a) { a.setSubtitle(); }
                MistUtil.event.send('playerUpdate_trackChanged', { type: 'subtitle', trackid: '' }, MistVideo.video);
                return true;
              }
              if ('setWSSubtitle' in a) {
                a.setWSSubtitle(trackid);
                MistUtil.event.send('playerUpdate_trackChanged', { type: 'subtitle', trackid: trackid }, MistVideo.video);
                return true;
              }
              if ('setSubtitle' in a) {
                var tracks = MistUtil.tracks.parse(MistVideo.info.meta.tracks);
                if (!tracks.subtitle || !(trackid in tracks.subtitle)) return false;
                var meta = MistUtil.object.extend({}, tracks.subtitle[trackid]);
                var subtitleSource = null;
                for (var i in MistVideo.info.source) {
                  var src = MistVideo.info.source[i];
                ...
                meta.src = MistUtil.http.url.addParam(subtitleSource, { track: trackid });
                meta.label = meta.displayName || ('track ' + trackid);
                meta.lang = meta.language || 'und';
                a.setSubtitle(meta);
                MistUtil.event.send('playerUpdate_trackChanged', { type: 'subtitle', trackid: trackid }, MistVideo.video);
                return true;
              }
            ...
            if ('setTrack' in a) {
              ...
              MistUtil.event.send('playerUpdate_trackChanged', { type: type, trackid: trackid }, MistVideo.video);
              return true;
            }
            if ('setTracks' in a) {
              var obj = {};
        ...
        if ("setSize" in MistVideo.player) {
          ...
          MistVideo.player.resize = function(options,oldsize){
            ...
            if (container) {
              var w = size.width;
              var bp = w < 384 ? "xs" : w < 576 ? "sm" : w < 768 ? "md" : w < 960 ? "lg" : "xl";
    ...
    function openSocket() {
      ...
      socket.addEventListener("message",function(e){
        ...
        if ("error" in data) {
          ...
          if (MistVideo.playerState) MistVideo.playerState.set('streamState', data.error);
          var buttons;
          switch (data.error) {
            ...
            case "Stream status is invalid?!":
              if (MistVideo.showIdleScreen) {
                var perc = ("perc" in data) ? data.perc : undefined;
                if ((MistVideo.player) && (MistVideo.player.api) && (!MistVideo.player.api.paused)) {
                  ...
                  if (!on_ended_show_state) {
                    on_ended_show_state = MistUtil.event.addListener(MistVideo.video,"ended",function(){
                      MistVideo.showIdleScreen(data.error, perc);
                    });
                  }
                  if (!on_waiting_show_state) {
                    on_waiting_show_state = MistUtil.event.addListener(MistVideo.video,"waiting",function(){
                      MistVideo.showIdleScreen(data.error, perc);
                    });
                  }
                  return;
                }
                MistVideo.showIdleScreen(e, perc);
                return;
              }
              if ((MistVideo.player) && (MistVideo.player.api) && (!MistVideo.player.api.paused)) {
                MistVideo.log(data.error,"error");
          ...
          if (MistVideo.playerState) MistVideo.playerState.set('streamState', 'Stream is online');
          MistVideo.clearError();
          if (MistVideo.hideIdleScreen) MistVideo.hideIdleScreen();
  ...
  this.unload = function(reason){
    ...
    if (this.playerState) { this.playerState.destroy(); }
