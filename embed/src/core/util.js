export const MistUtil = {
  format: {
    ...
    ago: function(date,range,translate){
      ...
      //if translate is a function, use it to translate output strings
  ...
  css: {
    ...
    generateCustomProperties: function(colors, tokens) {
      ...
      if (colors) {
        for (var key in colors) {
          if (key in map) { props[map[key]] = colors[key]; }
        }
      }
      if (tokens) {
    ...
    prependClass: function (css,prepend,applyToChildren) {
      ...
      //remove @-declarations without braces (e.g. @layer name, name2; @import url();)
      //save them for reinsertion so they don't interfere with selector prepending
      var atDecls = [];
      ...
      //remove all @ {} blocks (media, keyframes, layer, container etc) and save it to re-insert them after class prepending
      //uses a while loop to re-scan after each extraction, so nested @-blocks (e.g. @keyframes inside @layer) are consumed by their parent
      //placeholders avoid @ so the /@[^}]*}/ regex won't match them on the next iteration
      var save = [];
      var m;
      while (m = css.match(/@[^}]*}/)) {
        css = css.replace(m[0],"@@#@@");
        ...
        while (replacecount < (block.match(/{/g).length)) {
          ...
          //add the group (the code until the next }) to the save
          block += match[1];
      ...
      //convert placeholders to @@-prefixed so the selector regex ([^@]*?) skips them
      for (var i = 0; i < save.length; i++) {
        css = css.replace("##BLOCK_" + i + "##","@@BLOCK_" + i + "@@");
      }
      for (var i = 0; i < atDecls.length; i++) {
        css = css.replace("##DECL_" + i + "##","@@DECL_" + i + "@@");
      }
      ...
      for (var i = 0; i < save.length; i++) {
        css = css.replace("@@BLOCK_" + i + "@@",save[i]);
      }

      //reinsert @-declarations without braces
      for (var i = 0; i < atDecls.length; i++) {
        css = css.replace("@@DECL_" + i + "@@",atDecls[i]);
      }
  ...
  controlTooltip: function(element, label) {
    ...
    element.addEventListener("mouseenter",function(){ tip.style.opacity = "1"; });
    element.addEventListener("mouseleave",function(){ tip.style.opacity = "0"; });
    element.addEventListener("focus",function(){ tip.style.opacity = "1"; });
    element.addEventListener("blur",function(){ tip.style.opacity = "0"; });
