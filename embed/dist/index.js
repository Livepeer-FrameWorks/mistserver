...
var MistUtil = {
  ...
  array: {
    ...
    multiSort: function(array, sortby, getEntry) {
      if (!getEntry) getEntry = function(i2) {
        return i2;
      };
      ...
      function getValue(key, a) {
        a = getEntry(a);
        function parseIt(item, key2, sortvalue, getter) {
  ...
  event: {
    ...
    addListener: function(DOMelement, type, callback, storeOnElement) {
      ...
      if (!callback) {
        promise = {};
        promise.p = new Promise(function(resolve, reject2) {
  ...
  scripts: {
    ...
    insert: function(src, onevent, MistVideo2) {
      ...
      if (onevent.type) {
        scripttag.setAttribute("type", onevent.type);
      }
  ...
  tracks: {
    ...
    getSupported: function(trackmeta, source) {
      ...
      var result = {};
      for (var i2 in trackmeta) {
        var track = trackmeta[i2];
        if (track.type == "meta") continue;
        if (source && "sup_trk" in source) {
  ...
  shared: {
    ControlChannel: function(create_channel_func, MistVideo2, externalListenersObj) {
      var control = this;
      this.locked = false;
        ...
        if (this.readyState == "open") {
          if (queue.length) {
            for (var i2 = 0; i2 <= queue.length; i2++) {
              control.send(queue[i2], true);
            }
            queue = [];
          }
        }
        this.locked = false;
        if (this.debugging) console.log("\u{1F3AE}", "The control channel was unlocked");
      };
      this.init = function() {
        ...
        this.timeout = MistVideo2.timers.start(function() {
          if (control.readyState == "connecting") {
            ...
            if (control.debugging) console.log("\u{1F3AE}", "The control channel timed out");
        ...
        this.channel.addEventListener("close", function(ev) {
          ...
          if (control.debugging) console.log("\u{1F3AE}", "The control channel was closed", ev);
        ...
        this.channel.addEventListener("error", function(ev) {
          ...
          if (control.debugging) console.log("\u{1F3AE}", "The control channel threw an error", ev);
        ...
        this.channel.addEventListener("message", function(e) {
          ...
          if (typeof e.data == "string") {
            ...
            if (message) {
              ...
              if (control.debugging) console.info("\u{1F3AE}", "Received:", message.type, data);
      ...
      this.send = function(cmdObj, bypassLock) {
        if (!this.channel || this.readyState != "open") {
          queue.push(cmdObj);
          if (this.debugging) console.warn("\u{1F3AE}", "Want to send but control channel is " + this.readyState + ". Queue: " + queue.length);
          return;
        }
        if (!bypassLock && this.locked) {
          queue.push(cmdObj);
          if (this.debugging) console.warn("\u{1F3AE}", "Want to send but control channel is locked. Queue: " + queue.length, cmdObj);
          return;
        }
        var str;
        try {
        ...
        if (str) {
          ...
          if (this.debugging) console.warn("\u{1F3AE}", "Sent:", cmdObj.type, cmdObj);
        }
      };
      ...
      this.init = function(datachannel) {
        ...
        if ("readyState" in this.origin) {
          ...
          this.origin.onmessage = function(e) {
            if (converter.debugging) console.log("\u{1F500}", "Received metadata:", JSON.parse(e.data));
          };
    ...
    ControlChannelAPI: function(controller, MistVideo2, video, custom_funcs) {
      var api = this;
      ...
      this.play = function() {
            ...
            case "closed": {
              return new Promise(function(resolve, reject2) {
                controller.connect().then(function() {
                  video.play().then(resolve).catch(reject2);
                }).catch(reject2);
              });
            }
            default: {
        ...
        return new Promise(function(resolve, reject2) {
          ...
          if (controller.connecting) {
            controller.connecting.then(resolve).catch(reject2);
          } else {
            reject2();
          }
        });
      ...
      controller.control.addListener("pause", function(msg, m) {
        if (msg.reason && msg.reason == "at_dead_point") {
          MistVideo2.log("At dead point: seeking to return into buffer.");
          if (play_rate < 1) {
            controller.control.send({ type: "seek", seek_time: msg.begin + 1e3 });
      ...
      controller.control.addListener("on_time", function(msg) {
        ...
        if (MistVideo2.reporting && msg.tracks) {
          MistVideo2.reporting.stats.d.tracks = msg.tracks;
        }
      });
        ...
        if (!looping) {
          ...
          if (result instanceof Promise) {
            result.then(function() {
              ...
              MistVideo2.showError("Looping failed: " + err);
            });
          } else {
            controller.connect().then(function() {
              looping = false;
      ...
      defineProperty("playbackRate", {
        ...
        set: function(value) {
          if (value == 1) {
            if (MistVideo2.info.type != "live" || MistVideo2.options.liveCatchup && last_on_time.end - last_on_time.current < MistVideo2.options.liveCatchup * 1e3) {
              value = "auto";
            }
          }
          control.send({
      ...
      Object.defineProperty(api, "on_time", {
        get: function() {
          if (MistVideo2.info.type == "live") {
            let OnTime = function(orig) {
              var me = this;
              function wrap(key2) {
                switch (key2) {
                  case "begin":
                  case "end": {
                    Object.defineProperty(me, key2, {
                      get: function() {
                        return orig[key2] + ((/* @__PURE__ */ new Date()).getTime() - last_on_time._received.getTime());
                      }
                    });
                    break;
                  }
      ...
      if (custom_funcs) {
        for (var i2 in custom_funcs) {
          defineProperty(i2, custom_funcs[i2]);
        }
      }
    },
    BufferManager: function(controlChannel, MistVideo2, video, get) {
      var manager = this;
      this.settings = {
        bounds: {
      ...
      function TimeControl() {
        ...
        this.setSpeed = function(speed, tweak) {
          ...
          if (speed == this.speed.main && tweak == this.speed.tweak) return;
      ...
      var state = {
        //what the buffer manager is currently doing
        seeking: false,
        pending: false
      };
      ...
      this.addListener = function(type, func) {
        if (!(type in listeners)) throw "Not an event type: " + type;
      ...
      if (MistVideo2.player.api) {
        Object.defineProperty(MistVideo2.player.api, "playbackRate", {
          get: function() {
            return manager.timing.speed.main;
          }
        });
      } else {
      ...
      controlChannel.addSendListener("seek", function(msg) {
        state.seeking = true;
        if (!msg.ff_add) {
          msg.ff_add = Math.round(get.desiredBuffer);
        }
        return msg;
      });
      ...
      controlChannel.addListener("on_time", function(msg) {
        ...
        if (buffer !== null && !state.seeking && !state.pending) {
          if (buffer < desired * bounds.low && msg.play_rate_curr != "fast-forward" && timing.speed.tweak >= 1) {
            if (msg.current < msg.end) {
              state.pending = true;
              controlChannel.send({
                type: "fast_forward",
                ff_add: desired + 0
                //+0 ensures cast to number
              });
              if (timing.speed.tweak > 1) {
                timing.tweakSpeed(1);
              }
              MistVideo2.log("Our buffer (" + Math.round(buffer) + "ms) is small (<" + Math.round(desired * bounds.low) + "ms), requesting more data (+" + Math.round(desired) + "ms)..");
              var gotsetspeed = false;
              controlChannel.addListener("set_speed").then(function(m) {
                gotsetspeed = true;
                if (m.play_rate_prev == "fast-forward") {
                  controlChannel.addListener("on_time").then(function(m2) {
                    ...
                    if (buffer + increase < desired * bounds.low) {
                      ...
                      MistVideo2.log("Didn't receive enough extra data to increase our buffer (" + increase + "/" + Math.round(desired * bounds.low - buffer) + "ms): slowing down..");
                      emit("buffer_low");
                    } else {
              ...
              timing.tweakSpeed(actions.slower);
              MistVideo2.log("Our buffer (" + Math.round(buffer) + "ms) is small (<" + Math.round(desired * bounds.low) + "ms), but can't request more data: slowing down..");
              emit("buffer_low");
            }
          } else {
            if (timing.speed.tweak < 1 && buffer >= desired) {
              timing.tweakSpeed(1);
              MistVideo2.log("Our buffer (" + Math.round(buffer) + "ms) is large enough (>" + Math.round(desired) + "ms), so return to normal playback.");
              emit("buffer_ok");
            } else {
              if (MistVideo2.info.type == "live" && MistVideo2.options.liveCatchup) {
                if (msg.play_rate_curr == "auto" && timing) {
                  if (timing.speed.tweak <= 1 && buffer > desired * bounds.high) {
                    ...
                    MistVideo2.log("Our buffer (" + Math.round(buffer) + "ms) is big (>" + Math.round(desired * bounds.high) + "ms), so tweak the playback speed to catch up.");
                    emit("buffer_high");
                  } else if (timing.speed.tweak > 1 && buffer <= desired) {
                    timing.tweakSpeed(1);
                    MistVideo2.log("Our buffer (" + Math.round(buffer) + "ms) is small enough (<" + Math.round(desired) + "ms), so return to normal playback.");
                    emit("buffer_ok");
                  }
                }
                if (msg.play_rate_curr != "fast-forward") {
                  var distanceToLive = msg.end - msg.current;
                  if (distanceToLive < MistVideo2.options.liveCatchup * 1e3 && distanceToLive > Math.max(msg.jitter * 1.1, msg.jitter + 250) && buffer - desired < 1e3) {
                    controlChannel.send({
                      type: "fast_forward",
                      ff_add: 5e3
    ...
    DesiredBuffer: function(factors, clamp) {
      ...
      if (factors) {
        for (var i2 in factors) {
          this.addFactor(i2, factors[i2]);
        }
      }
      if (clamp) {
        this.clamp = {};
        if ("min" in clamp) {
          this.clamp.min = {};
          for (var i2 in clamp.min) {
            add(this.clamp.min, i2, clamp.min[i2]);
          }
        }
        if ("max" in clamp) {
          this.clamp.max = {};
          for (var i2 in clamp.max) {
            add(this.clamp.max, i2, clamp.max[i2]);
          }
        }
      }
    ...
    ABRController: function(MistVideo2, getter, threshold) {
      var ABR = this;
      if (!threshold) threshold = 3;
      var api = MistVideo2.player.api;
      this.current = {
        size: null,
        bitrate: null
      };
      ...
      this.request = function(type, value) {
        ...
        if (current.bitrate !== null) {
          var req = current.bitrate / MistVideo2.api.playbackRate;
          try {
            var trackidx = MistVideo2.reporting.stats.d.tracks;
            var meta2 = MistVideo2.info.meta.tracks;
            for (var i2 in meta2) {
              var t = meta2[i2];
      ...
      function BitMonitor() {
        ...
        Object.defineProperty(api, "maxBps", { get: function() {
          return bm.peak;
        } });
      }
      ...
      Object.defineProperty(this, "badness", {
        ...
        set: function(value) {
          badness = value;
          if (!MistVideo2.options.ABR_bitrate) return;
    ...
    testRTC: function(trackmeta) {
      ...
      for (var type in check) {
        ...
        for (var i2 in supported) {
          var codec = supported[i2].mimeType.toLowerCase();
      ...
      var canplay = MistUtil.event.addListener(MistVideo2.video, "canplay", function() {
        if (MistVideo2.api && MistVideo2.api.paused) {
          let autoplayFailed = function() {
            ...
            if ("localStorage" in window && localStorage != null && storedMutedValue !== false) {
...
var playbackBlueprints = {
  progress: function() {
    ...
    function getBufferWindow() {
      if ("on_time" in MistVideo2.player.api) {
        var ot = MistVideo2.player.api.on_time;
        return (ot.end - ot.begin) * 1e-3;
      }
    ...
    container.updateBar = function(currentTime) {
      if (this.kids.bar) {
        ...
        this.kids.bar.style.transform = "scaleX(" + w + ")";
    ...
    MistUtil.event.addListener(margincontainer, "mousedown", function(e) {
      ...
      dragging = true;
      container.kids.bar.style.transition = "none";
      ...
      var upListener = MistUtil.event.addListener(document, "mouseup", function(e2) {
        ...
        dragging = false;
        container.kids.bar.style.transition = "";
  ...
  speaker: function() {
    ...
    MistVideo2.playerState.on("muted", function(isMuted) {
      ...
      try {
        localStorage["mistMuted"] = isMuted ? "1" : "0";
      } catch (e) {
      }
    });
    ...
    var initevent = MistUtil.event.addListener(MistVideo2.video, "loadedmetadata", function() {
      if (!MistVideo2.options.muted && "localStorage" in window && localStorage != null && "mistMuted" in localStorage) {
        MistVideo2.api.muted = localStorage["mistMuted"] == "1";
      }
...
var mediaBlueprints = {
  ...
  keyControls: function() {
    ...
    function snapRate(rate) {
      ...
      return { index: Number(index), rate: speeds[index], speeds };
    }
    function snapVolume(volume) {
      ...
      for (var i2 in values) {
        var val = values[i2];
        if (volume < val) {
          index = i2 > 0 && val - volume > volume - values[i2 - 1] ? i2 - 1 : i2;
          break;
        }
      }
      return { index: Number(index), val: values[index], values };
    }
    var keepTimer = false;
    ...
    MistUtil.event.addListener(MistVideo2.options.keyControls == "focus" ? MistVideo2.container : document.body, "keydown", function(e) {
      ...
      if (matchesAction(e.key, "togglePlay")) {
        ...
        var target = current;
        if (snap.index + 1 < snap.values.length) target = snap.values[snap.index + 1];
        ...
        var snap = snapVolume(current);
        var target = current;
        if (snap.index - 1 >= 0) target = snap.values[snap.index - 1];
...
MistSkins.dev = {
  ...
  blueprints: {
      ...
      if (MistVideo2.player.api) {
        let processMultiOutput = function(values, formatter) {
          function getFormatter(label) {
            ...
            if ("default" in formatter) return formatter["default"];
            return formatter;
          }
          ...
          for (var i3 in values) {
            if (values[i3]) {
              ...
              c.appendChild(v);
              v.appendChild(document.createTextNode(getFormatter(i3)(values[i3])));
              out.appendChild(c);
            }
          }
        ...
        var videovalues = {
          ...
          "tracks playing": function() {
            if (MistVideo2.reporting && MistVideo2.reporting.stats && MistVideo2.reporting.stats.d && MistVideo2.reporting.stats.d.tracks) {
              var trackidxs2 = MistVideo2.reporting.stats.d.tracks;
              if (MistVideo2.info && MistVideo2.info.meta && MistVideo2.info.meta.tracks) {
                var out = {};
                for (var i3 in MistVideo2.info.meta.tracks) {
                  var track = MistVideo2.info.meta.tracks[i3];
                  if (trackidxs2.indexOf(track.idx) > -1) out[track.type] = track;
                }
                return processMultiOutput(out, function(track2) {
                  return track2.codec + " (" + track2.idx + ")";
                });
              }
              return trackidxs2.join(", ");
            }
            return "";
          },
          ...
          "current bitrate": function() {
            ...
            if (MistVideo2.player.monitor && "currentBps" in MistVideo2.player.monitor) {
              out = MistVideo2.player.monitor.currentBps;
            }
            if (MistVideo2.player.api && "currentBps" in MistVideo2.player.api) {
              out = MistVideo2.player.api.currentBps;
            }
            ...
            if (out) {
              return {
                x: (/* @__PURE__ */ new Date()).getTime() * 1e-3,
                y: out,
                options: { y: { min: 0 }, x: { count: 10 } },
                val: MistUtil.format.bits(out) + "ps"
              };
            }
            return out;
          },
          "Max bitrate": function() {
            if (MistVideo2.player.api && "maxBps" in MistVideo2.player.api) {
              var out = MistVideo2.player.api.maxBps;
              return out ? MistUtil.format.bits(out) + "ps" : out;
            }
          },
          "framerate in": function() {
            if (MistVideo2.player.api && "framerate_in" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.framerate_in(), {
                "default": function(v) {
                  return MistUtil.format.number(v) + "p/s";
                },
                "video": function(v) {
                  return MistUtil.format.number(v) + "fps";
                }
              });
            }
          },
          "Decoder fps": function() {
            if (MistVideo2.player.api && "framerate_decoder" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.framerate_decoder(), {
                "default": function(v) {
                  return MistUtil.format.number(v) + "p/s";
                },
                "video": function(v) {
          ...
          "framerate out": function() {
            if (MistVideo2.player.api && "framerate_out" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.framerate_out(), {
                "default": function(v) {
                  return MistUtil.format.number(v) + "p/s";
                },
                "video": function(v) {
          ...
          "Queue (in)": function() {
            if (MistVideo2.player.api && "receiveQueue" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.receiveQueue, MistUtil.format.number);
            }
          },
          "Queue (decoder)": function() {
            if (MistVideo2.player.api && "decodeQueue" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.decodeQueue, MistUtil.format.number);
            }
          },
          "Display buffer": function() {
            if (MistVideo2.player.api && "displayBuffer" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.displayBuffer, function(v) {
                return MistUtil.format.number(v) + "ms";
              });
            }
          },
          "Decode Time": function() {
            if (MistVideo2.player.api && "decodeTime" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.decodeTime, function(v) {
                return MistUtil.format.number(v) + "ms";
              });
            }
          },
          "Display Time": function() {
            if (MistVideo2.player.api && "displayTime" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.displayTime, function(v) {
                return MistUtil.format.number(v) + "ms";
              });
            }
          },
          "Audio/Video sync": function() {
            if (MistVideo2.player.api && "sync" in MistVideo2.player.api) return MistUtil.format.number(MistVideo2.player.api.sync) + "ms";
          },
          "Earliness": function() {
            if (MistVideo2.player.api && "earliness" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.earliness, function(v) {
                return MistUtil.format.number(v) + "ms";
              });
            }
          },
          "Server delay": function() {
            if (MistVideo2.player.api && "server_delay" in MistVideo2.player.api) return MistUtil.format.number(MistVideo2.player.api.server_delay) + "ms";
          },
          "Local jitter": function() {
            if (MistVideo2.player.api && "local_jitter" in MistVideo2.player.api) return MistUtil.format.number(MistVideo2.player.api.local_jitter) + "ms";
          },
          "Timestamp shift": function() {
            if (MistVideo2.player.api && "shift" in MistVideo2.player.api) {
              return processMultiOutput(MistVideo2.player.api.shift, function(v) {
                return MistUtil.format.number(v) + "\u03BCs";
              });
            }
          },
          "Buffering": function() {
            if (MistVideo2.player.api && "buffer_manager" in MistVideo2.player.api) {
              var bm = MistVideo2.player.api.buffer_manager;
              if (bm) {
                var current = bm.buffer();
                return {
                  x: (/* @__PURE__ */ new Date()).getTime() * 1e-3,
                  y: current,
                  options: { y: { min: 0 }, x: { count: 60 } },
                  val: Math.round(current) + "/" + Math.round(bm.desiredBuffer) + "ms"
                };
              }
              return 0;
            }
          ...
          "Speed": function() {
            if (MistVideo2.player.api && "buffer_manager" in MistVideo2.player.api) {
              if (!MistVideo2.player.api.buffer_manager) return 0;
              var speed = MistVideo2.player.api.buffer_manager.timing.speed;
              return processMultiOutput({
                "\u25B6\uFE0F playback": speed.combined,
                "\u2B07\uFE0F download": speed.main
              }, function(v) {
                return v + "\xD7";
              });
            }
      ...
      var menu = createMenu(MistVideo2, {
        ...
        onChange: function(val) {
          ...
          if (MistVideo2.options.forcePlayer != MistVideo2.playerName) {
            if (MistVideo2.options.startCombo) MistVideo2.options.startCombo = false;
    ...
    debuglevel: function() {
      ...
      var container = document.createElement("label");
      var label = document.createElement("span");
      label.appendChild(document.createTextNode("Debug level: "));
      container.appendChild(label);
      var select = document.createElement("select");
      var opts = { "false": "Quiet (default)", "true": "Some", "verbose": "Verbose" };
      for (var key in opts) {
        var option = document.createElement("option");
        option.value = key;
      ...
      select.addEventListener("change", function() {
        MistVideo2.player.debugging = this.value == "false" ? false : this.value == "true" ? true : this.value;
      });
      select.value = MistVideo2.player.debugging === true ? "true" : MistVideo2.player.debugging || "false";
      container.appendChild(select);
      return container;
    }
  }
...
function MistVideo(streamName, options) {
  ...
  options = MistUtil.object.extend({
    ...
    liveCatchup: 60,
    //when the player supports it, and the playback position is within 60 seconds from live, play slightly faster to catch up TODO
  ...
  this.checkCombo = function(options2, quiet) {
    ...
    function Combo(sourceIndex, playerIndex) {
      ...
      if ("score" in this.getSource()) {
        for (var i3 in this.getSource().score) {
          this.score_breakdown.scores[i3] = this.getSource().score[i3];
        }
      }
      if ("scores_result" in this.getSource()) {
        for (var i3 in this.getSource().scores_result) {
          this.score_breakdown.final[i3] = this.getSource().scores_result[i3];
        }
      }
      var scoring_variables = ["recovery", "cpu_viewer"];
      if ("getScore" in this.getPlayer()) {
        let getFactor = function(varname2) {
          if (MistVideo2.info && MistVideo2.info.capa && MistVideo2.info.capa.weights) {
            var factors = MistVideo2.info.capa.weights;
            if (varname2 == "cpu_viewer") {
              return MistValues.battery === true ? factors.cpu_viewer_batt : factors.cpu_viewer_pwrd;
            } else return factors[varname2] || 1;
          }
          return 1;
        };
        for (var i3 in scoring_variables) {
          var varname = scoring_variables[i3];
          var score = this.getPlayer().getScore(varname, this.getSource()) || 0;
          var factor = getFactor(varname);
          var weighted = score * factor / 10;
          this.score += weighted;
      ...
      this.canPlay = function() {
        if (this.canplay === null) {
          var tracktypes = this.getPlayer().isBrowserSupported(this.getSource().type, this.getSource(), MistVideo2);
          this.canplay = MistVideo2.scoreCombo(tracktypes);
        }
        return this.canplay;
      };
    ...
    for (var i2 in sources) {
      ...
      for (var j in players) {
        ...
        if (player2.isMimeSupported(mime)) {
          listofcombos.push(new Combo(i2, j));
        }
      }
    }
    var sortoptions = [];
    if (options2.forcePriority) {
      let addGetter = function(sortrules, get) {
        var out = [];
        for (var i3 in sortrules) {
          var v = sortrules[i3];
          if (typeof v == "function") {
            ...
            out.push(function(a) {
              return v in get(a) ? get(a)[v] : null;
            });
          }
        }
        return out;
      }, addSourceSorter = function() {
        if ("source" in options2.forcePriority) {
          ...
          sortoptions = sortoptions.concat(addGetter(options2.forcePriority.source, function(a) {
            return a.getSource();
          }));
        }
      }, addPlayerSorter = function() {
        if ("player" in options2.forcePriority) {
          ...
          sortoptions = sortoptions.concat(addGetter(options2.forcePriority.player, function(a) {
            return a.getPlayer();
          }));
        }
      };
      if (options2.forcePriority.first == "player") {
        addPlayerSorter();
        addSourceSorter();
      } else {
        addSourceSorter();
    ...
    var maxscore = MistVideo2.scoreCombo(MistUtil.object.keys(hastracktypes));
    var i2 = 0;
    if (options2.startCombo) {
      if ("list" in options2.startCombo) {
        var desc = options2.startCombo.list[options2.startCombo.start]._;
        var starters = listofcombos.filter(function(a) {
          return a._ == desc;
        });
        if (starters.length == 0) {
          MistVideo2.log("The list of combo has changed since startCombo was set. Resetting startCombo.");
          options2.startCombo = false;
        } else {
          i2 = listofcombos.indexOf(starters[0]);
        }
      } else {
        options2.startCombo.start = null;
        while (i2 < listofcombos.length) {
          var combo = listofcombos[i2];
          if (combo.getPlayer().shortname == options2.startCombo.player && combo.sourceIndex == options2.startCombo.source) {
            i2++;
            options2.startCombo.start = i2;
            break;
          }
          i2++;
        }
        if (options2.startCombo.start === null) {
          i2 = 0;
          options2.startCombo.start = 0;
        }
      }
    }
    if (debug) console.warn("Sorting of combos:\n" + listofcombos.map(function(a) {
      return a.toString();
    }).join("\n"), "\nStarting at " + i2, listofcombos);
    while (i2 < listofcombos.length) {
      var combo = listofcombos[i2];
      switch (combo.canPlay()) {
        ...
        case 0: {
          listofcombos.splice(i2, 1);
          if (!quiet) MistVideo2.log("Rejected " + combo + ": No playable tracks");
          break;
        }
        default: {
          if (combo.canPlay() >= maxscore) return combo.report();
          if (!quiet) MistVideo2.log("Skipped " + combo + " for now: canPlay " + combo.canPlay() + " < " + maxscore);
          i2++;
        }
      }
    }
    MistVideo2.log("No combo's can play all tracks, looking for highest score..");
    ...
    var i2 = 0;
    if (options2.startCombo.start) {
      i2 = options2.startCombo.start;
    }
    while (i2 < listofcombos.length) {
      var combo = listofcombos[i2];
  ...
  this.scoreCombo = function(tracktypes) {
    ...
    var scores = {
      ...
      audio: 1,
      subtitle: 0
      //subtitles can always be added seperately, so they are irrelevant
    };
    var score = 0;
    for (var i2 in tracktypes) {
      if (tracktypes[i2] in scores) score += scores[tracktypes[i2]];
    }
    return score;
  };
  this.choosePlayer = function() {
    ...
    var player2 = mistplayers[result.player];
    var source = result.source;
    MistVideo2.log("Selected: " + player2.name + " with " + source.type + " @ " + source.url + (result.score ? " (CanPlay: " + result.canplay + ", Score: " + result.score + ")" : ""));
    ...
    if (MistVideo2.choosePlayer()) {
      ...
      MistVideo2.player.build(MistVideo2, function(video) {
        ...
        if ("api" in MistVideo2.player) {
          ...
          if ("setSize" in MistVideo2.player.api) MistVideo2.player.setSize = MistVideo2.player.api.setSize;
        }
...
MistValues.battery = "getBattery" in navigator ? (function() {
  navigator.getBattery().then(function(d) {
    ...
    MistValues.battery = null;
  });
  return null;
})() : null;

// src/core/shared.js
var ControlChannel = MistUtil.shared.ControlChannel;
var DataChannel2WebSocket = MistUtil.shared.DataChannel2WebSocket;
var ControlChannelAPI = MistUtil.shared.ControlChannelAPI;
var BufferManager = MistUtil.shared.BufferManager;
var DesiredBuffer = MistUtil.shared.DesiredBuffer;
var ABRController = MistUtil.shared.ABRController;
var testMediaSource = MistUtil.shared.testMediaSource;
var testRTC = MistUtil.shared.testRTC;
...
registerWrapper("html5", {
  ...
  isBrowserSupported: function(mimetype, source, MistVideo2) {
    ...
    try {
      ...
      var supported = MistUtil.tracks.getSupported(MistVideo2.info.meta.tracks, source);
      for (var i2 in supported) {
        codecs[MistUtil.tracks.translateCodec(supported[i2])] = supported[i2];
      }
...
p2.prototype.build = function(MistVideo2, callback) {
  ...
  function getLastMs() {
    ...
    if (MistVideo2.info && MistVideo2.info.meta && MistVideo2.info.meta.tracks) {
      for (var i3 in MistVideo2.info.meta.tracks) {
        var value = Number(MistVideo2.info.meta.tracks[i3].lastms);
        if (isFinite(value)) {
          lastms = Math.max(lastms, value);
        }
      }
    }
    if (isFinite(lastms)) {
    ...
    var infoLastMs = MistVideo2.info ? Number(MistVideo2.info.lastms) : NaN;
    if (isFinite(infoLastMs)) {
      return infoLastMs;
    }
    return lastms;
  }
  function updateTimelineOffset() {
    ...
    if (timelineOffsetSource == "program-date-time") {
      return timelineOffset;
    }
    if (timelineOffsetSource == "api-live-edge") {
      return timelineOffset;
    }
  ...
  function mediaDuration() {
    if (MistVideo2.info.type == "live") {
      var details = me.hls && me.hls.latestLevelDetails;
      if (details && isFinite(details.edge) && details.edge) {
        return details.edge;
      }
  ...
  if (MistVideo2.info.type == "live" && "Proxy" in window && "Reflect" in window) {
    var overrides = { get: {}, set: {} };
    this.api = new Proxy(video, {
      get: function(target, key) {
        if (key in overrides.get) {
          return overrides.get[key].call(target);
        }
    ...
    overrides.set.currentTime = function(value) {
      MistVideo2.log("Seeking to " + MistUtil.format.time(value) + " (" + Math.round((value - me.api.duration) * -10) / 10 + "s from live)");
    ...
    overrides.get.buffered = function() {
      ...
      return {
        ...
        start: function(i3) {
          return buffered.start(i3) + offset;
        },
        end: function(i3) {
          return buffered.end(i3) + offset;
        }
      };
    };
    this.api.lastProgress = /* @__PURE__ */ new Date();
  ...
  function updateProgress() {
    MistVideo2.player.api.lastProgress = /* @__PURE__ */ new Date();
    if (MistVideo2.info.type == "live" && video.seekable.length) {
      var i3 = video.seekable.length - 1;
      var end = Math.max(video.seekable.end(i3), mediaDuration());
      MistVideo2.info.meta.buffer_window = (end - video.seekable.start(i3)) * 1e3;
    }
  }
...
p3.prototype.build = function(MistVideo2, callback) {
  ...
  this.onDashLoad = function() {
    ...
    player2.initialize(ele, MistVideo2.source.url, MistVideo2.options.autoplay);
    me.dash = player2;
...
registerWrapper("videojs", {
  ...
  isBrowserSupported: function(mimetype, source, MistVideo2) {
    ...
    if (!("customElements" in window)) {
      MistVideo2.log("Video.js 10 requires browser support for custom elements");
      return false;
    }
    var supported = MistUtil.tracks.getSupported(MistVideo2.info.meta.tracks, source);
    supported = MistUtil.shared.testMediaSource(supported);
...
p4.prototype.build = function(MistVideo2, callback) {
  var me = this;
  var ele;
  var isDash = MistVideo2.source.type == "dash/video/mp4";
  var isLive = MistVideo2.info.type == "live";
  function hasVideoJS() {
    return window.customElements && customElements.get("video-player") && customElements.get("live-video-player") && customElements.get("video-skin") && customElements.get("live-video-skin") && customElements.get("media-container") && customElements.get(isDash ? "dash-video" : "hlsjs-video");
  }
  function onVideoJSLoad() {
    ...
    if (!hasVideoJS()) {
      MistVideo2.showError("Video.js 10 loaded without registering the required player elements");
      return;
    }
    MistVideo2.log("Building Video.js 10 player..");
    ele = document.createElement(isDash ? "dash-video" : "hlsjs-video");
    ele.setAttribute("crossorigin", "anonymous");
    ele.setAttribute("playsinline", "");
    ele.setAttribute("src", MistVideo2.source.url);
    me.source = ele;
    me.mediaElement = ele.target;
    me.displayElement = ele;
    if (!me.mediaElement) {
      MistVideo2.showError("Video.js 10 loaded without creating its native video element");
      return;
    }
    if (MistVideo2.options.autoplay) {
      ele.setAttribute("autoplay", "");
    ...
    if (MistVideo2.options.controls == "stock") {
      ...
      if (MistVideo2.options.poster) {
        ...
        poster.setAttribute("src", MistVideo2.options.poster);
    ...
    function getFirstMs() {
      var firstms2 = Infinity;
      if (!MistVideo2.info || !MistVideo2.info.meta || !MistVideo2.info.meta.tracks) {
        return firstms2;
      }
      for (var i2 in MistVideo2.info.meta.tracks) {
        var value = Number(MistVideo2.info.meta.tracks[i2].firstms);
        if (isFinite(value)) {
          firstms2 = Math.min(firstms2, value);
        }
      }
      return firstms2;
    }
    function getLastMs() {
      ...
      if (MistVideo2.info && MistVideo2.info.meta && MistVideo2.info.meta.tracks) {
        for (var i2 in MistVideo2.info.meta.tracks) {
          var value = Number(MistVideo2.info.meta.tracks[i2].lastms);
          if (isFinite(value)) {
            lastms = Math.max(lastms, value);
          }
        }
    ...
    if ("Proxy" in window && "Reflect" in window) {
      ...
      overrides.get.buffered = function() {
        ...
        return {
          length: buffered.length,
          start: function(i2) {
            return buffered.start(i2) + offset;
          },
          end: function(i2) {
            return buffered.end(i2) + offset;
          }
        };
      };
    } else {
    ...
    me.api.unload = function() {
      ...
      MistVideo2.log("Video.js 10 instance disposed");
    };
    function updateProgress() {
      me.api.lastProgress = /* @__PURE__ */ new Date();
      if (isLive && ele.seekable.length) {
        var i2 = ele.seekable.length - 1;
        var end = Math.max(ele.seekable.end(i2), mediaDuration());
        MistVideo2.info.meta.buffer_window = (end - ele.seekable.start(i2)) * 1e3;
      }
    }
    ...
    MistVideo2.log("Built Video.js 10 elements");
...
registerWrapper("webrtc", {
  ...
  isBrowserSupported: function(mimetype, source, MistVideo2) {
    ...
    var supported = MistUtil.tracks.getSupported(MistVideo2.info.meta.tracks, source);
    supported = MistUtil.shared.testRTC(supported);
    return MistUtil.tracks.tracktypes(supported);
  },
  player: function() {
  },
...
p5.prototype.build = function(MistVideo2, callback) {
  ...
  function myRTC() {
    ...
    this.connect = function() {
      ...
      if (MistVideo2.info.type == "live") {
        this.control.addListener("on_time", function(msg) {
          if (msg.play_rate_curr) {
            switch (msg.play_rate_curr) {
              case "auto": {
                if (!MistVideo2.options.liveCatchup || msg.end - msg.current > MistVideo2.options.liveCatchup * 1e3) {
      ...
      this.connection.onconnectionstatechange = function(e) {
        if (MistVideo2.destroyed) new Promise(function(resolve, reject2) {
          reject2();
        });
  ...
  this.ABR = new MistUtil.shared.ABRController(MistVideo2, {
    bitCounter: function() {
      var total = 0;
...
p7.prototype.build = function(MistVideo2, callback) {
  ...
  function WSMP4() {
    ...
    this.createControlChannel = function() {
      controller.control = new MistUtil.shared.ControlChannel(function() {
        var ws = new WebSocket(MistVideo2.source.url);
        ws.binaryType = "arraybuffer";
        return ws;
      }, MistVideo2, controller.onmessage);
      controller.connection = controller.control;
      ...
      controller.control.addListener("channel_close").then(function() {
        ...
        if (controller.control.was_connected && controller.control.readyState != "closed" && !MistVideo2.destroyed && (!controller.sb || !controller.sb.paused) && MistVideo2.state == "Stream is online" && !(MistVideo2.video && MistVideo2.video.error)) {
      ...
      controller.control.addSendListener("seek", function(msg) {
        ...
        if (value != "live") video.currentTime = value;
        if (main.debugging) console.warn("[Seeking]", "[1/5]", MistUtil.format.time(value, { ms: true }), "Command sent");
        controller.control.addListener("seek").then(function(msg2) {
          ...
          if (main.debugging) console.warn("[Seeking]", "[3/5]", MistUtil.format.time(value, { ms: true }), "First on_time received", { begin: MistUtil.format.time(msg2.begin * 1e-3), current: MistUtil.format.time(msg2.current * 1e-3) });
          if (value != "live" && msg2.begin > value * 1e3) {
            value = msg2.begin;
          ...
          return new Promise(function(resolve, reject2) {
            ...
            seekprom.then(function() {
              MistUtil.event.removeListener(evtl);
              reject2("Cancelling old seek to " + MistUtil.format.time(value, { ms: true }) + " because a new seek was requested");
            }).catch(function() {
            });
          });
        }).then(function(buffern) {
          seeking = false;
          if (value) video.currentTime = value;
      ...
      controller.control.addListener("tracks", function(msg) {
        ...
        if (checkEqual(controller.ms.codecs, msg.codecs)) {
          ...
          new Promise(function(resolve, reject2) {
            ...
            MistVideo2.log("Track switch failed: " + e);
          });
        }
      });
    };
    ...
    this.connect = function() {
      ...
      this.connecting = new Promise(function(resolve, reject2) {
        var control = controller.control;
        ...
        controller.bm = new MistUtil.shared.BufferManager(controller.control, MistVideo2, video, {
          //getter
          desiredBuffer: new MistUtil.shared.DesiredBuffer({
            //all buffer components in ms
            base: MistVideo2.info.type == "live" ? 100 : 500,
            //never changes
            keepAway: 500,
            //slowly decays by keepAwayDecay every on_time if buffer state is ok, increases when waiting event is triggered
        ...
        channelReady.then(function() {
          ...
          controller.control.send({ type: "request_codec_data", supported_codecs: MistVideo2.source.supportedCodecs }, true);
  ...
  function myMediaSource() {
    ...
    function mySourceBuffer(codecs) {
      var msb = this;
      var sb = self.ms.addSourceBuffer("video/" + MistVideo2.source.type.split("/")[2] + ';codecs="' + codecs.join(",") + '"');
      sb.mode = "segments";
      this.sb = sb;
      this.queue = [];
      this.onupdateend = [];
      ...
      sb.addEventListener("updateend", function() {
        ...
        var do_funcs = msb.onupdateend.slice();
        msb.onupdateend = [];
        for (var i2 in do_funcs) {
          if (!sb) {
            if (main.debugging) {
              console.warn("I was doing onupdateend but the sb was reset");
            }
            break;
          }
          if (sb.updating) {
            msb.onupdateend.concat(do_funcs.slice(i2));
            if (player.debugging) {
              console.warn("I was doing onupdateend but was interrupted");
            }
            break;
          }
          do_funcs[i2](i2 < do_funcs.length - 1 ? do_funcs.slice(i2) : []);
        }
        if (!sb) return;
        msb.busy = false;
      ...
      this.append = function(data) {
        ...
        if (main.debugging) {
          msb.appending = new Uint8Array(data);
        }
        if (msb.busy) {
          ...
          switch (e.name) {
            ...
            case "InvalidStateError": {
              ...
              if (MistVideo2.video.error) {
                if (MistVideo2.video.error.message.slice(0, 33) == "CHUNK_DEMUXER_ERROR_APPEND_FAILED") {
                }
                return;
              }
              break;
      ...
      if ("changeType" in sb) {
        this.setCodecs = function(codecs2) {
          sb.changeType("video/" + MistVideo2.source.type.split("/")[2] + ';codecs="' + codecs2.join(",") + '"');
          this.applyQueue();
        };
      }
      this.close = function() {
          ...
          msb.do(function(remaining_do_on_updateend) {
            ...
            if (sb.updating) {
              if (sb.abort) sb.abort();
                ...
                return msb.close().then(resolve).catch(reject2);
              }
            }
            if (main.debugging == "dl") {
              ...
              ;
              var l = 0;
              for (var i2 = 0; i2 < msb.appended.length; i2++) {
                l += msb.appended[i2].length;
              }
              var d = new Uint8Array(l);
              var l = 0;
              for (var i2 = 0; i2 < msb.appended.length; i2++) {
                d.set(msb.appended[i2], l);
                l += msb.appended[i2].length;
              }
      ...
      this.applyQueue = function() {
        ...
        if (queues && queues.length) {
          if (queues[0]) {
            var do_do = false;
            if (queues[0].length) {
              for (var i2 in queues[0]) {
                if (sb.updating || msb.queue.length || msb.busy) {
                  msb.queue.push(queues[0][i2]);
                } else {
                  msb.append(queues[0][i2]);
                }
              }
            } else {
              do_do = true;
            ...
            queues.shift();
            if (queues.length == 0) {
              main.controller.queues = false;
            }
            MistVideo2.log("The newly initialized source buffer was filled with data from a separate message queue." + (queues ? " " + queues.length + " more message queue(s) remain." : ""));
            if (do_do) {
    ...
    this.init = function() {
      return new Promise(function(resolve, reject2) {
        ...
        self.ms.onsourceopen = function() {
          if (self.codecs) {
            self.addSourceBuffer().then(resolve, reject2);
          }
        };
      });
    };
    this.setCodecs = function(codecs) {
      ...
      function typeHasChanged(arr1, arr2) {
        ...
        if (MistVideo2.info && MistVideo2.info.meta && MistVisdeo.info.meta.tracks) {
          var found = [];
          for (var i2 in MistVideo2.info.meta.tracks) {
            var t = MistVideo2.info.meta.tracks[i2];
      ...
      return new Promise(function(resolve, reject2) {
        if (!codecs || !codecs.length) return reject2("No codecs provided");
        if (self.sb) {
          if ("setCodecs" in self.sb && !typeHasChanged(old, codecs)) {
            ...
            self.sb.close().then(self.addSourceBuffer).then(resolve, reject2);
          }
        } else {
          self.addSourceBuffer().then(resolve, reject2);
        }
    ...
    this.addSourceBuffer = function() {
      return new Promise(function(resolve, reject2) {
        if (!self.codecs || !self.codecs.length) return reject2("No codecs provided");
        try {
          ...
          if (self.was_connected) {
            ...
            self.ms = false;
            video.src = "";
            self.init().then(resolve, reject2);
          } else {
            reject2("SourceBuffer initialization failed: " + e);
          }
        }
      });
    };
  ...
  this.api = new MistUtil.shared.ControlChannelAPI(this.controller, MistVideo2, video, {
    ...
    currentTime: {
      ...
      set: function(value) {
        seeking = true;
        value = value == "live" ? "live" : Math.round(value * 1e3);
  ...
  this.ABR = new MistUtil.shared.ABRController(MistVideo2, {
    bitCounter: function() {
      return main.controller.control.bitCounter;
    }
  });
  var seeking = false;
  MistUtil.event.addListener(video, "waiting", function() {
  ...
  this.findBuffer = function(position) {
    ...
    for (var i2 = 0; i2 < video.buffered.length; i2++) {
      if (video.buffered.start(i2) <= position && video.buffered.end(i2) >= position) {
        buffern = i2;
        break;
      }
    }
      ...
      case "audio": {
        switch (track.codec) {
          case "PCM": {
            ...
            switch (track.size) {
              case 32: {
                track.codecstring += "-s32";
                break;
              }
              case 24: {
                track.codecstring += "-s24";
                break;
              }
              case 16: {
                track.codecstring += "-s16";
                break;
              }
              case 8: {
                track.codecstring += "u8";
                break;
              }
            }
...
var p8 = mistplayers.rawws.player;
...
p8.prototype.build = function(MistVideo2, callback) {
  ...
  if (!window.MediaStreamTrackGenerator) {
    ...
    var PolyMediaStreamTrackGenerator = function(options) {
      ...
      if (kind == "video") {
        ...
        let toArrayBuffer = function(audioData) {
          var opts = {
            planeIndex: 0,
            format: audioData.format
          };
          var a = new Float32Array(audioData.allocationSize(opts) / 4);
          ...
          if (options.track.codec == "PCM") {
            ...
            for (let i2 = 0; i2 < int32View.length; i2++) {
              normalized[i2] = int32View[i2] / 2147483648;
            }
            return normalized;
          }
          return a;
        ...
        track.writable = new WritableStream({
          start: function(controller) {
            var me = this;
            me.arrays = [];
            var workletCode = 'function worklet() {  registerProcessor("mstg-shim", class Processor extends AudioWorkletProcessor {    constructor() {      super();      this.arrays = [];      this.arrayOffset = 0;      this.port.onmessage = (e) => { this.arrays.push(new Float32Array(e.data)); };      this.emptyArray = new Float32Array(0);    }    process(inputs, [outputs]) {      for (let i = 0; i < outputs[0].length; i++) {        for (let j = 0; j < outputs.length; j++) {          if (!this.array || this.arrayOffset >= this.array.length) {            this.array = this.arrays.shift() || this.emptyArray;            this.arrayOffset = 0;          }          outputs[j][i] = this.array[this.arrayOffset++] || 0;        }      }      return true;    }  });}';
  ...
  function RawPlayer(url) {
    ...
    function WebCodecsWorker() {
      var self = this;
      function initWorker() {
        MistVideo2.log("Loading WebCodecsWorker");
        return new Promise(function(resolve, reject2) {
          var url2 = mistplayers.rawws.scriptsrc(MistVideo2.options.host);
          try {
            self.worker = new Worker(url2);
          } catch (e) {
            if (e.name == "SecurityError") {
              ...
              fetch(url2).then(function(response) {
                ...
                try {
                  ...
                  else reject2("Failed to load WebCodecsWorker after fetch");
                } catch (e2) {
                  reject2(e2);
                }
              }).catch(reject2);
          ...
          if (!self.worker) {
            reject2("Failed to load WebCodecsWorker");
            return;
          }
          resolve();
        });
      ...
      this.workerReady = function() {
        return workerProm ? workerProm : new Promise(function(resolve, reject2) {
      ...
      workerProm = initWorker().then(function() {
        workerProm = false;
        MistVideo2.log("WebCodecsWorker was loaded succesfully");
        ...
        if (callback2) {
          ...
          return new Promise(function(resolve, reject2) {
            self2.listeners[key] = function(msg) {
              if (msg.status == "error") return reject2(msg);
              resolve(msg);
            };
          });
        }
      ...
      this.post = function(msg, opts) {
        if (!this.worker || this.worker == "pending") {
          ...
          return new Promise(function(resolve, reject2) {
            postqueue.push([[msg, opts], resolve, reject2]);
          });
        }
        if (!("uid" in msg)) {
          msg.uid = uid++;
        ...
        return new Promise(function(resolve, reject2) {
          self.addListener(msg).then(resolve).catch(reject2);
        });
      };
    ...
    try {
      ...
      mistplayers.rawws.isBrowserSupported = function() {
        return false;
      };
      MistVideo2.nextCombo(false, "failed to load webcodecs worker");
      throw e;
      return;
    }
    ...
    function Pipeline(track) {
      ...
      if (cache) cache = cache[MistVideo2.stream];
      if (cache && track.idx in cache) {
        if (!cache[track.idx]) {
          MistVideo2.log("Cancelling pipeline " + track.idx + " because this codec (" + track.codec + ") is not supported");
          return;
        }
      } else {
      ...
      let p11 = mistplayers.rawws.isTrackSupported(track);
      if (!p11) return;
      if ("then" in p11) {
        p11.then(function(result) {
          if (!result.supported) {
            MistVideo2.log("Cancelling pipeline " + track.idx + " because this codec (" + track.codec + ") is not supported");
            pipeline.close();
          }
      ...
      pipeline.createTrackGenerator = function() {
        if (!window.MediaStreamTrackGenerator && controller.worker.haveVideoTrackGenerator) {
          if (track.type == "video") {
            ...
            pipeline.trackGenerator = new PolyMediaStreamTrackGenerator({ kind: "audio", track });
          ...
          pipeline.trackGenerator = typeof MediaStreamTrackGenerator !== "undefined" ? new MediaStreamTrackGenerator({ kind: track.type }) : new PolyMediaStreamTrackGenerator({ kind: track.type, track });
          post({
            type: "setwritable",
      ...
      MistVideo2.log("Creating pipeline for track " + track.idx + " (" + track.codec + (track.codecstring ? " " + track.codecstring : "") + " " + track.type + ")");
      post({
        type: "create",
        track,
    ...
    this.connect = function() {
      ...
      this.connecting = new Promise(function(resolve, reject2) {
        ...
        controller.bm = new MistUtil.shared.BufferManager(controller.control, MistVideo2, video, {
          //getter
          desiredBuffer: new MistUtil.shared.DesiredBuffer({
            //add all these factors: all buffer components in ms
            base: MistVideo2.info.type == "live" ? 100 : 500,
            //never changes
            keepAway: 250,
            //slowly decays by keepAwayDecay every on_time if buffer state is ok, increases when waiting event is triggered
            ...
            min: {
              //the desired buffer should be at least the frame duration (for sparse tracks eg JPEG)
          ...
          buffer: function() {
            return (controller.frameTiming.decoded - controller.frameTiming.out) * 1e-3;
          },
          keepAwayDecay: 0.25,
        ...
        return controller.control.addListener("channel_open").then(function() {
          ...
          var codecs = [
            Object.values(supportedCodecs2).filter(function(a) {
              return a.length;
            })
            /*.map(function(b){
          ...
          MistVideo2.log("Requesting codecs: " + codecs.flat(2).join(", "));
          controller.connecting = false;
          ...
          MistVideo2.log("Preparing to play codecs: " + Object.values(controller.pipelines).map(function(a) {
            return a.track.codec;
          }).join(", "));
    ...
    function checkTracksAndConnect() {
      ...
      var theCache2 = thePlayer.cacheSupported[MistVideo2.stream];
      function checkTrack(i3) {
        ...
        if (idx in theCache2) {
          ...
          promises.push(mistplayers.rawws.isTrackSupported(track).then(function(result) {
            theCache2[idx] = result.supported;
            if (result.supported) {
              pushIfUnique(supportedCodecs2[track.type], track.codec);
            }
          }));
...
p9.prototype.build = function(MistVideo2, callback) {
  ...
  player2.onDecoderLoad = function() {
    ...
    player2.vars = {};
    if (MistVideo2.options.autoplay) {
      player2.vars.wantToPlay = true;
    }
    player2.dropping = false;
    player2.frames = {
      ...
      history: {
        ...
        add: function() {
          this.log.unshift({
            ...
            received: player2.frames.received,
            bitsReceived: player2.frames.bitsReceived,
            decoded: player2.frames.decoded
          });
    ...
    function init() {
      function init_decoder() {
        ...
        decoder.set_image_callback(function(image) {
          player2.frames.decoded++;
          if (player2.vars.wantToPlay && player2.state != "seeking") {
          ...
          switch (player2.state) {
            ...
            case "seeking": {
              var t = player2.frames.frame2time(player2.frames.decoded + player2.frames.dropped);
              if (t >= player2.vars.seekTo) {
                emitEvent("seeked");
                player2.vars.seekTo = null;
                player2.state = "playing";
                if (!player2.vars.wantToPlay) {
                  emitEvent("timeupdate");
                  MistVideo2.player.send({ type: "hold" });
      ...
      function connect() {
        ...
        socket.onmessage = function(e) {
          if (typeof e.data == "string") {
            ...
            switch (d.type) {
              case "on_time": {
                player2.vars.paused = false;
                player2.frames.history.add();
                if (player2.vars.duration != d.data.end * 1e-3) {
                  player2.vars.duration = d.data.end * 1e-3;
              ...
              case "info": {
                ...
                if (typeof track != void 0 && track.fpks > 0) {
                  player2.frames.rate_theoretical = track.fpks * 1e-3;
                }
                break;
              }
              case "pause": {
                player2.vars.paused = d.paused;
                if (d.paused) {
                  player2.decoder.flush();
                  emitEvent("pause");
                }
                break;
              }
              case "on_stop": {
                if (player2.state == "ended") {
                  return;
                }
                player2.state = "ended";
                player2.vars.paused = true;
                socket.onclose = function() {
                };
                socket.close();
                player2.decoder.flush();
            ...
            let prepare = function(data2, infoBytes2) {
              setTimeout(function() {
                if (player2.dropping) {
                  if (player2.state != "waiting") {
                    emitEvent("waiting");
                    player2.state = "waiting";
                  }
                  if (isKeyFrame(infoBytes2)) {
                    if (player2.dropping.length) {
                      player2.frames.dropped += player2.dropping.length;
                      MistVideo2.log("Dropped " + player2.dropping.length + " frames");
                      player2.dropping = [];
                    } else {
                    ...
                    player2.dropping.push([infoBytes2, data2]);
                    if (!decoder.decoding) {
                      var d2 = player2.dropping.shift();
                      MistVideo2.player.process(d2[1], d2[0]);
                    }
                    return;
                  }
                } else {
                  if (player2.frames.behind() > 20) {
                    player2.dropping = [];
            ...
            player2.frames.received++;
            player2.frames.bitsReceived += e.data.byteLength * 8;
            var l = 12;
            var infoBytes = new Uint8Array(e.data.slice(0, l));
            var data = new Uint8Array(e.data.slice(l, e.data.byteLength));
            player2.frames.timestamps[player2.frames.received] = toTimestamp(infoBytes);
    ...
    function reroute(item) {
      Object.defineProperty(MistVideo2.player.api, item, {
        get: function() {
          return player2.vars[item];
        },
        set: function(value) {
          return player2.vars[item] = value;
        }
      });
    }
    var list = [
    ...
    Object.defineProperty(MistVideo2.player.api, "currentTime", {
      get: function() {
        var n = player2.frames.decoded + player2.frames.dropped;
        if (player2.state == "seeking") {
          return player2.vars.seekTo;
        }
        if (n in player2.frames.timestamps) {
          return player2.frames.frame2time(n);
        }
        return 0;
      },
      set: function(value) {
        emitEvent("seeking");
        player2.state = "seeking";
        player2.vars.seekTo = value;
    ...
    Object.defineProperty(MistVideo2.player.api, "buffered", {
      get: function() {
        return {
          start: function(i3) {
            if (this.length && i3 == 0) {
              return player2.frames.frame2time(player2.frames.decoded + player2.frames.dropped);
            }
          },
          end: function(i3) {
            if (this.length && i3 == 0) {
              return player2.frames.frame2time(player2.frames.received);
            }
          },
          length: player2.frames.received - player2.frames.decoded > 0 ? 1 : 0
        };
      }
    });
    if (MistVideo2.info.type != "live") {
