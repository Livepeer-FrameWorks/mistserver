 ...
 *
 * Possible future enhancement: NDIlib_framesync_instance_t for better frame synchronization.
...
namespace Mist {
  ...
  uint64_t inNDI::ndiTimeToMs(int64_t timestamp, int64_t timecode) const {
    // INT64_MAX is the sentinel for both an undefined receiver timestamp and a synthesized
    // timecode; dividing it would produce a bogus, offset-poisoning value.
    if (timestamp != INT64_MAX && timestamp > 0) { return (uint64_t)(timestamp / 10000); }
    if (timecode != INT64_MAX && timecode > 0) { return (uint64_t)(timecode / 10000); }
    return Util::bootMS();
  }
    ...
    while (config->is_active) {
      ...
      switch (dev->receiveAny(video_frame, audio_frame, metadata_frame)) {
        ...
        case NDIlib_frame_type_status_change: {
          // NDI sources may only report their full capabilities (PTZ, tally, recording, web
          // control) a few seconds after connecting. On a status change, re-query them rather
          // than only refreshing the web-control URL.
          INFO_MSG("NDI source status changed - re-querying capabilities");
          updateWebControlUrl();
          ::Device::DeviceInfo caps = dev->queryCapabilities();
          HIGH_MSG("NDI capabilities after status change: PTZ=%s", caps.hasPTZ ? "yes" : "no");
          break;
        }
  ...
  void inNDI::bufferVideo(const NDIlib_video_frame_v2_t & frame) {
    uint64_t msTimestamp = ndiTimeToMs(frame.timestamp, frame.timecode);
    ...
    // Derive the codec label from the actual NDI FourCC (its 4 bytes ARE the pixel-format name,
    // e.g. 'U','Y','V','Y'). The receiver requests UYVY_BGRA, so opaque frames arrive as UYVY;
    // alpha frames as BGRA. Previously the codec was hardcoded "UYVY", mislabeling any non-UYVY
    // frame. MistServer's raw-video pipeline carries UYVY, so warn on anything else.
    uint32_t fourcc = (uint32_t)frame.FourCC;
    char codecStr[5] = {(char)(fourcc & 0xFF), (char)((fourcc >> 8) & 0xFF), (char)((fourcc >> 16) & 0xFF),
                        (char)((fourcc >> 24) & 0xFF), 0};

    // Check if video format has changed (resolution, framerate, or pixel format)
    if (frame.xres != lastWidth || frame.yres != lastHeight ||
        (frame.frame_rate_N * 1000 / frame.frame_rate_D) != lastFpks || fourcc != lastFourCC) {
      ...
      if (frame.FourCC != NDIlib_FourCC_video_type_UYVY) {
        WARN_MSG("NDI video FourCC '%s' is not UYVY; downstream raw-video support may be limited", codecStr);
      }
      meta.setCodec(videoTrackIdx, codecStr);

      INFO_MSG("Video format: %ux%u@%u (%s)", frame.xres, frame.yres, frame.frame_rate_N / frame.frame_rate_D, codecStr);
  ...
  void inNDI::bufferAudio(const NDIlib_audio_frame_v3_t & frame) {
    ...
    // NDI carries 32-bit float planar (FLTP) audio; MistServer PCM is interleaved big-endian
    // signed 16-bit. Delegate the conversion to the SDK (reference-level-correct), which also
    // handles the endian swap into Mist's big-endian layout.
    size_t frameSize = 0;
    const uint8_t *audioOut = dev->audioFrameToPCM16BE(frame, frameSize);
    if (!audioOut || !frameSize) {
      WARN_MSG("NDI: audio conversion failed");
      return;
    }
    ...
    bufferLivePacket(thisTime, 0, thisIdx, (const char *)audioOut, frameSize, 0, true);
  ...
  void inNDI::bufferMetadata(const NDIlib_metadata_frame_t & frame) {
    // Metadata frames carry only a timecode (no receiver timestamp).
    uint64_t msTimestamp = ndiTimeToMs(INT64_MAX, frame.timecode);
  ...
  JSON::Value inNDI::enumerateSources(const std::string & device) {
    ...
    if (sources) {
      for (size_t i = 0; i < numSources; ++i) {
        ...
        source["url"] = sources[i].p_url_address ? sources[i].p_url_address : "";
        result.append(source);
      }
    }
    return result;
  ...
  bool inNDI::checkArguments() {
    ...
    // We convert NDI's 32-bit planar float audio to interleaved 16-bit signed PCM on input.
    audioSampleDepth = 16;
  ...
  bool inNDI::openStreamSource() {
    ...
    // Now connect, retrying with exponential backoff. The NDI source may not be online yet
    // at startup; give it a bounded window rather than failing immediately. (Once connected,
    // the NDI receiver transparently re-connects to the named source if it drops.)
    {
      bool connected = false;
      uint64_t backoffMs = 200;
      for (int attempt = 0; attempt < 6 && config->is_active; ++attempt) {
        if (dev->connect()) {
          connected = true;
          break;
        }
        WARN_MSG("NDI: connect attempt %d failed, retrying in %" PRIu64 " ms", attempt + 1, backoffMs);
        Util::wait(backoffMs);
        if (backoffMs < 3200) { backoffMs *= 2; }
      }
      if (!connected) {
        Util::logExitReason(ER_READ_START_FAILURE, "Failed to connect to NDI source after retries");
        return false;
      }
    }
