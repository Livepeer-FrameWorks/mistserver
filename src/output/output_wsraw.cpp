...
namespace Mist{
  namespace{
    void appendNV12FromUYVY(Util::ResizeablePointer & out, const char *data, size_t width, size_t height){
      const size_t yPlaneSize = width * height;
      const size_t uvPlaneSize = ((width + 1) / 2) * ((height + 1) / 2) * 2;
      out.allocate(12 + yPlaneSize + uvPlaneSize);
      out.append(0, yPlaneSize + uvPlaneSize);

      size_t offset = 12;
      for (size_t i = 0; i < yPlaneSize; ++i){
        out[offset + i] = data[i * 2 + 1];
      }

      offset += yPlaneSize;
      for (size_t y = 0; y < height; y += 2){
        for (size_t x = 0; x < width; x += 2){
          const size_t p = (x + width * y) * 2;
          out[offset++] = data[p];
          out[offset++] = data[p + 2];
        }
      }
    ...
    void appendNV12FromYUYV(Util::ResizeablePointer & out, const char *data, size_t width, size_t height){
      const size_t yPlaneSize = width * height;
      const size_t uvPlaneSize = ((width + 1) / 2) * ((height + 1) / 2) * 2;
      out.allocate(12 + yPlaneSize + uvPlaneSize);
      out.append(0, yPlaneSize + uvPlaneSize);
  ...
  void OutWSRaw::sendNext(){
    ...
    const std::string & codec = M.getCodec(thisIdx);
    if (codec == "UYVY"){
      webBuf[1] = 1;
      appendNV12FromUYVY(webBuf, dataPointer, M.getWidth(thisIdx), M.getHeight(thisIdx));
    }else if (codec == "YUYV"){
      webBuf[1] = 1;
      appendNV12FromYUYV(webBuf, dataPointer, M.getWidth(thisIdx), M.getHeight(thisIdx));
    }else{
      if (codec == "NV12"){webBuf[1] = 1;}
