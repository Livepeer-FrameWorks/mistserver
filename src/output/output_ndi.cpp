...
namespace Mist {
  ...
  void OutNDI::sendNext() {
    ...
    if (M.getType(thisIdx) == "video") {
      ...
      // Let NDI synthesize monotonic timecodes (100ns units). Mist timestamps are in ms and
      // are not in NDI's 100ns timebase, so passing them directly produced wrong timecodes.
      // (.timestamp is a receive-only field and is left zero on send.)
      ...
      // Synchronous send: NDI consumes the frame before returning, so the packet buffer
      // (dataPointer) does not need to outlive this call. (Async send would require us to
      // keep the pixel memory valid until the next sync - we intentionally avoid that.)
    ...
    if (M.getType(thisIdx) == "audio") {
      uint32_t channels = M.getChannels(thisIdx);
      uint32_t bits = M.getSize(thisIdx);
      if (channels == 0 || bits == 0) {
        WARN_MSG("Invalid audio format: %u channels, %u-bit", channels, bits);
        return;
      }
      // NDI carries 32-bit float planar audio; Mist PCM is interleaved big-endian signed integer.
      // sendAudioPCM16BE handles the endian swap + the SDK's reference-level-correct conversion.
      // Only 16-bit PCM is supported here (the common case); flag anything else instead of
      // silently sending mis-scaled audio.
      if (bits != 16) {
        WARN_MSG("NDI output supports 16-bit PCM audio only (got %u-bit); dropping audio frame", bits);
        audioFramesDropped++;
        return;
      }
      if (dev.sendAudioPCM16BE((const uint8_t *)dataPointer, dataLen, M.getRate(thisIdx), channels)) {
        audioFramesSent++;
    ...
    if (now > lastMetricsUpdate + 5) {
      // Write prometheus-style metrics into a per-user directory (0700) rather than the shared
      // world-writable /tmp namespace, so the predictable metrics filename cannot be squatted or
      // redirected by other local users.
      std::string metricsDir = "/tmp/mistserver-" + std::to_string(geteuid());
      mkdir(metricsDir.c_str(), 0700); // ignore EEXIST; validated below
      // Refuse to use the directory unless it is a real (non-symlink) directory that we own and
      // that is not group/other-writable - otherwise another local user pre-created it.
      struct stat mdStat;
      if (lstat(metricsDir.c_str(), &mdStat) != 0 || !S_ISDIR(mdStat.st_mode) ||
          mdStat.st_uid != geteuid() || (mdStat.st_mode & (S_IWGRP | S_IWOTH))) {
        WARN_MSG("NDI: %s is not a secure self-owned directory; skipping metrics write", metricsDir.c_str());
        lastMetricsUpdate = now;
        return;
      }
      char tmpFileName[256];
      snprintf(tmpFileName, sizeof(tmpFileName), "%s/mistXXXXXX", metricsDir.c_str());
      ...
      if (fd != -1) {
        ...
        std::string targetFileName = metricsDir + "/" + safeName + "_ndi_metrics.txt";
