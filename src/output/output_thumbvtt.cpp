...
namespace Mist {
  ...
  bool OutThumbVTT::isReadyForPlay() {
    return true;
  }
  ...
  void OutThumbVTT::onHTTP() {
    ...
    if (H.GetVar("track").size()) {
      ...
      if (M.getValidTracks().count(tid)) { vttTrack = tid; }
    }
    ...
    userSelect.clear();
    if (vttTrack != INVALID_TRACK_ID) { userSelect[vttTrack].reload(streamName, vttTrack); }
    if (pushMode && spriteTrack != INVALID_TRACK_ID) { userSelect[spriteTrack].reload(streamName, spriteTrack); }
