...
class binding {
  public:
    ...
    bool update(const std::string & name, const std::string & phost, const std::string & _prot, uint64_t currTime,
                Socket::UDPConnection & U) {
      prot = _prot;
      bool ret = false;
      if (phost.size() == 6) {
        ret = (phost != porthost4);
...
void addListEntry(std::string & reply, const std::string & name, const binding & B, const std::string & phost, size_t & currEntry) {
  if (reply.size() == 25) { Foghorn::writeUint16(reply, 21, currEntry); }
  const size_t nameLength = std::min<size_t>(name.size(), 100);
  const size_t protocolLength = std::min<size_t>(B.prot.size(), 100);
  size_t len = 3 + phost.size() + nameLength + protocolLength;
  reply += (char)len;
  reply += (char)nameLength;
  reply.append(name.data(), nameLength);
  reply += (char)protocolLength;
  reply.append(B.prot.data(), protocolLength);
...
int main(int argc, char **argv) {
  ...
  uint64_t rangeSize = 65535 - 1025 + 1;
  ...
  if (conf.getOption("range").asStringRef().size()) {
    ...
    if (dash != std::string::npos) {
      ...
      if (numA && numB && numA <= numB) {
        ...
        rangeBegin = numA;
        rangeSize = numB - numA + 1;
      }
    }
  }

  if (conf.getOption("altserver").asStringRef().size()) { altserver = conf.getOption("altserver").asStringRef(); }
    ...
    while (conf.is_active) {
      ...
      while (upnp.Receive()) {
        if (upnp.data.size()) {
          ...
          if (P.Read(str)) { gateways.insert(P.GetHeader("location")); }
        }
      }
    }
  ...
  do {
    ...
    if (r > 0) {
      while (U.Receive()) {
        ...
        if (U.data[20] == 0) { // Announce public port mapping
          ...
          if (!Foghorn::parseAnnouncement(U.data, U.data.size(), announcement)) { continue; }
          const std::string & prot = announcement.protocol;
          const std::string & name = announcement.path;
          ...
          bool updated = false;
          if (B.update(name, ph, prot, currTime, U)) { updated = true; }
          // Do we know the internal port?
          if (announcement.hasLocalPort) { B.predictablePort = (announcement.localPort == port); }
          if (announcement.hasFlags) {
            B.isOpen = (announcement.flags & 0x01);
            B.consistentPort = (announcement.flags & 0x02);
          }
          if (updated) {
            INFO_MSG("📣 Ahoy mate! Spotted %s %s over %s on %s", B.firewallName(), name.c_str(), prot.c_str(),
                     U.getRemoteAddr().toString().c_str());
          }
        } else if (U.data[20] == 2) { // Hole punching request
          Foghorn::RelayRequest request;
          if (!Foghorn::parseRelayRequest(U.data, U.data.size(), request)) { continue; }
          const std::string & prot = request.protocol;
          const std::string & name = request.path;
          const std::string & addi = request.additionalData;
          bool reqIsPredictable = (U.getRemoteAddr().port() == request.localPort);
          uint8_t fireType = 0;
          uint8_t bits = request.flags | 0x80; // set MSB to indicate we have received bits at all
          if (!bindings.count(name)) {
            if (addi.size()) {
              INFO_MSG("🌊 Castaway! %s will not find %s (%s)!", U.getRemoteAddr().toString().c_str(), name.c_str(),
                       addi.c_str());
            } else {
          ...
          binding & B = bindings[name];
          if (B.prot != prot) {
            INFO_MSG("Ignoring %s request for %s endpoint %s", prot.c_str(), B.prot.c_str(), name.c_str());
            continue;
          }
          ...
          if (ph.size() > 6 && !B.porthost6.size()) {
            if (addi.size()) {
              INFO_MSG("🌊 Castaway! %s %s cannot ride the IPv6 waves toward %s (%s)!", hostType,
                       U.getRemoteAddr().toString().c_str(), name.c_str(), addi.c_str());
            } else {
              INFO_MSG("🌊 Castaway! %s %s cannot ride the IPv6 waves toward %s!", hostType,
                       U.getRemoteAddr().toString().c_str(), name.c_str());
            }
            continue;
          } else if (ph.size() == 6 && !B.porthost4.size()) {
            if (addi.size()) {
              INFO_MSG("🌊 Castaway! %s %s cannot ride the IPv4 waves toward %s (%s)!", hostType,
                       U.getRemoteAddr().toString().c_str(), name.c_str(), addi.c_str());
            } else {
              INFO_MSG("🌊 Castaway! %s %s cannot ride the IPv4 waves toward %s!", hostType,
                       U.getRemoteAddr().toString().c_str(), name.c_str());
            }
            continue;
          }
          ...
          if (addi.size()) {
            INFO_MSG("🛟 Castaway! %s %s is swimming toward %s %s (%s)!", hostType,
                     U.getRemoteAddr().toString().c_str(), B.firewallName(), name.c_str(), addi.c_str());
          } else {
            INFO_MSG("🛟 Castaway! %s %s is swimming toward %s %s!", hostType, U.getRemoteAddr().toString().c_str(),
                     B.firewallName(), name.c_str());
          }
          { // Send punch instruction to client side
            std::string reply("FOGH0123456789ABCDEF\003\000\000", 23);
            if (addi.size()) {
              Foghorn::writeUint16(reply, 21, addi.size());
              reply += addi;
            }
            reply += usePortHost;
          ...
          if (!B.isOpen) { // Send punch instruction to server side, if not open port
            ...
            reply += ph;
            // Add random port number, since we might need it
            if (B.firewallType() == FIRE_PREDICTABLE) { reply.append(usePortHost.data(), 2); }
            Foghorn::calcHash(reply, passphrase);
            U.SendNow(reply);
          }
          ...
          if (U.data.size() != 23 && U.data.size() != 25) { continue; }
          uint16_t startEntry = 0;
          if (!Foghorn::readUint16(U.data, U.data.size(), 21, startEntry)) { continue; }
          size_t endEntry = startEntry;
          if (U.data.size() == 25) {
            uint16_t requestedEnd = 0;
            if (!Foghorn::readUint16(U.data, U.data.size(), 23, requestedEnd)) { continue; }
            endEntry = requestedEnd;
          }
          ...
          Foghorn::writeUint16(reply, 23, totalEntry);
          for (std::map<std::string, binding>::iterator it = bindings.begin(); it != bindings.end(); ++it) {
            if (it->second.porthost4.size()) {
              if (currEntry >= startEntry && currEntry <= endEntry) {
    ...
    }
    {
      ...
      for (std::map<std::string, binding>::iterator it = bindings.begin(); it != bindings.end(); ++it) {
        if (it->second.outdated(currTime)) { toDelete.insert(it->first); }
      }
