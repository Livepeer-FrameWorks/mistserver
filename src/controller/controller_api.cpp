...
#include "../icon.h"
    ...
    if (aConn->H.url.size() > 14 && aConn->H.url.substr(0, 14) == "/translations/" &&
      ...
      const std::string code = aConn->H.url.substr(14, aConn->H.url.size() - 19);
      const std::string path = "/" + code + ".json";
      const bool validCode = !code.empty() &&
                             code.find_first_not_of("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-") ==
                                 std::string::npos;
      const EmbeddedFile *entry = validCode ? vfsLookup(translations_vfs, translations_vfs_count, path.c_str()) : 0;
      aConn->H.Clean();
      aConn->H.SetHeader("Server", APPIDENT);
...
#include "server_variants.h"
      ...
      for (int i = 0; i < lsp_variant_count; i++){
        ...
        if (url.substr(0, pfx.size() + 1) == pfx + "/"){
          variant = &lsp_variants[i];
          filePath = url.substr(pfx.size());
          break;
        }
      }
...
///
  ...
  if (Request.isMember("camera_config")){Controller::cameraConfigure(Request["camera_config"], Response["camera_config"]);}
  if (Request.isMember("camera_list")){Controller::listCameras(Response["camera_list"]);}
  if (Request.isMember("camera_remove")){Controller::removeCamera(Request["camera_remove"], Response["camera_list"]);}
  if (Request.isMember("camera_associate")){
    Controller::associateCameras(Request["camera_associate"], Response["camera_associate"]);
  }
  if (Request.isMember("camera_disassociate")){
    Controller::disassociateCamera(Request["camera_disassociate"], Response["camera_disassociate"]);
  }
  if (Request.isMember("camera_query")){Controller::sendCommand(Request["camera_query"], Response["camera_query"]);}
  if (Request.isMember("camera_update")){Controller::updateCamera(Request["camera_update"], Response["camera_list"]);}
  if (Request.isMember("camera_create_stream")){Controller::createCameraStream(Request["camera_create_stream"], Response["camera_create_stream"]);}
  if (Request.isMember("camera_presets")){Controller::listPresets(Request["camera_presets"], Response["camera_presets"]);}
