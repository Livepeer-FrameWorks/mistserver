...
namespace Controller {
  ...
  // Per-device command serialization token. DeviceInfo itself is guarded by CameraRegistry.
  class CameraEntry {
      friend class CameraRegistry;
  ...
  class CameraRegistry {
      ...
      std::map<std::string, std::string> endpointIndex;

      std::string canonicalKey(const ::Device::DeviceInfo & dev) const;
      std::string resolveIdLocked(const std::string & id) const;
      void indexEntryLocked(const std::string & cameraId, const ::Device::DeviceInfo & info);
      void unindexEntryLocked(const ::Device::DeviceInfo & info);
    ...
    public:
      ...
      // Return an immutable copy; network operations must use snapshots rather than CameraEntry::info.
      bool snapshot(const std::string & id, ::Device::DeviceInfo & out) const;
      std::vector<std::pair<std::string, ::Device::DeviceInfo>> snapshots() const;

      // Mutate DeviceInfo while holding the registry lock and keep the endpoint index in sync.
      bool update(const std::string & id, std::function<void(::Device::DeviceInfo &)> fn);

      size_t size() const;

      // Merge discovered/enriched devices (exclusive lock, additive update)
      void merge(const std::vector<::Device::DeviceInfo> & devices);
      std::string mergeOne(const ::Device::DeviceInfo & device);

      // Explicitly combine/split logical cameras. Returned IDs are empty on validation failure.
      std::string associate(const std::string & primaryId, const std::string & secondaryId,
                            std::string & error);
      std::string disassociate(const std::string & id, const std::string & protocol,
                               std::string & error);
      size_t inferUniqueIpAssociations();

      // Remove device by ID or canonical key
      void remove(const std::string & id);

      // Remove a device only if its sole protocol is `proto` (used to drop ephemeral, discovery-only
      // sources such as NDI without disturbing multi-protocol / manually-configured cameras).
      // Returns true if a camera was removed.
      bool removeIfProtocolOnly(const std::string & id, const std::string & proto);
      ...
      void forEachDevice(std::function<void(const std::string & key, const ::Device::DeviceInfo & info)> fn) const;
  };
