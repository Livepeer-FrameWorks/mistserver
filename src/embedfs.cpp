...
static inline void md5_add64(uint32_t *hash, const char *data){
  ...
  for (size_t i = 0; i < 16; ++i){
    M[i] = (uint8_t)data[i << 2] | ((uint8_t)data[(i << 2) + 1] << 8) |
           ((uint8_t)data[(i << 2) + 2] << 16) | ((uint8_t)data[(i << 2) + 3] << 24);
  }
...
static std::string md5hex(const std::string &input){
  ...
  std::stringstream ss;
  for (int i = 0; i < 16; ++i){
    ss << std::hex << std::setw(2) << std::setfill('0') << (unsigned)(uint8_t)out[i];
  }
  return ss.str();
}
...
int main(int argc, char *argv[]){
  ...
  for (int i = 1; i < argc; ++i){
    ...
    if (arg == "--output" && i + 1 < argc){
      ...
      std::cerr << "Usage: " << argv[0]
                << " --output <out.h> --prefix <prefix> [--vpath <vpath> <file>]... [--manifest <file>]"
                << std::endl;
      return 1;
    }
  }
  ...
  if (basedir.size()){
    for (size_t i = 0; i < entries.size(); ++i){
      if (entries[i].filepath[0] != '/'){
        entries[i].filepath = basedir + "/" + entries[i].filepath;
      }
    }
  }
  ...
  for (size_t i = 0; i < entries.size(); ++i){
    ...
    out << "  {\"" << entries[i].vpath << "\", " << varName << "_data, "
        << contents[i].size() << ", \"" << mime << "\", \"" << etag << "\"}," << std::endl;
  }
  out << "};" << std::endl;
